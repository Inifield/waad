...
void ExtractMapsFromMpq()
{
	...
	// Ajout BranRuz - WaadTeam, Creation d'un fichier pour completer la table WorldMap_Info
    FILE *WorldMapInfoFile;
	if( (WorldMapInfoFile  = fopen( "maps\\WorldMap_info.txt","wt" )) == NULL )
	{
		printf("WorldMap_Info.txt can't created!\n"); 
		printf("Pas de fichier Recap pour WorldMap_Info!\n");
	}
    //----
    ...
    for(uint32 i = 0; i < MapCount; ++i)
    {
		...
		if(WorldMapInfoFile) fprintf(WorldMapInfoFile, "%u;%s\n",map->id,map->name); // Ajout BranRuz - WaadTeam
	...
	// Ajout Branruz - WaadTeam
   if(WorldMapInfoFile) 
   {
     fclose(WorldMapInfoFile); 
     printf("  WorldMap_Info.txt successfull created!\n"); 
   } else printf(" No WorldMap_Info.txt created!\n"); 
...
{
	printf("Ascent map extractor for version 3.3.5.12340 \n");
	printf("Modified WaadTeam for worldMap_Info file\n");
	printf("============================================================\n\n");
