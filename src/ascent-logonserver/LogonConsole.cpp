...
{
	sLog.outString("Fichier de configuration recharger...");
	Rehash();
}
...
void LogonConsole::Kill()
{
	...
	printf("Attente de la fin du processus de la console....\n");
	while(_thread != NULL)
	{
		Sleep(100);
	}
	printf("Arret de la Console.\n");
#endif
}
...
bool LogonConsoleThread::run()
{
	...
	SetThreadName("Interprete de la Console");
...
void LogonConsole::ProcessCmd(char *cmd)
{
	...
	for (size_t i = 0; i < sizeof(cmds)/sizeof(SCmd); i++)
		...
		printf("Console:Commande de console inconnue (utiliser \"help\" pour l'aide).\n");
}
...
void LogonConsole::ProcessHelp(char *command)
{
	if (command == NULL)
	{
		sLog.outString("Console:--------aide--------");
		sLog.outString("   help, ?: affiche ce texte");
		sLog.outString("   reload: recharge les comptes");
		sLog.outString("   shutdown, exit: ferme le programme");
	}
}
//------------------------------------------------------------------------------
