...
void AuthSocket::HandleChallenge()
{
	...
	sLog.outDetail("[AuthChallenge] Reception Ok (%u octets)", full_size);
	...
	{
		sLog.outDebug("[AuthChallenge] Deconnexion.");
		Disconnect();
		return;
	}

	sLog.outDebug("[AuthChallenge] Reception complete.");
...
//#endif
	...
	if(build < LogonServer::getSingleton().min_build)
	{
		...
		Log.Debug("Patch", "Patch %u%s selectionne pour le client.", m_patch->Version,m_patch->Locality);
	...
	sLog.outDebug("[AuthChallenge] Nom du compte: \"%s\"", AccountName.c_str());

	m_account = AccountMgr::getSingleton().GetAccount(AccountName);
	if(m_account == 0)
	{
		sLog.outDebug("[AuthChallenge] Compte invalide.");
	...
	sLog.outDebug("[AuthChallenge] Etat du compte : Banni = %u", m_account->Banned);
...
void AuthSocket::HandleProof()
{
	...
	if(m_patch&&!m_account)
	{
		...
		sLog.outDebug("[AuthLogonProof] Initialisation PatchJob");
	...
	sLog.outDebug("[AuthLogonProof] Intercalation et verification de preuve...");
	...
	if(memcmp(lp.M1, M.AsByteArray(), 20) != 0)
	{
		...
		//SendProofError(4, 0);
		SendChallengeError(CE_NO_ACCOUNT);
		sLog.outDebug("[AuthLogonProof] les valeurs M1 ne correspondent pas.");
		return;
	}
	...
	sLog.outDebug("[AuthLogonProof] Succes d'identification.");
...
void AuthSocket::OnRead()
{
	...
	if(Command < MAX_AUTH_CMD && Handlers[Command] != NULL)
		...
		Log.Notice("AuthSocket", "Commande inconnue %u", Command);
}
...
void AuthSocket::HandleReconnectChallenge()
{
	...
	uint16 full_size = *(uint16*)&ReceiveBuffer[2];
	sLog.outDetail("[AuthChallenge] Recu. (%u octets recues)",full_size);
	...
	sLog.outDebug("[AuthChallenge] Trame Ok.");
...
{
	sLog.outDebug("Transfert accepte");
	if(!m_patch)
		return;
...
{
	sLog.outDebug("Reprise du transfert");
	if(!m_patch)
		return;
