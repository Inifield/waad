...
{
	setBusy.Acquire();
	if(!silent) sLog.outString("[AccountMgr] Rechargement des comptes...");
	...
	if(!silent) sLog.outString("[AccountMgr] %u comptes trouves.", AccountDatabase.size());
	setBusy.Release();
...
void AccountMgr::UpdateAccount(Account * acct, Field * field)
{
	...
	if(id != acct->AccountId)
	{
		...
		sLog.outColor(TYELLOW, " >> supression de compte en double %u [%s]...", id, Username.c_str());
	...
	if ((uint32)UNIXTIME > acct->Banned && acct->Banned != 0 && acct->Banned != 1) //1 = perm ban?
	{
		//Accounts should be unbanned once the date is past their set expiry date.
		acct->Banned = 0;
		sLog.outDebug("Le ban du compte %s a expire.",acct->UsernamePtr->c_str());
		sLogonSQL->Execute("UPDATE accounts SET banned = 0 WHERE acct=%u",acct->AccountId);
	}
	acct->SetGMFlags(GMFlags.c_str());
	if(strcmp(field[7].GetString(), "enUS"))
...
void IPBanner::Reload()
{
	...
	if( result != NULL )
	{
		do 
		{
			...
			if( i == string::npos )
			{
				printf("IP ban \"%s\" non indique. Supposition /32 \n", ip.c_str());
			}
			else
				smask = ip.substr(i+1);
			...
			if( ipraw == 0 || ipmask == 0 )
			{
				printf("IP ban \"%s\" ne peut etre analyse. Ignore\n", ip.c_str());
				continue;
			}
...
void InformationCore::SendRealms(AuthSocket * Socket)
{
	...
	for(; itr != m_realms.end(); ++itr)
	{
		...
		data << uint8(itr->second->TimeZone); // 1);   // time zone
		data << uint8(6);
	}
	realmLock.Release();
...
void InformationCore::TimeoutSockets()
{
	...
	for(itr = m_serverSockets.begin(); itr != m_serverSockets.end();)
	{
		...
		{
			// ping timeout
			printf("Fermeture du socket en raison du ping timeout.\n");
			s->removed = true;
...
void InformationCore::CheckServers()
{
	...
	for(itr = m_serverSockets.begin(); itr != m_serverSockets.end();)
	{
		...
		{
			printf("Socket deconnecte: %s en raison d'une IP qui n'est plus allouee.\n", s->GetRemoteIP().c_str());
			s->Disconnect();
		}
	}
