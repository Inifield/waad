...
PatchMgr::PatchMgr()
{
	// load patches
#ifdef WIN32
	Log.Notice("PatchMgr", "Chargement des patches...");
	...
	do 
	{
		...
		Log.Notice("PatchMgr", "Patch trouve pour b%u locale `%s`.", srcversion,locality);
		pPatch = new Patch;
	 ...
	 *nix patch loader
	 */
	Log.Notice("PatchMgr", "Chargement de patches...");
	char Buffer[MAX_PATH*10];
	char Buffer2[MAX_PATH*10];
	char Buffer3[MAX_PATH*10];
	...
	{
		Log.Error("PatchMgr", "Pas de patch trouve.");
		return;
	}
	...
	while(filecount--)
	{
		...
		{
			printf("Ne peut pas stat %s\n", Buffer3);
			continue;
		}

		Log.Notice("PatchMgr", "Patch trouve pour b%u locale `%s` (%u octets).", srcversion,locality, sb.st_size);
		pPatch = new Patch;
		size = sb.st_size;
		pPatch->FileSize = size;
		pPatch->Data = new uint8[size];
