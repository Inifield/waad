...
void LogonCommServerSocket::OnConnect()
{
	...
	{
		printf("Connection server pour %s:%u REFUSER, IP non autorise.\n", GetRemoteIP().c_str(), GetRemotePort());
		Disconnect();
		return;
	}
...
void LogonCommServerSocket::HandlePacket(WorldPacket & recvData)
{
	...
	{
		printf("A obtenu un paquet inconnu de logoncomm: %u\n", recvData.GetOpcode());
		return;
	}
...
void LogonCommServerSocket::HandleRegister(WorldPacket & recvData)
{
	...
	sLog.outString("Enregistrement du royaume `%s` sous l'ID %u.", realm->Name.c_str(), my_id);
...
void LogonCommServerSocket::HandleSQLExecute(WorldPacket & recvData)
{
	...
	printf("!! HandleSQLExecute Erreur. %s\n", GetRemoteIP().c_str());
}

void LogonCommServerSocket::HandleReloadAccounts(WorldPacket & recvData)
{
	printf("!! HandleReloadAccounts Erreur. %s\n", GetRemoteIP().c_str());
...
void LogonCommServerSocket::HandleAuthChallenge(WorldPacket & recvData)
{
	...
	sLog.outString("Demande d'identification de %s, resultat %s.", GetRemoteIP().c_str(), result ? "OK" : "ECHEC");
...
void LogonCommServerSocket::HandleMappingReply(WorldPacket & recvData)
{
	...
	{
		printf("Decompression du mapping rate.\n");
		return;
	}
...
void LogonCommServerSocket::HandleTestConsoleLogin(WorldPacket & recvData)
{
	...
	recvData.read(key, 20);
	sLog.outDebug("LogonCommServerSocket : Testing console login: %s\n", accountname.c_str());
	...
	if((pAccount->GMFlags == NULL) || (strstr(pAccount->GMFlags, "255:") == NULL) )
	{
		sLog.outError("RemoteConsole: Player %s Flag: %s , Connection non permise!\n", pAccount->UsernamePtr,pAccount->GMFlags);
		data << uint32(0);
		SendPacket(&data);
		return;
	}
	...
	{
		sLog.outError("RemoteConsole: Player %s Flag: %s , Connection non permise,Erreur HKey!\n", pAccount->UsernamePtr,pAccount->GMFlags);
		data << uint32(0);
		SendPacket(&data);
		return;
	}
...
void LogonCommServerSocket::HandleDatabaseModify(WorldPacket& recvData)
{
	...
	case 5:		// ip ban reomve
     ...
     case 6:	// DK created ( Wrath Extension ) (WaadTeam - Branruz)
		{       // Il faut pouvoir effacer le flag si le DK est effacé, Param est '0' ou '1'
			uint32 _guid;
			uint32 _OneDKCreated;
			recvData >> _guid >> _OneDKCreated;

			if(_OneDKCreated)
             sLogonSQL->Execute("UPDATE `accounts`,`characters` SET `AlreadyDK` = %u WHERE `accounts`.`acct` = `characters`.`acct` AND `characters`.`guid` = %u", _guid, _guid);
			else
			 sLogonSQL->Execute("UPDATE `accounts` SET `AlreadyDK` = 0 WHERE `AlreadyDK` = %u",_guid);
		}break;
	}
}
