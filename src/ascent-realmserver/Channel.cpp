...
Channel::Channel(const char * name, uint32 team, uint32 type_id, uint32 id)
{
	...
	{
		m_minimumLevel = 1; //Les newbies y ont droit aussi !!!
		m_general = true;
		m_announce = false;
	}
}
...
void Channel::AttemptJoin(RPlayerInfo* plr, const char * password)
{
	...
	if(m_members.find(plr) != m_members.end())
	{
		if( !m_general )
		{
			data << uint8(CHANNEL_NOTIFY_FLAG_ALREADY_ON) << m_name << uint64(plr->Guid);
			plr->GetSession()->SendPacket(&data);
		}
...
void Channel::Part(RPlayerInfo* plr, bool silent)
{
	...
	if(plr->GetSession())
	{
		if( !silent )
		{
			data << uint8(CHANNEL_NOTIFY_FLAG_YOULEFT) << m_name << m_typeId << uint8(0);
			plr->GetSession()->SendPacket(&data);
		}
	}
...
Channel::~Channel()
{
	...
	for(MemberMap::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
	{
		data << uint8(CHANNEL_PART); //left channel
