...
void WServer::HandleTeleportRequest(WorldPacket & pck)
{
	...
	sLog.outDebug("TeleportRequest", "session %u, mapid %u, instanceid %u", sessionid, mapid, instanceid);
	...
	if(s)
	{
		...
		if (dest == NULL)
		{
			sLog.outDebug("TeleportRequest", "Could not find instance, will use prototype...");
		...
		if(!dest)
		{
			sLog.outDebug("TeleportRequest", "INSTANCE_ABORT_NOT_FOUND");
...
void WServer::HandleChannelAction(WorldPacket & pck)
{
	...
	default:
		{
			sLog.outDebug("WServer", "HandleChannelAction opcode, unhandled action %u", action);
			return;
		}
	}
}
...
void WServer::HandleChannelUpdate(WorldPacket & pck)
{
	...
	sLog.outDebug("WServer", "Received ICMSG_CHANNEL_UPDATE opcode, update type %u", updatetype);
	...
	case UPDATE_CHANNELS_ON_ZONE_CHANGE:
		{
			sLog.outDebug("WServer", "HandleChannelUpdate opcode, UPDATE_CHANNELS_ON_ZONE_CHANGE");
			...
			for( ; itr != m_channels.end(); )
			{
				...
				if( pChannel->m_flags & 0x10 && !(pChannel->m_flags & 0x40) && pChannel->pDBC )
				{
					...
					pChannel = channelmgr.GetCreateChannel(strbuf, plr, dbcid);
	...
	default:
		{
			sLog.outDebug("WServer", "HandleChannelUpdate opcode, unhandled update type %u", updatetype);
			return;
		}
	}
...
void WServer::HandleChannelLFGDungeonStatusReply(WorldPacket& pck)
{
	...
	sLog.outDebug("LfgChannelJoin", "%s, unk %u", channelname.c_str(), unk);
}
