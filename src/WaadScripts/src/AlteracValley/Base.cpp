...
{
 SetDisplayWeaponIds(pMainHand ? _unit->proto->ItemEntry1 : 0, 0, 0, 
	                 pOffHand  ? _unit->proto->ItemEntry2 : 0, 0, 0 );
...
SpellDesc* MoonScriptCreatureAI::AddSpell(uint32 pSpellId, TargetType pTargetType, float pChance, float pCastTime, int32 pCooldown, float pMinRange, float pMaxRange, bool pStrictRange, char* pText, TextType pTextType, uint32 pSoundId)
{
	...
	if ( Cooldown == 0 ) int32 Cooldown = Info->categoryRecoveryTime; // Ajouté par randdrick - Permet de récupérer le temps de rechargement du sort (cooldown) dans le spell.dbc, si la valeur est à 0. En principe ...
