...
public:
	void GossipHello(Object *pObject, Player *plr, bool AutoSend)
	{
		...
		if( pObject->GetUInt32Value(UNIT_NPC_FLAGS) & UNIT_NPC_FLAG_VENDOR )
			Menu->AddItem( 0, "Je souhaiterais voir vos marchandises.", 3 );

		if( ZMg_superiorTeam == Team && ZMCityOwners != Team && !plr->HasAura( Team == 0 ? 32430 : 32431 ) )
			Menu->AddItem( 0, Team == 0 ? "Donnez moi le drapeau, Je le planterais dans la tour centrale pour la gloire de l'Alliance!" : "Donnez moi le drapeau, Je le planterais dans la tour centrale pour la gloire de la Horde!" , Team+1);
		if(AutoSend)
			Menu->SendTo(plr);
    }
