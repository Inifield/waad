...
public:
	...
	{
		// Deja init au chargement de la Dll qd le player entre dans la zone
		switch(towerid)
		{
         case TOWER_STADIUM: 
		 case TOWER_BROKENHILL:
		 case TOWER_OVERLOOK:
			 Log.Notice("[Zone PvP]","%s est %s %s",pPlayer->GetName(),(ControlPointName[0]=='s') ? "au":"a la",ControlPointName);
			                 break;
    ...
    void OnZoneHook(Player *pPlayer, uint32 Zone, uint32 OldZone)
    {
	 ...
	 {
		if( g_superiorTeam == pPlayer->GetTeam() )
...
void SpawnObjects(MapMgr *pmgr)
{
	...
	for(i = 0; i < 3; ++i)
	{
		...
		Log.Notice("[SpawnObjects]","Spawned gameobject entry %u for world pvp on hellfire.\n", p->entry);
	}
}
...
void SetupPvPHellfirePeninsula(ScriptMgr *mgr)
{
	...
	// Concerne essentiellement un Player
	// Est a placé dans le GameObjectAIScript, est maintenant rafraichi par le OnZoneHook
	// (Meme principe que pour le OnActivatePosition)
	//mgr->register_hook(SERVER_HOOK_EVENT_ON_ZONE, ZoneHook);

	// Il faut faire l'enregistrement apres avoir spawn les GameObjects complementaires
	// C'est seulement ensuite que le register Mgr intervient (voir code dans SpawnObjects)
	// Le registerHook appel SpawnObjects apres son referencement dans la table du ServerHook
	// Cette zone est persistante et doit etre init des la creation du serveur et pas sur
	// intervention d'un player
