...
//	###		Misc		###
	...
	SetupUnsorted(mgr);
// 	WaadScripts
	SetupQueteEpreuveLegion(mgr);  // Franck
	SetupQueteLivreAnciens(mgr);   // Franck
	SetupQueteArikara(mgr);        // BB
	SetupScarletEnclave(mgr);		// Crash
	SetupToundraBoreenne(mgr);		// Crash
	SetupZulDrak(mgr);				// Crash
	SetupLespicsfoudroyes(mgr);		// Crash
	SetupLesgrisonnes(mgr);			// Crash
	SetupLacouronnedeglace(mgr);	// Crash
	SetupJougdhiver(mgr);			// Crash
	SetupForetduchantdecristal(mgr);	// Crash
	SetupFjordhurlant(mgr);			// Crash
	SetupDesolationdesdragons(mgr);		// Crash
	SetupDalaran(mgr);				// Crash
	SetupBassindesholazar(mgr);		// Crash
	SetupTeldrassil(mgr);           // Branruz
}
