	...
	void GossipSelectOption(Object* pObject, Player* plr, uint32 Id, uint32 IntId, const char * EnteredCode)
    {
		...
		switch (IntId)
		{
			...
			if(en && en->GetMobCount(2) < en->GetQuest()->required_mobcount[2])
			{
				...
				en->SendUpdateAddKill(2,0); // A Verif: Trouver le GUID (Branruz)
				en->UpdatePlayerFields();
				return;
			}break;
			}
...
bool MarquerunMurloc(uint32 i, Spell * pSpell)
{
  ...
  Creature * murloc = (Creature*)pSpell->GetUnitTarget();
  if(!murloc)
	  return true;
...
void Setup( ScriptMgr * mgr )
{
   ...
   mgr->register_dummy_spell(30877, &MarquerunMurloc);
}