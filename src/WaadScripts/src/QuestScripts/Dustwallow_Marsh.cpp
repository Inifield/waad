...
bool CouronneHyal(uint32 i, Spell * pSpell)
{
	...
	GameObject * pHyal = sEAS.SpawnGameobject( pPlayer, 501541, pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), pPlayer->GetOrientation(), 1 );
	if ( pHyal != NULL )
	{
		sEAS.GameobjectDelete( pHyal, 2 * 60 * 1000 );
	}
...
void SetupDustwallowMarsh(ScriptMgr * mgr)
{
	...
	mgr->register_dummy_spell(42390, &CouronneHyal);
}
