...
public:
	...
	void OnActivate(Player * pPlayer)	
	{
		if(pPlayer->GetQuestLogForEntry(9678))
		{
			...
			if (Brazier)
			{
				Brazier->SetByte( GAMEOBJECT_BYTES_1,0, 0); //SetUInt32Value(GAMEOBJECT_STATE, 0);
...
public:
	...
	void OnDied(Unit *mKiller) 
	{
		...
		if (Brazier)
		{
			Brazier->SetByte( GAMEOBJECT_BYTES_1,0,1); //SetUInt32Value(GAMEOBJECT_STATE, 1);
		}
	}
};
...
bool DouseEternalFlame(uint32 i, Spell* pSpell)
{
	...
	Player *plr = static_cast<Player*>(pSpell->m_caster);
