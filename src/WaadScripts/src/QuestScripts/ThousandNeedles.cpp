...
// Quête : Saoulez les gobelins par Randdrick pour WAAD
class GetGoblinsDrunk : public QuestScript
{
public:
	...
	void OnQuestComplete( Player * mTarget, QuestLogEntry * qLogEntry)
	{
		...
		Creature* GobPitBoss = mTarget->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(SSX, SSY, SSZ, 4496);
		if(GobPitBoss == NULL)
			return;
			
		GobPitBoss->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "Ah la bière pour l'équipage ! A la votre !");
		sEAS.EventCastSpell(GobPitBoss, GobPitBoss, 20436, 30000);
...
void SetupThousandNeedles(ScriptMgr * mgr)
{
	...
	mgr->register_quest_script(1121, CREATE_QUESTSCRIPT(GetGoblinsDrunk));
}
