...
public:
	...
	void OnReachWP(uint32 iWaypointId, bool bForwards)
	{
		if(iWaypointId == 195)
		{
			_unit->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "Merci bien, jeune guerrier!");
	...
	void OnDied(Unit *mKiller)
	{
		...
		if(mKiller->IsPlayer())
		{
			...
			if(kolkarskilled > 8 && mPlayer->GetQuestLogForEntry(851))
			{
				...
				kolkarskilled = 0;
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je me suis fait massacré! J'appel Verog!");
			}
		}
	}
