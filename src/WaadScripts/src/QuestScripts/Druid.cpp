...
public:
  ...
  {
		// fix par franck77
	if(!mTarget->HasSpell(18960))
        mTarget->addSpell(18960);

  }
};

// Par Randdrick pour la communauté de WAAD - Projet WaadScripts -
class NoMereDreamQuest : public QuestScript 
{ 
public:
	void OnQuestStart(Player *pPlayer, QuestLogEntry *pQuest)
	{
		...
		Creature *pClintarSpirit = sEAS.SpawnCreature( pPlayer, 22916, 7485.16f, -3090.72f, 440.283f, 0.9574f, 0 );
		if ( pClintarSpirit == NULL )
			return;
	}		
};
...
void SetupDruid(ScriptMgr * mgr)
{
  ...
  QuestScript *NoMereDream = (QuestScript*) new NoMereDreamQuest();
  mgr->register_quest_script(5921, Moonglade);
  mgr->register_quest_script(5922, Moonglade);
  mgr->register_quest_script(10965, NoMereDream);
