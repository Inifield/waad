...
public:
	...
	void OnActivate(Player * pPlayer)
	{
  		...
  		if(qle)
		{
			...
			if (Orb)
			{
				Orb->SetByte( GAMEOBJECT_BYTES_1,0, 0); //SetUInt32Value(GAMEOBJECT_STATE, 0);
  				qle->SetMobCount(0, 1);
	...
	if(Bloodoath != NULL)
	{
		if(qle->GetMobCount(1) < qle->GetQuest()->required_mobcount[1])
		{
			if(plr->CalcDistance(plr, Bloodoath) < 15)
			{
				...
				qle->SendUpdateAddKill(1,0); // A Verif: Trouver le GUID (Branruz)
				qle->UpdatePlayerFields();
				for(uint8 i = 0; i < 2; i++)
				{
	...
	if(Dawnchaser != NULL)
	{
		if(plr->CalcDistance(plr, Dawnchaser) < 15)
		{
			if(qle->GetMobCount(2) < qle->GetQuest()->required_mobcount[2])
			{
				...
				qle->SendUpdateAddKill(2,0); // A Verif: Trouver le GUID (Branruz)
				qle->UpdatePlayerFields();
				for(uint8 i = 0; i < 2; i++)
				{
