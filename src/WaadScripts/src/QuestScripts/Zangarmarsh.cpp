			...
			case 17901:
				{
					...
					{
						en->SetMobCount(1, 1);
						en->SendUpdateAddKill(1,0); // A Verif: Trouver le GUID (Branruz)
						en->UpdatePlayerFields();
					}
...
bool Unespritallie(uint32 i, Spell * pSpell) 
{
	...
	Creature * espritallie = sEAS.SpawnCreature(pPlayer, 18185, -353, 7255, 49.36, 6.28, 0);
	espritallie->Despawn(6*60*1000, 0);
	return true;
}
...
void SetupZangarmarsh(ScriptMgr * mgr)
{
	...
	mgr->register_dummy_spell(32037, &Unespritallie);
}
