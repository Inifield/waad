...
void Hanazua(Player * pPlayer, Object * pObject)
{
	...
	say = "Allez vite vite, ";
	say+= pPlayer->GetName();
	say+= ", mon destin est entre vos mains.";
...
{
	((Creature*)pObject)->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, 
		//"Greatmother Hawkwind gestures to the pitcher of water sitting on the edge of the well.");
	     "Grandmere Hawkwind fait des gestes dans l'eau reposant sur le bord du puits.");
}
...
{
	((Creature*)pObject)->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "Tue Sarkoth !!");
}

void TheFamilyAndTheFishingPole(Player* pPlayer, Object* pObject)
{
	((Creature*)pObject)->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, 
		//"I got some extras with me; if you pay me back what i paid for them, you can have one. I got some bait too.");
		"J'ai eu quelques frais supplémentaires; si vous me remboursé ce que j'ai payé, vous pourrez en avoir un. J'ai l'amorce aussi.");
}

void MillysHarvest(Player * pPlayer, Object * pObject)
{
	((Creature*)pObject)->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "J'espere pour vous!");
...
{
	((Creature*)pObject)->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, 
		//"Think of small pumpkins, my friend!!");
		"Pensez aux petits potirons, mon ami!!");
...
{
	char msg[256];
	snprintf((char*)msg, 256, "Vous etes ecarté %s . ", pPlayer->GetName());
...
{
	Log.Success("[Script]","QuestHooks.cpp => OnQuestAccept ( %s : Quete %u )",pPlayer->GetName(),pQuest->id);
