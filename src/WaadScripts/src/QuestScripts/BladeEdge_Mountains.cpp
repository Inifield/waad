	...
	void OnDied(Unit * mKiller)
	{
		if (mKiller->IsPlayer()) 
		{
			...
			if(en && en->GetMobCount(0) < en->GetQuest()->required_mobcount[0])
			{
				...
				en->SendUpdateAddKill(0,0); // A Verif: Trouver le GUID (Branruz)
...
{
public:           //EventableObject GameObject
	...
	void OnActivate(Player * pPlayer)
	{
		...
		if(te) pPlayer->event_AddEvent(te);
	}
	...
	void ResetGO(Player* plr)
	{
		...
		if (obelisk1 != NULL)
			obelisk1->SetByte(GAMEOBJECT_BYTES_1,0,1) ; //SetUInt32Value(GAMEOBJECT_STATE, 1);
		if (obelisk2 != NULL)
			obelisk2->SetByte(GAMEOBJECT_BYTES_1,0,1) ; //SetUInt32Value(GAMEOBJECT_STATE, 1);
		if (obelisk3 != NULL)
			obelisk3->SetByte(GAMEOBJECT_BYTES_1,0,1) ; //SetUInt32Value(GAMEOBJECT_STATE, 1);
		if (obelisk4 != NULL)
			obelisk4->SetByte(GAMEOBJECT_BYTES_1,0,1) ; //SetUInt32Value(GAMEOBJECT_STATE, 1);
		if (obelisk5 != NULL)
			obelisk5->SetByte(GAMEOBJECT_BYTES_1,0,1) ; //SetUInt32Value(GAMEOBJECT_STATE, 1);
	}
};
...
{
	if(pSpell->m_caster == NULL || !pSpell->m_caster->IsPlayer())
		return true;
...
public:
	...
	void OnDied(Unit * mKiller)
	{
		...
		if ( pQuest->GetMobCount( 0 ) < pQuest->GetQuest()->required_mobcount[0] )
		{
			...
			pQuest->SendUpdateAddKill( 0 ,0); // A Verif: Trouver le GUID (Branruz)
...
bool AppeldeRexxar(uint32 i, Spell * pSpell)
{
...
bool FiletDragonnet(uint32 i, Spell * pSpell)
{
	Player * pPlayer = (Player*)pSpell->m_caster;
	if(!pPlayer)
		return true;
	...
	Creature * dragonnet = (Creature*)pSpell->GetUnitTarget();
	if(!dragonnet)
		return true;

    dragonnet->Despawn(1000, 6*60*1000);
...
void SetupBladeEdgeMountains(ScriptMgr * mgr)
{
	...
	mgr->register_dummy_spell(39223, &AppeldeRexxar);
	mgr->register_dummy_spell(38177, &FiletDragonnet);
}
