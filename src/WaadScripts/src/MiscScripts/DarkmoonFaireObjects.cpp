...
public:
	...
	void OnActivate(Player * pPlayer)
	{
		...
		tonkConsole->SetByte( GAMEOBJECT_BYTES_1,0,1); //SetUInt32Value(GAMEOBJECT_FLAGS, 1);
		tonkConsole->SetByte( GAMEOBJECT_BYTES_1,0,0); //SetUInt32Value(GAMEOBJECT_STATE, 0);
	...
	void AIUpdate()
	{
		if (!Plr->HasAura(33849) || Tonk->isDead())
		{
			...
			Console->SetByte( GAMEOBJECT_BYTES_1,0,0); //SetUInt32Value(GAMEOBJECT_FLAGS, 0);
			Console->SetByte( GAMEOBJECT_BYTES_1,0,1); //SetUInt32Value(GAMEOBJECT_STATE, 1);
			RemoveAIUpdateEvent();
		}
	}
