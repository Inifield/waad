...
public:
  ...
  {
    _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Qui ose me deranger");
  }
};
...
// Undercity Guardian par Randdrick pour Waad
// should activate only on target with <= 20% hp - http://fr.wowhead.com/?spell=7160

class UndercityGuardian : public CreatureAIScript
{
public:
	ADD_CREATURE_FACTORY_FUNCTION(UndercityGuardian);
	SP_AI_Spell spells[1];
    bool m_spellcheck[1];
	
    UndercityGuardian(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		nrspells = 1;
		...
		spells[0].info = dbcSpell.LookupEntry(__Execute);
		spells[0].targettype = TARGET_RANDOM_ENNEMIS;
	...
	void CastSpellOnRandomTarget(uint32 i, float mindist2cast, float maxdist2cast, int minhp2cast, int maxhp2cast)
	{
		...
		if(_unit->GetCurrentSpell() == NULL && _unit->GetAIInterface()->GetNextTarget())
        {
			...
			{ 
				...
				if (((spells[i].targettype == TARGET_RANDOM_ENNEMIS && isHostile(_unit, (*itr))) || (spells[i].targettype == TARGET_RANDOM_ENNEMIS && isFriendly(_unit, (*itr)) && (*itr) != _unit)) && ((*itr)->GetTypeId()== TYPEID_UNIT || (*itr)->GetTypeId() == TYPEID_PLAYER) && (*itr)->GetInstanceID() == _unit->GetInstanceID()) // isAttackable(_unit, (*itr)) && 
				{
					Unit* RandomTarget = NULL;
					RandomTarget = (Unit*)(*itr);

					if (RandomTarget->isAlive() && _unit->GetDistance2dSq(RandomTarget) >= mindist2cast*mindist2cast && _unit->GetDistance2dSq(RandomTarget) <= maxdist2cast*maxdist2cast && ((RandomTarget->GetHealthPct() >= minhp2cast && RandomTarget->GetHealthPct() <= maxhp2cast && spells[i].targettype == TARGET_RANDOM_FRIEND) || (_unit->GetAIInterface()->getThreatByPtr(RandomTarget) > 0 && isFriendly(_unit, RandomTarget))))
					{
						TargetTable.push_back(RandomTarget);
					} 
				}
			...
			if (_unit->GetHealthPct() >= minhp2cast && _unit->GetHealthPct() <= maxhp2cast && (spells[i].targettype == TARGET_RANDOM_FRIEND || spells[i].targettype == TARGET_RANDOM_ENNEMIS))
			{	
...
void SetupMiscCreatures(ScriptMgr *mgr)
{
	...
	//Guardian
	mgr->register_creature_script(5624, &UndercityGuardian::Create);
}
