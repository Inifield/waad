 ...
 * WaadScripts est un projet de la communauté Waad soumis à la 
...
public:
	...
	void OnActivate(Player * pPlayer)
	{
		...
		pDoor-> SetByte(GAMEOBJECT_BYTES_1,0, 0); // SetUInt32Value(GAMEOBJECT_STATE, 0);
	}
};
...
public:
	...
	void OnActivate(Player * pPlayer)
	{
		...
		if(qle->GetMobCount(0) < qle->GetQuest()->required_mobcount[0])
		{
			...
			qle->SendUpdateAddKill(0,0); // A Verifier: Trouver le Guid des oeufs (Branruz)
...
public:
	...
	void OnActivate(Player * pPlayer)
	{
		...
		{
			qle->SetMobCount(0, qle->GetMobCount(0)+1);
			qle->SendUpdateAddKill(0,0); // A Verif: Trouver le GUID du Go (Branruz)
			qle->UpdatePlayerFields();
		}
		return;
	}
...
public:
	...
	void OnActivate(Player * pPlayer)
	{
		...
		{
			qle->SetMobCount(0, qle->GetMobCount(0)+1);
			qle->SendUpdateAddKill(0,0); // A Verif: Trouver le GUID du Mob (Branruz)
			qle->UpdatePlayerFields();
		}
...
public:
	...
	void OnActivate(Player * pPlayer)
	{
		...
		if(qle->GetMobCount(1) < qle->GetQuest()->required_mobcount[1])
		{
			...
			qle->SendUpdateAddKill(1,0); // A Verif: Trouver le GUID du Mob (Branruz));
			qle->UpdatePlayerFields();
		}
	}
};
...
public:
	...
	void OnActivate(Player * pPlayer)
	{
		...
		if(pQuest && pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
		{
			...
			pQuest->SendUpdateAddKill(0,0); // A Verif: Trouver le GUID du Mob (Branruz));
...
/*--------------------------------------------------------------------------------------------------------*/
// Par Randdrick pour la communauté de WAAD - Projet WaadScripts -

class Bougies_de_Malukaz : public GameObjectAIScript
{
public:
	Bougies_de_Malukaz(GameObject* goinstance) : GameObjectAIScript(goinstance) {}
	static GameObjectAIScript *Create(GameObject * GO) { return new Bougies_de_Malukaz(GO); }

	void OnActivate(Player * pPlayer)
	{
		if(pPlayer->GetQuestLogForEntry(10791))
		{
			float SSX = pPlayer->GetPositionX();
			float SSY = pPlayer->GetPositionY();
			float SSZ = pPlayer->GetPositionZ();
			...
			pPlayer->GetMapMgr()->GetInterface()->SpawnCreature(18384, SSX+2, SSY+2, SSZ, SSO, true, false, 0, 0)->Despawn(600000, 0);
		}
		else
		{
			pPlayer->BroadcastMessage("Quête manquante : L'esprit du Loup");
		}
	}
};
//----------------------------------------------------------------------------------------------------------
// 1ier Etage du Fort d'Ebene (191539)
...
public:
 	...
 	{
		pPlayer->SafeTeleport(609, 0,2396.622070,-5636.051270,377.062164,0.591405);  
 	}*/
	void OnSpawn()
	{
	...
	void AIUpdate()
	{
		...
		{
			plr->SafeTeleport(609, 0,2396.622070,-5636.051270,377.062164,0.591405);
		}
	}
 };
// Rez dechaussée du Fort d'Ebene  (191538)
...
public:
 	...
 	/*void OnActivatePosition(Player * pPlayer)	
 	{
		pPlayer->SafeTeleport(609, 0,2396.622070,-5636.051270,420.837524,0.591405);
 	}*/
	void OnSpawn()
	{
		RegisterAIUpdateEvent( 500 );
	...
	void AIUpdate()
	{
		...
		if( _gameobject->CalcDistance( _gameobject, plr ) <= 1.5f && plr->GetMapId() == 609)
		{
			plr->SafeTeleport(609, 0,2396.622070,-5636.051270,420.837524,0.591405);
		}
	}
 };
/*--------------------------------------------------------------------------------------------------------*/
...
class PrisonBreak : public GameObjectAIScript
{
public:
	PrisonBreak(GameObject* goinstance) : GameObjectAIScript(goinstance) {}
	static GameObjectAIScript *Create(GameObject * GO) { return new PrisonBreak(GO); }

	void OnActivate(Player * pPlayer)
	{
		if(!pPlayer)
			return;
		Log.Notice("OnActivate","Player %s a activer le script de fin de quete",pPlayer->GetName()); 

		QuestLogEntry *qle = pPlayer->GetQuestLogForEntry(11587);
		if(qle == NULL)
			return;
...
class Autel_de_la_maitrise_des_flots : public GameObjectAIScript
{
public:
	Autel_de_la_maitrise_des_flots(GameObject* goinstance) : GameObjectAIScript(goinstance) {}
	static GameObjectAIScript *Create(GameObject * GO) { return new Autel_de_la_maitrise_des_flots(GO); }
	...
	void OnActivate(Player * pPlayer)
	{
		...
		if(qle == NULL)
			...
			pPlayer->GetMapMgr()->GetInterface()->SpawnCreature(16292, SSX+2, SSY+2, SSZ, SSO, true, false, 0, 0)->Despawn(600000, 0);
	}
};
/*--------------------------------------------------------------------------------------------------------*/

class Aimant_fantome : public GameObjectAIScript
{
public:
	Aimant_fantome(GameObject* goinstance) : GameObjectAIScript(goinstance) {}
	static GameObjectAIScript *Create(GameObject * GO) { return new Aimant_fantome(GO); }
	...
	void OnActivate(Player * pPlayer)
	{
		...
		QuestLogEntry *qle = pPlayer->GetQuestLogForEntry(6134);
		if(qle == NULL)
			return;
			...
			pPlayer->GetMapMgr()->GetInterface()->SpawnCreature(11560, SSX+2, SSY+2, SSZ, SSO, true, false, 0, 0)->Despawn(600000, 0);

			if(qle->GetMobCount(8) < qle->GetQuest()->required_mobcount[0])
			{
				GameObject *gobj = pPlayer->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 177746);
				gobj->Despawn(6*60*1000);
			}
	}
};
...
void SetupGoHandlers(ScriptMgr * mgr)
{
	...
	mgr->register_gameobject_script(135139, &Bougies_de_Malukaz::Create);

	// Portail du DK (WaadTeam)
	...
	mgr->register_gameobject_script(187561, &PrisonBreak::Create);
	mgr->register_gameobject_script(181157, &Autel_de_la_maitrise_des_flots::Create);
	mgr->register_gameobject_script(177746, &Aimant_fantome::Create);
}
