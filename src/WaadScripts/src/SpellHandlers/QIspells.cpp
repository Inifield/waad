...
bool CleansingVial(uint32 i, Spell* pSpell)
{
	...
	if(!((Player* )pSpell->m_caster))
		return true;
	
	Player* pPlayer = ((Player* )pSpell->m_caster);
...
{
	if(!((Unit *)pSpell->m_caster))
		return true;

	Unit* pUnit = ((Unit *)pSpell->m_caster);
...
bool RuuanokClaw(uint32 i, Spell* pSpell)
{
	...
	((Player* )pSpell->m_caster)->GetMapMgr()->GetInterface()->SpawnCreature(21767, 3210.960693f, 5348.308594f, 144.537476f, 5.450696f, true, false, 0, 0);
	return true;
}
...
bool KarangsBanner(uint32 i, Spell* pSpell)
{
	...
	pPlayer->GetMapMgr()->GetInterface()->SpawnCreature(12921, 2231.710205f, -1543.603027f, 90.694946f, 4.700579f, true, false, 0, 0);
	pPlayer->GetMapMgr()->GetInterface()->SpawnCreature(12921, 2232.534912f, -1556.983276f, 89.744415f, 1.527570f, true, false, 0, 0);

	pPlayer->GetMapMgr()->GetInterface()->SpawnCreature(12757, 2239.357178f, -1546.649536f, 89.671097f, 3.530336f, true, false, 0, 0);
...
{
	if(!((Player* )pSpell->m_caster))
		((Player* )pSpell->m_caster)->AddQuestKill(10506, 0);
...
bool UnyieldingBattleHorn(uint32 i, Spell* pSpell)
{
	...
	Creature* creat = ((Player* )pSpell->m_caster)->GetMapMgr()->GetInterface()->SpawnCreature(19862, -1190.856079f, 2261.246582f, 46.625797f, 1.705882f, true, false, 0, 0);
...
{
	Player* p_caster = ((Player* )pSpell->m_caster);
	if(!p_caster)
		return true;
...
{
	if(!((Player* )pSpell->m_caster) || !pSpell->GetUnitTarget() || !pSpell->GetUnitTarget()->IsCreature())
		return true;
...
bool GoreBladder(uint32 i, Spell* pSpell)
{
	...
	if(!target || target->GetEntry() != 29392 || target->isAlive())
		return true;
...
{
	if(!((Player* )pSpell->m_caster)) return true;

	uint32 Weather = 46736 + RandomUInt(4);

	((Player* )pSpell->m_caster)->CastSpell(((Player* )pSpell->m_caster), dbcSpell.LookupEntry(Weather), true);
	return true;
}
...
bool PurifiedAshes(uint32 i, Spell* pSpell)
{
	...
	Unit* target = pSpell->GetUnitTarget();
	if(!target || target->GetEntry() != 26633 || target->isAlive())
		return true;
...
bool SummonShadra(uint32 i, Spell* pSpell)
{
	...
	((Player* )pSpell->m_caster)->GetMapMgr()->GetInterface()->SpawnCreature(2707, SSX, SSY, SSZ, SSO, true, false, 0, 0);
	return true;
}

bool SummonEcheyakee(uint32 i, Spell* pSpell)
{
	if(!((Player* )pSpell->m_caster) || i != 1)  //Effect_1 = SEND_EVENT,Effect_2 = DUMMY
		return true;
	...
	((Player* )pSpell->m_caster)->GetMapMgr()->GetInterface()->SpawnCreature(3475, SSX, SSY, SSZ, SSO, true, false, 0, 0);
...
bool SummonAquementas(uint32 i, Spell* pSpell)
{
	...
	((Player* )pSpell->m_caster)->GetMapMgr()->GetInterface()->SpawnCreature(9453, SSX, SSY, SSZ, SSO, true, false, 0, 0);
...
{
	QuestLogEntry* en = ((Player* )s->m_caster)->GetQuestLogForEntry(9427);
...
bool HunterTamingQuest(uint32 i, Aura* a, bool apply)
{
	...
	Player* p_caster = ((Player* )a->GetCaster());
...
bool ArcaneDisruption(uint32 i, Aura* pAura, bool apply)
{
	...
	if(pAura->GetCaster() == NULL)
		return false;

	Player* plr = ((Player* )pAura->GetCaster());
	...
	if(crate)
	{
		...
		go->CreateFromProto(190095, crate->GetMapMgr()->GetMapId(), crate->GetPositionX(), crate->GetPositionY(), crate->GetPositionZ(), crate->GetOrientation());
...
bool CenarionMoondust(uint32 i, Spell* pSpell) // Body And Heart (Alliance)
{
	...
	if(!pSpell->((Player *)pSpell->m_caster)->IsInWorld())
		return true;
	...
	//Moonkin Stone aura
	sEAS.SpawnGameobject(p_caster, 177644, 6331.01f, 88.245f, 22.6522f, 2.01455f, 1.0);
	...
	uint32 md = lunaclaw->GetCreatureInfo()->Male_DisplayID;
...
bool CenarionLunardust(uint32 i, Spell* pSpell)  // Body And Heart (Horde)
{
	...
	if(!((Player *)pSpell->m_caster)->IsInWorld())
		return true;
	...
	//Moonkin Stone aura
	sEAS.SpawnGameobject(p_caster, 177644, -2499.54f, -1633.03f, 91.8121f, 0.262894f, 1.0);
...
{
	Player* caster = ((Player *)pSpell->m_caster);
	if(caster == NULL)
		return true;
...
bool Triage(uint32 i, Spell* pSpell)
{
	...
	if(!((Player *)pSpell->m_caster) || pSpell->GetUnitTarget() == NULL) return true;
	((Player *)pSpell->m_caster)->CastSpell(pSpell->GetUnitTarget(), dbcSpell.LookupEntry(746), true);
	QuestLogEntry* en = ((Player *)pSpell->m_caster)->GetQuestLogForEntry(6624);
...
{
	if(!((Player *)pSpell->m_caster) || !((Player *)pSpell->m_caster)->IsInWorld())
		return true;
	...
	if(posX == 3747.07f || posX == 4023.5f || posX == 4126.12f )
	{
		pQuest->SendUpdateAddKill(0,0);
		pQuest->UpdatePlayerFields();
	}
...
{
	if(((Player *)pSpell->m_caster) == NULL || !((Player *)pSpell->m_caster)->IsInWorld())
		return true;
...
/*bool TestingTheAntidote(uint32 i, Spell* pSpell)
{
	...
	spawned->GetAIInterface()->SetNextTarget(((Unit *)pSpell->m_caster));
...
{
	Player* pCaster = ((Player *)pSpell->m_caster);
	if(pCaster == NULL)
		return true;
	...
	if(pSinloren != NULL)
	{
		if(qle->GetMobCount(0) < qle->GetQuest()->required_mobcount[0])
		{
			if(pPlayer->CalcDistance(pPlayer, pSinloren) < 15)
			{
				...
				obj = sEAS.SpawnGameobject(pPlayer, GO_FIRE, 13214.3f, -7059.19f, 17.5717f, 1.58573f, 1);
				sEAS.GameobjectDelete(obj, 2 * 60 * 1000);

				obj = sEAS.SpawnGameobject(pPlayer, GO_FIRE, 13204.2f, -7059.38f, 17.5717f, 1.57787f, 1);
				sEAS.GameobjectDelete(obj, 2 * 60 * 1000);
	...
	if(pBloodoath != NULL)
	{
		if(qle->GetMobCount(1) < qle->GetQuest()->required_mobcount[1])
		{
			if(pPlayer->CalcDistance(pPlayer, pBloodoath) < 15)
			{
				...
				obj = sEAS.SpawnGameobject(pPlayer, GO_FIRE, 13329.4f, -6994.70f, 14.5219f, 1.38938f, 1);
				sEAS.GameobjectDelete(obj, 2 * 60 * 1000);

				obj = sEAS.SpawnGameobject(pPlayer, GO_FIRE, 13315.4f, -6990.72f, 14.7647f, 1.25979f, 1);
				sEAS.GameobjectDelete(obj, 2 * 60 * 1000);
	...
	if(pDawnchaser != NULL)
	{
		if(pPlayer->CalcDistance(pPlayer, pDawnchaser) < 15)
		{
			if(qle->GetMobCount(2) < qle->GetQuest()->required_mobcount[2])
			{
				...
				obj = sEAS.SpawnGameobject(pPlayer, GO_FIRE, 13284.1f, -7152.65f, 15.9774f, 1.44828f, 1);
				sEAS.GameobjectDelete(obj, 2 * 60 * 1000);

				obj = sEAS.SpawnGameobject(pPlayer, GO_FIRE, 13273.0f, -7151.21f, 15.9774f, 1.39723f, 1);
				sEAS.GameobjectDelete(obj, 2 * 60 * 1000);
...
bool ManaRemnants(uint32 i, Spell* pSpell)
{
	...
	for(uint32 i = 0; i < 2; i++)
	{
		...
		if(qle != NULL && qle->GetMobCount(0) < qle->GetQuest()->required_mobcount[0])
		{
			...
			pPlayer->SetSpellTargetType(44981,((Unit *)Ward->GetGUID()));
...
{
	if(!((Player *)pSpell->m_caster)) return true;
	Player* mTarget = ((Player *)pSpell->m_caster);
...
{
	if(((Player *)pSpell->m_caster) == NULL)
		return true;
...
{
	if(!((Player *)pSpell->m_caster) || pSpell->GetUnitTarget() == NULL || !pSpell->GetUnitTarget()->IsCreature())
		return true;

	QuestLogEntry* qLogEntry = ((Player *)pSpell->m_caster)->GetQuestLogForEntry(5163);
	if(qLogEntry == NULL)
		return true;
	...
	for(uint32 j = 0; j < sizeof(friends) / sizeof(uint32); j++)
	{
		if(target->GetEntry() == friends[j] && qLogEntry->GetMobCount(j) < qLogEntry->GetQuest()->required_mobcount[j])
		{
			...
			qLogEntry->SendUpdateAddKill(j,0);
...
void SetupQuestItems(ScriptMgr* mgr)
{
	...
	mgr->register_dummy_spell(38729, &RodofPurification);
	...
	mgr->register_dummy_spell(45668, &CraftyBlaster);  			//http://www.wowhead.com/?item=34812
	...
	mgr->register_dummy_aura(19548, &HunterTamingQuest);
	mgr->register_dummy_aura(19674, &HunterTamingQuest);
	mgr->register_dummy_aura(19687, &HunterTamingQuest);
	mgr->register_dummy_aura(19688, &HunterTamingQuest);
	mgr->register_dummy_aura(19689, &HunterTamingQuest);
	mgr->register_dummy_aura(19692, &HunterTamingQuest);
	mgr->register_dummy_aura(19694, &HunterTamingQuest);
	mgr->register_dummy_aura(19696, &HunterTamingQuest);
	mgr->register_dummy_aura(19697, &HunterTamingQuest);
	mgr->register_dummy_aura(19699, &HunterTamingQuest);
	mgr->register_dummy_aura(19700, &HunterTamingQuest);
	mgr->register_dummy_aura(30099, &HunterTamingQuest);
	mgr->register_dummy_aura(30102, &HunterTamingQuest);
	mgr->register_dummy_aura(30105, &HunterTamingQuest);
	mgr->register_dummy_aura(30646, &HunterTamingQuest);
	mgr->register_dummy_aura(30653, &HunterTamingQuest);
	mgr->register_dummy_aura(30654, &HunterTamingQuest);
