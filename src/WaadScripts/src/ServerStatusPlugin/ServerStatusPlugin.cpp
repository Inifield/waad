...
static int compteur = 0; // Pour la base SQL
#define LIMIT_SQL_STAT   720 // 2min par stat, 30/heures, 720 par jour
...
void GenerateUptimeString(char * Dest)
{
    ...
    sprintf(Dest, "%d jours, %d heures, %d minutes, %d secondes", (int)days, (int)hours, (int)mins, (int)seconds);
}
...
void FillOnlineTime(uint32 Time, char * Dest)
{
    ...
    sprintf(Dest, "%d heures, %d minutes, %d secondes", (int)hours, (int)mins, (int)seconds);
}
...
void StatDumper::DumpStats()
{
    ...
    std::deque<Player*> gms;
    {
...
#ifdef WIN32
		fprintf(f, "    <platform>AscentWaad %s r%u/%s-Win-%s</platform>\n", BUILD_TAG, BUILD_REVISION, CONFIG, ARCH);		
#else
		fprintf(f, "    <platform>AscentWaad %s r%u/%s-%s</platform>\n", BUILD_TAG, BUILD_REVISION, PLATFORM_TEXT, ARCH);
#endif
		...
		// patch stats mysql ... Branruz && Franck77 mod 16 dec 08
		if(compteur >=  LIMIT_SQL_STAT) compteur = 0;
		else                            compteur++;
		CharacterDatabase.WaitExecute("REPLACE INTO serveurstats (id,tagStat,uptime,player,gm,alliance,horde,accept,peak) VALUES ('%u','%u','%s','%u','%u','%u','%u','%u','%u');",
		compteur,t,uptime,(unsigned int)(sWorld.AlliancePlayers + sWorld.HordePlayers),(unsigned int)GMCount,
		(unsigned int)sWorld.AlliancePlayers,(unsigned int)sWorld.HordePlayers,(unsigned int)sWorld.mAcceptedConnections,
		(unsigned int)sWorld.PeakSessionCount);
		
	}
    fprintf(f, "  </status>\n");

	// Attention  pas d'espace dans les parse XML (attention aussi a adapter les scripts de lecture du XML)
    ...
    while(!gms.empty())
    {
	 ...
	 if(!plr->GetSession()->GetPermissions())
             fprintf(f, "      <permissions> Aucune </permissions>\n"); 
	 else 
	 {
	  if(plr->GetUInt32Value(PLAYER_FLAGS) & PLAYER_FLAG_GM)
              fprintf(f, "      <permissions> Game Master </permissions>\n"); 
	  else
    ...
    for (itr = objmgr._players.begin(); itr != objmgr._players.end(); itr++)
    {
     ...
     if(itr->second->GetSession() && itr->second->IsInWorld())
     {
      ...
      fprintf(f, "      <latency>%u</latency>\n", (unsigned int)plr->GetSession()->GetLatency());
      fprintf(f, "    </plr>\n");
      if(!plr->GetSession()->HasPermissions()) gms.push_back(plr);
      }
     }
     objmgr._playerslock.ReleaseReadLock();
     fprintf(f, "  </sessions>\n");
