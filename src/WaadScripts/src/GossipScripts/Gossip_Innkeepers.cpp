...
void InnkeeperGossip::GossipHello(Object * pObject, Player* Plr, bool AutoSend)
{
    ...
    if( pObject->GetUInt32Value(UNIT_NPC_FLAGS) & UNIT_NPC_FLAG_VENDOR )
        Menu->AddItem( 1, "J'aimerais voir vos marchandises.", 1 );
    
    Menu->AddItem( 5, "Faire de cette auberge mon nouveau foyer.", 2 );
    Menu->AddItem( 0, "Que puis-je faire a l'auberge?", 3 );
    if(AutoSend)
        Menu->SendTo(Plr);
}
