...
public:
    ...
    VoidReaverAI( Creature *pCreature ) : MoonScriptBossAI( pCreature )
    {
		...
		if ( pPounding != NULL )
		{
			pPounding->AddEmote( "Lancement des mesures alternatives...", Text_Yell, 11218 );
			pPounding->AddEmote( "Calculs des paramètres de puissance...", Text_Yell, 11219 );
		}
		...
		AddEmote( Event_OnCombatStart, "Alerte, vous êtes designés pour être exterminés!", Text_Yell, 11213 );
		AddEmote( Event_OnTargetDied, "Extermination, accomplie.", Text_Yell, 11215 );
		AddEmote( Event_OnTargetDied, "Forme de vie intruse placée en disfonctionement.", Text_Yell, 11216 );
		AddEmote( Event_OnTargetDied, "Menace neutralisé.", Text_Yell, 11217 );
		AddEmote( Event_OnDied, "Exctinction du système...", Text_Yell, 11214 );
...
class HighAstromancerSolarianAI : public MoonScriptBossAI
{
	...
	HighAstromancerSolarianAI(Creature* pCreature) : MoonScriptBossAI(pCreature)
	{
		...
		mDisappear->AddEmote("Vous êtes deseperement surclassés!", Text_Yell, 11139);
		mDisappear->AddEmote("Je vais balayer vos illusions de grandeur!", Text_Yell, 11140);
		...
		AddEmote(Event_OnDied, "La chaleur du solei...m'attend.", Text_Yell, 11135);
		AddEmote(Event_OnTargetDied, "Votre ame appartiens aux abysses!", Text_Yell, 11136);
		AddEmote(Event_OnTargetDied, "Par le sang des bien né!", Text_Yell, 11137);
		AddEmote(Event_OnTargetDied, "Pour le puit de soleil!", Text_Yell, 11138);
	}
...
public:
	...
	DarkenerAI(Creature* pCreature) : MoonScriptCreatureAI(pCreature)
	{
		...
		AddEmote(Event_OnCombatStart, "Préparez vous!", Text_Yell, 11203);
		AddEmote(Event_OnDied, "Pardonnez moi mon prince! J'ai...échouer.", Text_Yell, 11204);
...
public:
	...
	SanguinarAI(Creature* pCreature) : MoonScriptCreatureAI(pCreature)
	{
		...
		AddEmote(Event_OnCombatStart, "Sang pour sang!", Text_Yell, 11152);
		AddEmote(Event_OnDied, "Non! Je...ne...peut.", Text_Yell, 11153);
		SetCanEnterCombat(false);
	}
...
public:
	...
	CapernianAI(Creature* pCreature) : MoonScriptCreatureAI(pCreature)
	{
		...
		AddEmote(Event_OnCombatStart, "Les sin'dorei regnent en maitres!", Text_Yell, 11117);
		AddEmote(Event_OnDied, "Ca n'est pas...terminé!", Text_Yell, 11118);
		SetCanEnterCombat(false);
	}
...
public:
	...
	TelonicusAI(Creature* pCreature) : MoonScriptCreatureAI(pCreature)
	{
		...
		AddEmote(Event_OnDied, "D'autre perils...vous attendent.", Text_Yell, 11158);			// not sure
		SetCanEnterCombat(false);
	}
...
public:
	...
	KaelThasAI(Creature* pCreature) : MoonScriptBossAI(pCreature)
	{
		...
		mSummonWeapons->AddEmote("Comme vous le voyez j'ai plus d'une corde à mon arc.", Text_Yell, 11261);
		...
		mMindControl->AddEmote("Obéis moi.", Text_Yell, 11268);
		mMindControl->AddEmote("Soumet toi à ma volontée.", Text_Yell, 11269);
		...
		mPhoenix->AddEmote("Par le pouvoir du soleil!", Text_Yell, 11266);
		...
		AddEmote(Event_OnCombatStart, "L'energie, la puissance...mon peuple en est dependant. Cette adiction se fit jour après la destruction du puit de soleil...bienvenue dans l'avenir. Dommage que vous arriviez trop tard pour tout arrêter. Plus personne ne peux desormais se dresser contre moi! Selama ashal'anore.", Text_Yell, 11256);
		AddEmote(Event_OnTargetDied, "Vous ne triompherais pas.", Text_Yell, 11270);
		AddEmote(Event_OnTargetDied, "Vous avez joué...et perdu!", Text_Yell, 11271);
		AddEmote(Event_OnTargetDied, "C'etait un jeu d'enfant.", Text_Yell, 11272);
		AddEmote(Event_OnDied, "Pour...Quel...thalas!", Text_Yell, 11274);
		...
		for (int i = 0; i < 2; ++i)
		{
			...
			if ((pGameobject != NULL) && (pGameobject->GetByte(GAMEOBJECT_BYTES_1,0) == 1) ) //GetUInt32Value(GAMEOBJECT_STATE) == 1)
			{
				pGameobject->SetByte( GAMEOBJECT_BYTES_1,0,0); //SetUInt32Value(GAMEOBJECT_STATE, 0);
			}
		}
	}
    ...
    void OnCombatStart(Unit * mTarget)
    {
		...
		{
			GameObject *pGameobject = _unit->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(Gates[i].mX, Gates[i].mY, Gates[i].mZ, Gates[i].mAddition);
			if ((pGameobject != NULL) && (pGameobject->GetByte( GAMEOBJECT_BYTES_1,0) == 0) )//,GetUInt32Value(GAMEOBJECT_STATE) == 0)
			{
				pGameobject->SetByte( GAMEOBJECT_BYTES_1,0,1); //SetUInt32Value(GAMEOBJECT_STATE, 1);
			}
		}
	...
	void SendAdvisorEmote()
	{
		...
		{
		case 1:
			Emote("Impressionant, voyons comment tiendrons vos nerf face à l'assombrisseur, Thaladred.", Text_Yell, 11259);
			SetAIUpdateFreq(5000);
			break;
		case 2:
			Emote("Vous avez tenu tête à certain des mes plus talentueux conseiller. Mais personne ne peux resister à la puissance du marteau de sang. Behold, Lord Sanguinar.", Text_Yell, 11260);
			SetAIUpdateFreq(12000);
			break;
		case 3:
			Emote("Capernian fera en sorte que votre sejour ici ne se prolonge pas.", Text_Yell, 11257);
			SetAIUpdateFreq(5000);
			break;
		case 4:
			Emote("Bien! Vous êtes digne de mesurer votre talent à celui de mon maitre ingenieur, Telonicus.", Text_Yell, 11258);
	...
	void AIUpdate()
	{
		...
		if (GetPhase() == 5)
		{
			...
			else if (IsTimerFinished(mEventTimer))
			{
				...
				Emote("Peut-etre vous ai-je sous estimé. Il ne serais pas très loyal de vous faire combatre mes 4 conseillers en même temps mais...mon peuple n'a jamais été traité avec loyauté. Je ne fait que rendre la politesse.", Text_Yell, 11262);
