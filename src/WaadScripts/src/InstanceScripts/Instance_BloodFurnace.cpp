...
public:
    ...
    KELIDANTHEBREAKERAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		spells[2].perctrigger = 0.0f;
		spells[2].attackstoptimer = 1000;
		spells[2].speech = "Approchez...approchez...et brulez!";
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous ne devez pas le libérer!");
			_unit->PlaySoundToSet(10166);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Morveux ignorants!");
			_unit->PlaySoundToSet(10167);
			break;
		case 2:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Fous, il nous tuera tous!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "C’est tout ce que vous méritez.");
				_unit->PlaySoundToSet(10169);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vos amis vont bientôt vous rejoindre.");
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Bonne chance...vous en aurez besoin.");
		...
		// Frk 2 Jav 2009
		if(pDoor) pDoor->SetByte( GAMEOBJECT_BYTES_1,0, 0); // GAMEOBJECT_STATE
		...
		// Frk 2 Jav 2009
		if(pDoor2) pDoor2->SetByte( GAMEOBJECT_BYTES_1,0, 0); // GAMEOBJECT_STATE
    }
    ...
    void AIUpdate()
	{
		...
		{
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Approchez...approchez...et brulez!");
...
public:
	...
	{
		AddEmote(Event_OnCombatStart, "Ma tâche ne doit pas être interrompue.", Text_Yell, 10286);
		AddEmote(Event_OnCombatStart, "Je pourrais peut-être me servir de vous.", Text_Yell, 10287);
		AddEmote(Event_OnCombatStart, "La colère, la haine, ce sont des outils dont je peux me servir.", Text_Yell, 10288);
		AddEmote(Event_OnTargetDied, "Voyons ce que je peux faire de vous.", Text_Yell, 10289);
		AddEmote(Event_OnTargetDied, "Résister ne sert à rien.", Text_Yell, 10290);
		AddEmote(Event_OnDied, "Restez loin...de moiiii....", Text_Yell, 10291);
