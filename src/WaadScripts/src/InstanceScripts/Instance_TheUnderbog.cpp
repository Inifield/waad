			...
			for(int i=0;i<nrspells;i++)
			{
				...
				if(m_spellcheck[i])
				{
					...
					if (i == 1)
					{
						...
						{
							_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Sale bête, obéi! Tue-le, maintenant!");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Nous combattrons jusqu’à la mort!");
			_unit->PlaySoundToSet(10384);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je vais finir ça rapidement!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			case 0:
				...
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "C'est terminé!");
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Très... bien..."); // not sure
