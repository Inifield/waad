...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		RegisterAIUpdateEvent(_unit->GetUInt32Value(UNIT_FIELD_BASEATTACKTIME));
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "De l’énergie...à consommer.");
    ...
    void OnDied(Unit * mKiller)
    {
		...
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Le vide...m’appelle.");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Cette enveloppe est vide.");
				_unit->PlaySoundToSet(11251);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Plus...de...vie.");
	...
	void Speech()
	{
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "L’ombre...vous engloutira.");
			_unit->PlaySoundToSet(11253);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Les ténèbres consument...l’univers.");
	...
	void VoidZoneArc()
	{
		...
		float vzO = RTarget->GetOrientation();
		Creature *VoidZone = _unit->GetMapMgr()->CreateCreature(cp->Id);
		VoidZone->Load(cp, vzX, vzY, vzZ, vzO);
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		CastTime();
		RegisterAIUpdateEvent(_unit->GetUInt32Value(UNIT_FIELD_BASEATTACKTIME));
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Il n’est pas très avisé de me mettre en colère.");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Complètement inapte! Ca me rappelle fortement quelqu’un!");
				_unit->PlaySoundToSet(11087);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous n’avez pas choisi le bon adversaire!");
    ...
    void OnDied(Unit * mKiller)
    {
		CastTime();
		RemoveAIUpdateEvent();
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Maintenant je suis vraiment...énervé.");
	...
	void HealSound()
	{
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Ah, voila, ca va beaucoup mieux.");
			_unit->PlaySoundToSet(11091);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Ah, j’en avais bien besoin.");
	...
	void WhirlwindSound()
	{
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Récoltez la tempête!");
			_unit->PlaySoundToSet(11089);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je vais vous découper en petit morceaux!");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		CastTime();
		RegisterAIUpdateEvent(_unit->GetUInt32Value(UNIT_FIELD_BASEATTACKTIME));
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Enfin, de quoi évacuer ma frustration!"); 
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Ca fait du bien par où ca passe.");
				_unit->PlaySoundToSet(11239);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Ah, je me sens beaucoup mieux!");
    ...
    void OnDied(Unit * mKiller)
    {
		CastTime();
		RemoveAIUpdateEvent();
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "C’était...la seule...issue...pour moi.");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		CastTime();
		RegisterAIUpdateEvent(_unit->GetUInt32Value(UNIT_FIELD_BASEATTACKTIME));
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Contemplez celui qui sera votre perte!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Votre destin est écrit!");	
				_unit->PlaySoundToSet(11124);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Le chaos que j’ai semé ici, c’est qu’un avant-gout...");
    ...
    void OnDied(Unit * mKiller)
    {
		...
		CastTime();
       RemoveAIUpdateEvent();
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je ne suis qu’un parmi une multitude infinie.");
	...
	void FearSound()
	{
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Déguerpissez.");
			_unit->PlaySoundToSet(11129);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je vous montrerai des horreurs qui dépassent vos pires cauchemars.");
	...
	void DominationSound()
	{
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Tu feras comme je l’ordonne faible créature.");
			_unit->PlaySoundToSet(11127);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Ta volonté ne t’appartient plus.");
