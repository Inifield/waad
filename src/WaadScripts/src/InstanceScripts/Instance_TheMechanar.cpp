...
public:
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Votre stratégie est imparfaite!");	// flat?
				_unit->PlaySoundToSet(11102);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "C’était la seule issue logique!");	// magical outcome? // Logical Outcome
    ...
    void OnDied(Unit * mKiller)
    {
		CastTime();
       RemoveAIUpdateEvent();
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Voila...ahhhgggg...aaaghhh...imponderable.");
	...
	void SawBladesSound()
	{
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Mesurer deux fois, couper une fois.");
			_unit->PlaySoundToSet(11104);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Et si ma division est bien posée, je crois que vous êtes mort!");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		CastTime();
		RegisterAIUpdateEvent(_unit->GetUInt32Value(UNIT_FIELD_BASEATTACKTIME));
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Il vous reste approximativement 5 secondes à vivre!");	// corrected
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Une conclusion bien prévisible!"); // fixed 
				_unit->PlaySoundToSet(11110);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "L’opération continuera selon le plan!"); // fixed
    ...
    void OnDied(Unit * mKiller)
    {
		CastTime();
       RemoveAIUpdateEvent();
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Mes calculs ne prévoyaient pas...");	// Verified
	...
	void HammerSound()
	{
		...
		case 0:
			{
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Avec l’angle et la vitesse d’approche parfaite...");	// Corrected
				_unit->PlaySoundToSet(11112);
			}break;
		case 1:
			{
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Un peu dépassé, mais très efficace!");	// this one too =/
...
public:
    ...
    MechanoLordCapacitusAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		spells[1].perctrigger = 7.0f;
		spells[1].attackstoptimer = 1000;
		spells[1].speech = "Vous croyez que vous pouvez me faire mal? Vous croyez que j’ai peur de vous?";
		...
		spells[2].perctrigger = 7.0f;
		spells[2].attackstoptimer = 1000;
		spells[2].speech = "Allez-y! Donner moi ce que vous avez de mieux! Je peux encaisser!";
    ...
    void OnCombatStart(Unit* mTarget)
    {
		CastTime();
		RegisterAIUpdateEvent(_unit->GetUInt32Value(UNIT_FIELD_BASEATTACKTIME));
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Dégagez tant que vous le pouvez.");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Ne dites pas que je ne vous avez pas prévenus!");	// checks here
				_unit->PlaySoundToSet(11163);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Oh qu’est ce que je suis bon! Je me la pette mais c’est la fête!");
    ...
    void OnDied(Unit * mKiller)
    {
		CastTime();
       RemoveAIUpdateEvent();
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Brutes!");
...
*/
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous n’aimez pas la vie c’est ca?");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Et n’y revenez pas!");	// Verified
    ...
    void AIUpdate()
	{
		...
		{
			_unit->CastSpell(_unit, spells[0].info, spells[0].instant);
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je ne suis pas seule!");	// checks!
	...
	void DragonsBreathSound()
	{
		...
		case 0:
			{
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous supporterez toute cette chaleur?");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		CastTime();
		RegisterAIUpdateEvent(_unit->GetUInt32Value(UNIT_FIELD_BASEATTACKTIME));
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Notre calendrier est serré, je ne vous laisserais pas perturber notre travail!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Une simple petite nuisance.");
				_unit->PlaySoundToSet(11194);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "On dirait bien que vous avez perdu.");
    ...
    void OnDied(Unit * mKiller)
    {
		...
		CastTime();
       RemoveAIUpdateEvent();
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Le projet...continuera...");
    ...
    void AIUpdate()
	{
		...
		if (!SummonTimer)
		{
			...
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Il est temps de compléter mon équipe de travail.");
	...
	void DominationSound()
	{
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je recherche un bon coéquipier...");
			_unit->PlaySoundToSet(11197);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous êtes à mon service maintenant!");
