...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Qu’avons-nous là? ...");
			_unit->PlaySoundToSet(10331);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Heh... ca risque de faire un peu male.");
			_unit->PlaySoundToSet(10332);
			break;
		case 2:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je vais aimer ca!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Dites adieu!");
				_unit->PlaySoundToSet(10334);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Beaucoup trop facile!");
    ...
    void AIUpdate()
    {
		...
		{
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Soignez-moi! Vite!");
...
public:
	...
	OmorTheUnscarredAI(Creature *pCreature) : MoonScriptCreatureAI(pCreature)
	{
		...
		pSummon->AddEmote("Achor-she-ki! Repait toi mon familier! Gave-toi!", Text_Yell, 10277);
		...
		AddEmote(Event_OnCombatStart, "Je ne serais pas vaincu!", Text_Yell, 10279);
		AddEmote(Event_OnCombatStart, "Vous osez m’affronter moi?", Text_Yell, 10280);
		AddEmote(Event_OnCombatStart, "Votre insolence sera votre perte!", Text_Yell, 10281);
		AddEmote(Event_OnTargetDied, "Meurs mauviette!", Text_Yell, 10282);
		AddEmote(Event_OnDied, "Ce n’est pas...terminé.", Text_Yell, 10284);
	}
	...
	void OnCombatStop(Unit *pTarget)
	{
		...
		if (IsAlive())
		{
			Emote("A moi la victoire!", Text_Yell, 10283);
		}
	}
