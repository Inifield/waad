...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		if (GrandWarlock)
		{
			...
			{
			case 0:
				GrandWarlock->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous pouvez prendre celui-là, je n’en est plus l’usage.");
				GrandWarlock->PlaySoundToSet(10263);
				break;
			case 1:
				GrandWarlock->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Oui, tapez-le sans pitié! Il a le crâne aussi dur qu’un ogre!");	// to add
				GrandWarlock->PlaySoundToSet(10264);
				break;
			case 2:
				GrandWarlock->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Ne perdez pas votre temps avec celui-ci, il est tout faible.");
				GrandWarlock->PlaySoundToSet(10265);
				break;
			case 3:
				GrandWarlock->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous le voulez? Très bien prenez-le.");
    ...
    void OnDied(Unit * mKiller)
    {
		...
		if (GrandWarlock)	// any emotes needed?
		{
			...
			{
			case 0:
				GrandWarlock->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Un pauvre diable au tapis, allé, prenez en un autre.");
				GrandWarlock->PlaySoundToSet(10267);
				break;
			case 1:
				GrandWarlock->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Ohhh, quel gâchis...Au suivant !");
				GrandWarlock->PlaySoundToSet(10268);
				break;
			case 2:
				GrandWarlock->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "De toute façon je l’aurai tué quant même.");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		if (Started)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je m’ennuie déjà!");
				_unit->PlaySoundToSet(10271);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Allons, un peu de nerf!");
				_unit->PlaySoundToSet(10272);
				break;
			case 2:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "C’est encore moins drôle que de torturer des peons");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Perdu!");
				_unit->PlaySoundToSet(10274);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Mais mourez donc!");
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Quel...embarras.");
...
public:
	...
	void OnCombatStart(Unit *pTarget)
	{
		...
		{
		case 0:
			mLeftHead->Emote( "Si toi sympa, moi te laisser vivre.", Text_Yell, 10308 );
			mSpeechTimer = AddTimer( 4000 );
			mSpeechId = 1;
			break;
		case 1:
			mLeftHead->Emote( "Moi faim!", Text_Yell, 10309 );
			mSpeechTimer = AddTimer( 2500 );
			mSpeechId = 2;
			break;
		case 2:
			mLeftHead->Emote( "Crabouille!", Text_Yell, 10306 );
	...
	void OnTargetDied(Unit *pTarget)
    {
		...
		{
		case 0:
			mLeftHead->Emote( "Suis fatigué. Le suivant est pour toi!", Text_Yell, 10320 );
			break;
		case 1:
			mLeftHead->Emote( "Celui-là mourir facile!", Text_Yell, 10310 );
	...
	void OnDied(Unit *pKiller)
    {
		...
		mLeftHead->Emote( "Ca tout... ta faute!", Text_Yell, 10311 );
		mLeftHead->Despawn( 1000 );
	...
	void AIUpdate()
	{
		...
		if ( mSpeechTimer != INVALIDATE_TIMER && IsTimerFinished( mSpeechTimer ) )
		{
			...
			if ( mLeftHead != NULL && mRightHead != NULL )
			{
				...
				{
				case 0:
					mRightHead->Emote( "C'est parce que c'est moi qui fait tout le travail!", Text_Yell, 10321 );
					break;
				case 1:
					mRightHead->Emote( "Non, nous ne laisserons personne vivre!", Text_Yell, 10318 );
					break;
				case 2:
					mRightHead->Emote( "T'as toujours faim. C'est ta faute si nous sommes aussi gros!", Text_Yell, 10319 );
					break;
				case 3:
					mRightHead->Emote( "Et si je m'occupais des pourparler!", Text_Yell, 10317 );
					break;
				case 4:
					mRightHead->Emote( "J'ai pas terminé, pauvre idiot!", Text_Yell, 10313 );
					break;
				case 5:
					mRightHead->Emote( "Ennuié, il était presque mort!", Text_Yell, 10316 );
					break;
				case 6:
					mRightHead->Emote( "C'est pas drôle", Text_Yell, 10314 );
					mSpeechTimer = AddTimer( 6000 );
					mSpeechId = 8;
					ResetSpeech = false;
					break;
				case 7:
					mRightHead->Emote( "Qu'est ce que tu fait!?", Text_Yell, 10315 );
					break;
				case 8:
	...
	void ShiftAggro()
	{
		...
		if ( pTarget != NULL )
		{
			...
			{
			case 0:
				mLeftHead->Emote( "Nous, tuer son ami!", Text_Yell, 10301 );
				break;
			case 1:
				mLeftHead->Emote( "Moi, pas aimer celui-là...", Text_Yell, 10300 );
				mSpeechTimer = AddTimer( 3000 );
				mSpeechId = 4;
				break;
			case 2:
				...
				break;
			case 3:
				mLeftHead->Emote( "Hé, tête d'abruti.", Text_Yell, 10305 );
				mSpeechTimer = AddTimer( 3000 );
				mSpeechId = 5;
				break;
			case 4:
				...
				break;
			case 5:
				mLeftHead->Emote( "Toi rester là. Moi tuer quelqu'un d'autre!", Text_Yell, 10303 );
				mSpeechTimer = AddTimer( 4000 );
				mSpeechId = 6;
				break;
			case 6:
				mLeftHead->Emote( "Nous tuer quelqu'un d'autre!", Text_Yell, 10302 );
				mSpeechTimer = AddTimer( 3000 );
				mSpeechId = 7;
				break;
			}
...
class HeadAI : public MoonScriptCreatureAI
{
	...
	void AIUpdate()
	{
		...
		Emote( "Je... je te...déteste!", Text_Yell, 10322 );
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Nous sommes la vrai horde! La seule horde!");
			_unit->PlaySoundToSet(10323);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je vais vous découper comme des pièces de boucherie!");
			_unit->PlaySoundToSet(10324);
			break;
		case 2:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "On m’appelle l’amepoing pour une bonne raison, vous allez le comprendre!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Pour la véritable horde!");
				_unit->PlaySoundToSet(10326);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je suis le seul chef de guerre!");
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "La vrai horde... triomphera!");
