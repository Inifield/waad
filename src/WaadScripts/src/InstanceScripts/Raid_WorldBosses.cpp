...
public:
    ...
    KazzakAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
        ...
        RegisterAIUpdateEvent(_unit->GetUInt32Value(UNIT_FIELD_BASEATTACKTIME));
        //Spawn intro.
        _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je me rappelle bien le gout amer de la défaite à la fin de la 3 eme guerre. J'ai attendu bien trop longtemps, l'occasion de me venger. Maintenant, l'ombre de la légion tombe sur cette terre. Votre création ratée sera bientôt défaite. Ce n'est qu'une question de temps.");
    ...
    void OnCombatStart(Unit* mTarget)
    {
        ...
        {
        case 0:
            _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "La légion envahira l'univers!");
            _unit->PlaySoundToSet(11333);
            break;
        case 1:
            _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Que les mortels périssent tous!");
    ...
    void OnTargetDied(Unit* mTarget)
    {
        if(_unit->GetHealthPct() > 0)
        {
            ...
            case 0:
                ...
                break;
            case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Pitoyable vermine.");
    ...
    {
        enrage = 0;
        _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "L’univers sera redéfini.");
    ...
    {
        enrage = 0;
        _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "La légion…elle tombera…jamais…");
    ...
    void RandomSpeech()
    {
        ...
        {
        case 0:
            _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Toute vie doit-être éradiquée.");
...
public:
    ...
    {
        enraged = false;
        _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Cessez toute activité, vous allez être eliminés!");
    ...
    void OnTargetDied(Unit* mTarget)
    {
        if(_unit->GetHealthPct() > 0)
        {
            ...
            {
            case 0:
                _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Niveau de menace...nul.");
                _unit->PlaySoundToSet(11349);
                break;
            case 1:
                _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Instruction executée.");
                _unit->PlaySoundToSet(11350);
                break;
            case 2:
                _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Cible exterminée.");
    ...
    void OnDied(Unit* mKiller)
    {
        _unit->RemoveAura(AURA_OF_DEATH);
        enraged = false;
        _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Panne système dans cinq... quatre...");
        ...
        if(_unit->GetCurrentSpell() == NULL && _unit->GetAIInterface()->GetNextTarget())
        {
            ...
            if (m_spellcheck[i])
            {
                ...
                if (m_spellcheck[0] == true) //Earthquake
                {
                    ...
                    {
                    case 0:
                        _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Debut de la perturbation tectonique.");
                        _unit->PlaySoundToSet(11345);
                    break;
                    case 1:
                        _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Magnetude reglé. Déclanchement. ");
                ...
                if (m_spellcheck[3] == true) //Overrun
                {
                    ...
                    {
                    case 0:
                        _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Trajectoire verouillée.");
                        _unit->PlaySoundToSet(11347);
                    break;
                    case 1:
                        _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vitesse maximale enclanchée.");
