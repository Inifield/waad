...
public:
    ...
    MagtheridonTriggerAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		if (Gate)
			Gate->SetByte( GAMEOBJECT_BYTES_1,0,0); //SetUInt32Value(GAMEOBJECT_STATE, 0);
	}
	...
	void AIUpdate()
    {
				...
				if ( Channeler && !Channeler->isAlive() && Channeler->IsInWorld() )
				{
					...
					for (size_t x = 0; x < ChannelersTable.size(); x++)
					{
						...
						if (BuffedChanneler && BuffedChanneler != Channeler && BuffedChanneler->isAlive())
						{
							...
							Aura * aura = new Aura(dbcSpell.LookupEntry(SOUL_TRANSFER), -1, INSTANT_DURATION, BuffedChanneler, BuffedChanneler);
							if (aura != NULL)
							{
		...
		if (!EventStarted)
		{
			...
			if (EventStarted)
			{
				...
				GameObject* Gate = _unit->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(-72.5866f, 1.559f, 0.0f, 183847);
				if (Gate)
					Gate->SetByte( GAMEOBJECT_BYTES_1,0,1); //SetUInt32Value(GAMEOBJECT_STATE, 1);
			}
		}
	...
	void AIUpdate()
	{
		...
		if (Counter >= ACTIVE_CUBES_TO_BANISH && Magtheridon && Magtheridon->isAlive())
		{
			...
			if(Magtheridon->GetCurrentSpell())
				Magtheridon->GetCurrentSpell()->cancel(40); // SPELL_FAILED_INTERRUPTED

			// We add channeling player aura that does not allow that go to be used again in 1.3 min
			Aura *aura = new Aura(dbcSpell.LookupEntry(MIND_EXHAUSTION), 78000, INSTANT_DURATION, Magtheridon, Channeler);
			if (aura != NULL)
			{
...
public:
    ...
    MagtheridonAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		Aura * aura = new Aura(dbcSpell.LookupEntry(BANISHMENT), -1, INSTANT_DURATION, _unit, _unit);
		if (aura != NULL)
		{
