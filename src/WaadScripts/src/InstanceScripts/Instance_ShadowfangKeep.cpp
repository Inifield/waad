...
public:
    ...
    void OnDied(Unit * mKiller)
    {
	   _unit->SendChatMessageAlternateEntry(4275, CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Qui ose interférer avec les fils Arugal? Votre fin vous attend étrangers!");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous aussi vous servirez!");
        _unit->PlaySoundToSet(5793);
    }
    
    void OnTargetDied(Unit* mTarget)
    {
        _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Un de plus qui tombe!");
    ...
    void SpellCast(uint32 val)
    {
        if(_unit->GetCurrentSpell() == NULL && _unit->GetAIInterface()->GetNextTarget())//_unit->getAttackTarget())
        {
            ...
            if(m_mindcontrol)
            {
                ...
                m_mindcontrol = false;
                _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Libérez votre rage!");
