...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Les profondeurs vous dévorerons!");
			_unit->PlaySoundToSet(10361);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Périssez habitants de la surface!");
			_unit->PlaySoundToSet(10362);
			break;
		case 2:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous vous noierez dans le sang!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			{
			case 0: 
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Plonger dans les profondeurs de l’oubli!");
				_unit->PlaySoundToSet(10364);
				break;
			case 1:
				//_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "For my Lady and Master."); //B.B. => message incomprehensible :/
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Notre matrone…se chargera…de vous!");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		if (_unit->GetCurrentSpell() != NULL)
			_unit->GetCurrentSpell()->cancel(40); // SPELL_FAILED_INTERRUPTED
    }
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous allez salement déguster!");
			_unit->PlaySoundToSet(10368);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Bouffe du métal chaud! Vermine!");
			_unit->PlaySoundToSet(10369);
			break;
		case 2:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "C’est moi que v’la!");
			_unit->PlaySoundToSet(10370);
			break;
		case 3:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "J’ai apporté les pains!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "T’as eu ce que tu méritais!");
				_unit->PlaySoundToSet(10372);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je t'ai eu!");
    ...
    void OnDied(Unit * mKiller)
    {
	   _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Maman");
    ...
    void AIUpdate()
    {
			...
			for (std::vector<Unit*>::iterator itr = Gnomes.begin(); itr < Gnomes.end(); itr++)
			{
				...
				if (_unit->GetDistance2dSq(Gnome) > 250.0f)
				{
					if (Gnome->GetCurrentSpell() != NULL)
						Gnome->GetCurrentSpell()->cancel(40); // SPELL_FAILED_INTERRUPTED
		...
		if ((_unit->GetHealthPct() <= 75 && GnomeCounter == 0) || (_unit->GetHealthPct() <= 50 && GnomeCounter == 1) || (_unit->GetHealthPct() <= 25 && GnomeCounter == 2))
        {
			...
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Faites leur une vidange les gars!");
...
public:
    ...
    WarlordKalitreshAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		spells[3].speech = "C’est loin d’être terminé!";
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		{
		case 0: 
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Votre tête va tomber!");
			_unit->PlaySoundToSet(10392);
			break;
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je méprise toute votre espèce!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	// Hack to prevent double yelling (OnDied and OnTargetDied when creature is dying)
		{
			...
			{
			case 0: 
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Tortue toi, vers de la surface!");
				_unit->PlaySoundToSet(10395);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Ah ha ha ha ha ha ha!");
				_unit->PlaySoundToSet(10396);
				break;
			}
		}
    }
	...
	void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Pour son excellence...pour...Vashj!");
			...
			else
			{
				if (EnrageTimer == 0 && RagePhase == 0)
				{
					...
					if (_unit->GetDistance2dSq(pDistiller) <= 100.0f)
					{
						...
						_unit->GetAIInterface()->m_canMove = false;
						_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "C'est loin d'être terminé");
						_unit->PlaySoundToSet(10391);
