...
class MutantWarHoundAI : public MoonScriptCreatureAI
{
	...
	void OnDied(Unit *pKiller)
	{
		...
		Aura *pAura = new Aura(dbcSpell.LookupEntry(MUTANT_WAR_HOUND_CLOUD_OF_DISEASE), 20000, INVOC_DURATION,_unit, _unit);
		if (pAura != NULL)
		{
...
class IllidariArchonAI : public MoonScriptCreatureAI
{
	...
	void AIUpdate()
	{
		...
		if (mIsShadowPriest && _unit->GetUInt32Value(UNIT_FIELD_HEALTH) <= 2500 && pDeath->mEnabled)
		{
			...
			{
				pCurrentSpell->cancel(40); // SPELL_FAILED_INTERRUPTED
			}
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous allez mourrir, au nom de Dame Vashj!");
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Le Seigneur Illidan va... vous écraser.");
	...
	void OnTargetDied(Unit* mTarget)
    {
		...
		{
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Il est temps pour vous de partir!");
			_unit->PlaySoundToSet(11456);
			break;
		default:
			{
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Votre réussite n'aura pas durée!");	// not sure
	...
	{	// Disabled till I find way to make it dispellable like on blizz
		...
		if (t > spells[3].casttime)
		{
			...
			{
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Restez là!");
				_unit->PlaySoundToSet(11451);
				break;
			default:
				{
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je m'occuperai de vous tout à l'heure!"); 
...
public:
    ...
    {
		RegisterAIUpdateEvent(_unit->GetUInt32Value(UNIT_FIELD_BASEATTACKTIME));
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Contemplez celui qui sera votre perte!");	// used when he kills Warden Mellichar
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je ne suis qu'un parmis... une multitude infinie.");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Votre destin est écrit!");	// this one needs verification
				_unit->PlaySoundToSet(11124);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Le chaos que j'ai semé ici n'est qu'un avant-goût....");
	...
	void PhaseOne()
	{
		...
		if(timer >= 45)
        {
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Nous nous étendons sur l'univers, aussi innombrable que les étoiles!");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "La horde va vous écraser!");
	...
	void OnCombatStop(Unit *mTarget)
    {
		...
		if (_unit->isAlive())
		{
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "J'arracherai la chair de vos os!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		...
		{
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Encore! J'en veux encore!");
			_unit->PlaySoundToSet(11434);
			break;
		default:
			{
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Au banquet!");
...
public:
    ...
    {
		RegisterAIUpdateEvent(_unit->GetUInt32Value(UNIT_FIELD_BASEATTACKTIME));
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Douleur et souffrance, voilà tout ce qui vous attend.");
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je ne veux pas y retourner!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Regardez ce que vous m'avez fait faire.");
...
public:
    ...
    {
		RegisterAIUpdateEvent(_unit->GetUInt32Value(UNIT_FIELD_BASEATTACKTIME));
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous pouvez avoir tout ce que vous désirez... en y mettant le prix.");
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je vous attendrai.");
	...
	void OnTargetDied(Unit* mTarget)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Le bonheur est à portée de main.");
...
public:
    ...
    {
		RegisterAIUpdateEvent(_unit->GetUInt32Value(UNIT_FIELD_BASEATTACKTIME));
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Prenez garde! Je suis vivant.");
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je ne vous laisserai pas m'oublier!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Assez! Terminé!");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Alors, vous êtes en vacances?");
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je n'avais pas terminé.");
	...
	void OnTargetDied(Unit* mTarget)
    {
		...
		{
		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Les histoires d'amour finissent mal... En général.");
			_unit->PlaySoundToSet(11509);
			break;
		default:
			{
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Un de perdu, dix de retrouvés.");
	...
	void AIUpdate()
	{
		...
		if (_unit->GetAIInterface()->GetIsTaunted() && SoundTimer > 10)
		{
			...
			{
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Pas très impressionnant.");
				_unit->PlaySoundToSet(11502);
				break;
			case 2:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous vous amusez bien?");
				_unit->PlaySoundToSet(11503);
				break;
			default:
				{
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous jouez, vous payez.");
		...
		{
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Arrêtez de jouer avec mes sentiments!");
		...
		if (t > AuraChange)
		{
			...
			Aura *aura = new Aura(dbcSpell.LookupEntry(SpellId), 15000, INVOC_DURATION, _unit, _unit);
			if (aura)
			{
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous voulez me tester?");
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous avez eu de la chance!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Bel effort.");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Allons donc... Quelle grossièreté. Bandal!");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "J'ai mieux à faire!");
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Seigneur Illidan... Je...");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		...
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Fuyez ou mourrez!");
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Le destin... attend!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Mon travail est terminé.");
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "A moi la vengeance!");
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	
		{
			...
			{
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Le pire est à venir...");
				_unit->PlaySoundToSet(11515);
				break;
			default:
				{
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous me serez utile!");
    ...
    void OnDied(Unit * mKiller)
    {
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "La roue... tourne... encore....");
...
public:
	...
	void OnTargetDied(Unit* mTarget)
    {
		if (_unit->GetHealthPct() > 0)	
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je ne tiendrai pas très longtemps...");
				_unit->PlaySoundToSet(11385);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Il est temps de libérer votre âme de la poigne d'Illidan!");
				//_unit->PlaySoundToSet(11510);
				break;
    ...
    void AIUpdate()
	{
		...
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Abandonnez tout espoir! La légion est revenue terminer ce qui avait été commencé il y a si longtemps. Cette fois, il n'y aura pas d'échapatoire!");
...
class AkamaAI : public MoonScriptBossAI
{
	...
	AkamaAI(Creature *pCreature) : MoonScriptBossAI(pCreature)
    {
		...
		mHealingPotion->AddEmote("Je ne tiendrai pas très longtemps!!", Text_Yell, 11385);
		mHealingPotion->AddEmote("Non! Pas encore!", Text_Yell, 11386);
		mDespawn = AddSpell(AKAMA_DESPAWN, Target_Self, 0, 0.5f, 0);

		AddEmote(Event_OnTargetDied, "Illidan va payer!", Text_Yell, 11381);
		AddEmote(Event_OnTargetDied, "Un pas en avant!", Text_Yell, 11382);
		AddEmote(Event_OnDied, "Continuez mes amis! Tuez le et mettez fin à sa malédiction sur mon peuple!", Text_Yell, 11391);
	...
	void ToIllidanEvent()
	{
		...
		{
			Emote("C'est étrange qu'Illidan ne se protège pas contre les intrus.", Text_Say, 0);
		...
		case 1:
			...
			break;
		case 2:
			Emote("La porte est tout ce qu'il reste entre nous et le Traître. Eloignez-vous, mes amis.", Text_Say, 0);
		...
		case 4:
			...
			break;
		case 5:
			Emote("Je ne peux le faire seul...", Text_Say, 0);
		...
		case 6:		// summoning two spirits to help Akama with breaking doors
			...
			break;
		case 7:
			mUdaloAI->Emote("Tu n'es pas seul, Akama.", Text_Say, 0);
			break;
		case 8:
			mOlumAI->Emote("Ton peuple sera toujours à tes côtés.", Text_Say, 0);
			break;
		case 9:
			if (pDoorTrigger != NULL)
			{
		...
		case 12:
			...
			break;
		case 13:
			Emote("Je vous remercie pour votre aide, mes frères. Notre peuple sera racheté!", Text_Say, 0);
	...
	void IllidanDialogEvent()
	{
		...
		{
			Emote("Pas cette fois mes amis.", Text_Say, 0);
		...
		case 2:
			...
			break;
		case 3:
			mIllidanAI->Emote("Akama... ta duplicité n'est pas très étonnante. J'aurais dû vous massacrer depuis longtemps, toi et ton frère déformé.", Text_Yell, 11463);
			break;
		case 4:
		...
		case 5:
			...
			break;
		case 6:
			Emote("Nous sommes venus mettre fin à ton règne, Illidan. Mon peuple et l'Outreterre toute entière seront libres!", Text_Yell, 11389);
		...
		case 8:
			...
			break;
		case 9:
			mIllidanAI->Emote("Bien parlé. Mais je ne suis pas convaincu.", Text_Yell, 11464);
			mIllidanAI->GetUnit()->Emote(EMOTE_ONESHOT_QUESTION);
			break;
		case 10:
			mIllidanAI->GetUnit()->Emote(EMOTE_ONESHOT_QUESTION);
			break;
		case 11:
			Emote("L'heure est venue! C'est le moment!", Text_Yell, 11380);
		...
		case 12:
			...
			break;
		case 13:
			mIllidanAI->Emote("Vous n'êtes pas prêts!", Text_Yell, 11466);
	...
	void FightWithIllidan()
	{
		...
		if (GetCurrentWaypoint() < 18 && mScenePart != 0)
		{
			...
			{
			case 1:
				mIllidanAI->Emote("Venez, mes serviteurs. Occupez-vous de ce traître comme il le mérite!", Text_Yell, 11465);
				break;
			case 2:
				...
				break;
			case 3:
				Emote("Je vais m'occuper de ces dégénérés! Frappez maintenant, mes amis! Frappez le Traître!", Text_Yell, 11390);
	...
	void EncounterEnd()
	{
		...
		{
		case 1:{
			Emote("La lumière reviendra bénir ces sols lugubres. J'en fais le serment.", Text_Yell, 11387);
...
class MaievAI : public MoonScriptBossAI
{
	...
	void FightWithIllidan()
	{
		...
		if (mIllidanAI->IsAlive())
		{
			...
			{
				Emote("Gouttez à dix-mille ans de haine!", Text_Yell, 11495);
			...
			else if (IsTimerFinished(mYellTimer))
			{
				...
				{
				case 0:
					Emote("Saigne comme j'ai saigné!", Text_Yell, 11494);
					break;
				case 1:
					Emote("Voilà pour Naisha!", Text_Yell, 11493);
					break;
	...
	void DeathScene()
	{
		...
		if ((mIllidanAI != NULL && mScenePart <= 6) || mScenePart > 6)
		{
			...
			{
			case 1:
				Emote("Ah, c'est fini. Tu es vaincu.", Text_Yell, 11496);

				mIllidanAI->GetUnit()->Emote(EMOTE_ONESHOT_CUSTOMSPELL06);
				if (mIllidanAI->GetUnit()->GetCurrentSpell() != NULL)
					mIllidanAI->GetUnit()->GetCurrentSpell()->cancel(40); // SPELL_FAILED_INTERRUPTED
				break;
			case 2:
				...
				break;
			case 3:
				mIllidanAI->Emote("Tu as gagné... Maiev. Mais la chasseresse... n'est rien sans la chasse. Tu... n'est plus rien... sans moi.", Text_Yell, 11478);
				_unit->SetUInt32Value(UNIT_NPC_EMOTESTATE, 0);
				SetWieldWeapon(false);
				break;
			case 4:{
			...
			case 5:
				Emote("Il a raison. Je ne ressens rien. Je ne suis plus rien.", Text_Yell, 11497);
				break;
			case 6:{
				Emote("Adieu, champions.", Text_Yell, 11498);
...
class IllidanStormrageAI : public MoonScriptBossAI
{
	...
	IllidanStormrageAI(Creature* pCreature) : MoonScriptBossAI(pCreature)
	{
		...
		mEnrage = AddPhaseSpell(5, AddSpell(ILLIDAN_ENRAGE, Target_Self, 0, 1.3f, 0, 0, 0, true, "Vous avez perdu trop de temps mortels, vous allez succomber!", Text_Yell, 11474));

		AddEmote(Event_OnTargetDied, "Qui sera le prochain à tâter de mes lames?!", Text_Yell, 11473);
		AddEmote(Event_OnTargetDied, "C'est vraiment trop facile!", Text_Yell, 11472);
	...
	void PhaseOne()
	{
		...
		else if (GetHealthPercent() <= 65 && !IsCasting())
		{
			...
			Emote("Je ne me laisserai pas atteindre par la racaille!", Text_Yell, 11479);
	...
	void PhaseTwo()
	{
		if (mScenePart != 0)
		{
			...
			{
			case 0:{
				Emote("Contemplez les Flammes d'Azzinoth!", Text_Yell, 11480);
		...
		if (GetMoveType() != Move_WantedWP && !IsCasting())
		{
			...
			if (mMiscEventPart != 0)
			{
				if (mMiscEventPart == 1)
				{
					...
					{
						Emote("Soutenez le regard du Traître!", Text_Yell, 11481);
		...
		if (pTransformation == Ascend)
		{
			if (mMiscEventPart == 5)
			{
				...
				if (mPlaySound)
				{
					...
					{
					case 0:
						Emote("Vous ne savez rien de la puissance!", Text_Yell, 11469);
						mPlaySound = false;
						break;
					case 1:
						Emote("Quelle arrogance!", Text_Yell, 11471);
						mPlaySound = false;
						break;
					}
				}
	...
	void MaievScene()	// thinking about converting Maiev's functions to MoonScriptCreatureAI functions
	{
		...
		{
		case 1:
			Emote("C'est tout, mortels? Est-ce là toute la fureur que vous pouvez évoquer?", Text_Yell, 11476);
			break;
		case 2:
		...
		case 4:
			...
			break;
		case 5:
			pMaievAI->Emote("Leur fureur n'est rien à côté de la mienne, Illidan. Nous avons de vieilles querelles à régler.", Text_Yell, 11491);
			break;
		case 6:
		...
		case 7:
			...
			break;
		case 8:{
			Emote("Maiev... Comment est-ce possible?", Text_Yell, 11477);
		...
		case 10:
			pMaievAI->Emote("Ah, ma longue chasse est enfin terminée. Aujourd'hui, Justice sera faite!", Text_Yell, 11492);
			pMaievAI->GetUnit()->Emote(EMOTE_ONESHOT_EXCLAMATION);
			pMaievAI->SetDisplayWeapon(false, false);
			break;
		case 11:
	...
	void PhaseFive()
	{
		...
		else if (!IsCasting())
		{
			...
			if (mYellTimer <= 0 && _unit->GetAIInterface()->GetNextTarget() != NULL)
			{
				...
				{
					Emote("Gouttez à dix-mille ans de haine!", Text_Yell, 11470);
					mYellTimer = 25000;
				}
			}
...
/*		GameObject *pRightGate = _unit->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(745.07f, 241.802f, 354.292f, 200000);
		...
		if (pRightGate != NULL)
		{
			pRightGate->SetByte( GAMEOBJECT_BYTES_1,0,0); //SetUInt32Value(GAMEOBJECT_STATE, 0);
		}
		if (pLeftGate != NULL)
		{
			pLeftGate->SetByte( GAMEOBJECT_BYTES_1,0,0); //SetUInt32Value(GAMEOBJECT_STATE, 0);
		}*/
