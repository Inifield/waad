...
public:
    ...
    {
        _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Van Cleef paie bien pour vos têtes!");
...
public:
    ...
    void OnDied(Unit * mKiller)
    {
        ...
        // Open the door Frk 2 jav 2009
 		pDoor->SetByte( GAMEOBJECT_BYTES_1,0, 0); // GAMEOBJECT_STATE
	}
...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
        _unit->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "Personne ne défie impunément la confrérie.");
    ...
    void OnTargetDied(Unit* mTarget)
    {
        ...
        if(mTarget->GetTypeId() == TYPEID_PLAYER)
            sprintf(msg, "Et ne vous relevez pas, %s.", static_cast<Player*>(mTarget)->GetName());
        else if (mTarget->GetTypeFromGUID() == HIGHGUID_TYPE_PET)
            sprintf(msg, "Et ne vous relevez pas, %s.", static_cast<Pet*>(mTarget)->GetName().c_str());
    ...
    void ChangeToPhase1()
    {
        ...
        _unit->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "Chien, tous autant que vous êtes.");
    ...
    void ChangeToPhase2()
    {
        ...
        _unit->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "Imbéciles, notre cause est juste.");
    ...
    void ChangeToPhase3()
    {
        ...
        // Play sound, and send text.
        _unit->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "La confrérie prévaudra.");
...
public:
    ...
    void OnDamageTaken(Unit * mAttacker, float fAmount)
    {
        if(_unit->GetHealthPct() <= 66 && phase == 0)
        {
            ...
            _unit->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "Vous êtes plus dur que je ne pensais. Je vais devoir improviser!");
        ...
        else if(_unit->GetHealthPct() <= 33 && phase == 1)
        {
            ...
            _unit->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "D'ah! Je commence sérieusement à m’échauffer");
    ...
    {	
        ...
        case 0:
            ...
            break;
        case 1:
			_unit->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID_1, 7419);
...
//***********************************************************************************************
//script des leviers de portes par B.B.
//***********************************************************************************************
class Levier_RhahkZor : public GameObjectAIScript
{
	public:
	Levier_RhahkZor(GameObject* goinstance) : GameObjectAIScript(goinstance) {}
	static GameObjectAIScript *Create(GameObject * GO) { return new Levier_RhahkZor(GO); }

	void OnActivate(Player * pPlayer)
	{
		//recherche du GOB de la porte
		GameObject * pDoor = pPlayer->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(
            -289.691650f, -535.988953f, 49.440678f, 13965);
        if(pDoor == 0)
            return;

        //ouverture de la porte
		pDoor->SetUInt32Value(GAMEOBJECT_FLAGS, 64);
        pDoor->SetByte( GAMEOBJECT_BYTES_1,0, 0); // SetUInt32Value(GAMEOBJECT_STATE, 0);
	}
};
//***********************************************************************************************
class Levier_sneed : public GameObjectAIScript
{
	public:
	Levier_sneed(GameObject* goinstance) : GameObjectAIScript(goinstance) {}
	static GameObjectAIScript *Create(GameObject * GO) { return new Levier_sneed(GO); }

	void OnActivate(Player * pPlayer)
	{
		//recherche du GOB de la porte
		GameObject * pDoor = pPlayer->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(
			-190.860092f, -456.332184f, 54.496822f, 16400);
        if(pDoor == 0)
            return;
...
//***********************************************************************************************
class Levier_Gilnid : public GameObjectAIScript
{
	public:
	Levier_Gilnid(GameObject* goinstance) : GameObjectAIScript(goinstance) {}
	static GameObjectAIScript *Create(GameObject * GO) { return new Levier_Gilnid(GO); }

	void OnActivate(Player * pPlayer)
	{
		//recherche du GOB de la porte
		GameObject * pDoor = pPlayer->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(
			-176.569f, -577.640991f, 19.311600f, 16399);
        if(pDoor == 0)
            return;
...
//***********************************************************************************************
class Levier_Canon : public GameObjectAIScript
{
	public:
	Levier_Canon(GameObject* goinstance) : GameObjectAIScript(goinstance) {}
	static GameObjectAIScript *Create(GameObject * GO) { return new Levier_Canon(GO); }

	void OnActivate(Player * pPlayer)
	{
		//recherche du GOB de la porte
		GameObject * pDoor = pPlayer->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(
			-101.004f, -666.607788f, 7.422822f, 16397);
        if(pDoor == 0)
            return;
...
//***********************************************************************************************
//***********************************************************************************************
//script de pop de la poudre à canon
class Poudre_canon : public GameObjectAIScript
{
	public:
	Poudre_canon(GameObject* goinstance) : GameObjectAIScript(goinstance) {}
	static GameObjectAIScript *Create(GameObject * GO) { return new Poudre_canon(GO); }
	...
	void OnActivate(Player * pPlayer)
	{
		...
		pPlayer->GetMapMgr()->GetInterface()->SpawnCreature(4417, SSX-5, SSY-6, SSZ, SSO-4, true, false, 0, 0)->Despawn(600000, 0);
	}
};
...
void SetupDeadmines(ScriptMgr * mgr)
{
	...
	// Declaration (BB)
	mgr->register_gameobject_script(101831, &Levier_RhahkZor::Create);
	mgr->register_gameobject_script(101832, &Levier_sneed::Create);
	mgr->register_gameobject_script(101834, &Levier_Gilnid::Create);
	mgr->register_gameobject_script(101833, &Levier_Canon::Create);
	mgr->register_gameobject_script(17155, &Poudre_canon::Create);
}
