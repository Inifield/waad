...
public:
	...
	void OnCombatStart(Unit* mTarget)
	{
		...
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je ne peut pas vous laisser nous gêner!");
	...
	void OnTargetDied(Unit* mTarget)
	{
		if(_unit->GetHealthPct() > 0)	
		{
			if(!form)
			{
				...
				{
				case 0:
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Ils m'ont forcé à le faire...");
					_unit->PlaySoundToSet(11291);
					break;

				case 1:
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je n'avais pas le choix.");
			...
			else
			{
				...
				{
				case 0:
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL,  "Je vais nous debarasser de vous.");
					_unit->PlaySoundToSet(11298);
					break;

				case 1:
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous ne vallez pas mieux qu'eux!");
	...
	{
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "C'est vous la maladie...pas moi.");
	...
	void AIUpdate()
	{
		if(!form) //water form
		{
			...
			if(dist > 25)
			{
				...
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Aaghh, le poison...");
		...
		else //poison form
		{
			...
			if(dist < 25)
			{
				...
				form = false;
				_unit->SetUInt32Value(UNIT_FIELD_DISPLAYID, 20162);
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Ahhh ca va mieux, beaucoup mieux.");
...
public:
	...
	void OnCombatStart(Unit* mTarget)
	{
		...
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Enfin mon exil s'acheve!");
	...
	void OnTargetDied(Unit* mTarget)
	{
		if (_unit->GetHealthPct() > 0)
		{
			if(Phase) //blood elf form
			{
				...
				{
				case 0:
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Tue! TUE!");
					_unit->PlaySoundToSet(11314);
					break;

				case 1:
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Voila! J'AIME CA!");
					_unit->PlaySoundToSet(11315);
					break;

				case 2:
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Qui est le maitre maintenant?");
			...
			else //demon form
			{
				...
				{
				case 0:
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je n'ai pas mon egal.");
					_unit->PlaySoundToSet(11306);
					break;
				case 1:
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Péri, créature mortelle.");
					_unit->PlaySoundToSet(11307);
					break;
				case 2:
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "oui, OUI! Ahahah!");
	...
	void OnDied(Unit * mKiller)
	{
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous ne pouvez pas me tuer! Imbeciles, Je reviendrais! Je... aarghh...");
...
// Frk encore un petit modif ... Item*SlotDisplay ? ItemEntry* ?
		_unit->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID, _unit->proto->ItemEntry1);
		_unit->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID_1, _unit->proto->ItemEntry2);
		...
		if(Phase == 0 || Phase == 3) //nightelf phase
		{
			...
			if(Phase == 0)
			{
				...
				if(!SwitchTimer)
				{
					...
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Hors d'ici elfe insignifiant, je prend le contrôle");
			...
			else
			{
				...
				case 0:
					...
					_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Non... non! Mais qu'avez vous fait? C'est moi le maitre! Vous entendez? Moi... aaggh! Impossible de le retenir.");
...
public:
	...
	void Free()
	{
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Enfin je suis libre. Cela fait bien trop longtemps que je n'ai gouter à la vrai liberté!");
...
bool ChaosBlast(uint32 i, Spell * pSpell)
{
	...
	if ( pSpell == NULL || pSpell->m_caster == NULL )
		return true;

	pSpell->m_caster->CastSpell( static_cast<Unit*>(pSpell->m_targets.m_target), dbcSpell.LookupEntry( CHAOS_BLAST_EFFECT ), true );
	return true;
}
...
public:
	...
	void OnCombatStart(Unit* mTarget)
	{
		...
		EnrageTimer = 600;
		Enraged = false;
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Gardes, en position! Nous avons de la visite...");
	...
	void OnDied(Unit * mKiller)
	{
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Son excellence vous attend...");
	...
	void OnTargetDied(Unit* mTarget)
	{
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Me voila debarrassé de vous.");
	...
	void AIUpdate()
	{
		...
		{
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Votre présomption causera votre perte! Gardes! Prêtez moi votre force!");
   ...
   void OnCombatStart(Unit* pTarget)
   {
	...
	void OnDied(Unit* pKiller)
	{
		...
		if(FLK)
		{
			...
			FLK->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je suis plus puissant que jamais!");
...
public:
	...
	void OnDied(Unit * pKiller)
	{
		...
		if(FLK)
		{
			...
			FLK->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Aller tuez-les, je ne m'en porterai que mieux!");
...
public:
	...
	void OnDied(Unit * pKiller)
	{
		...
		if(FLK)
		{
			...
			FLK->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Plus de savoir, plus de pouvoir!");
...
public:
	...
	void OnCombatStart(Unit* mTarget)
	{
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Que les flots des profondeurs vous emportent!");
	...
	void OnDied(Unit * mKiller)
	{
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Par...les grands courants...d'ageon.");
	...
	void OnTargetDied(Unit* mTarget)
	{
		if (_unit->GetHealthPct() > 0)	
		{
			...
			{
			case 0:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Seul les plus fort surviverons.");
				_unit->PlaySoundToSet(11328);
				break;
			case 1:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Luter ne fait qu'empirer les choses.");
				_unit->PlaySoundToSet(11327);
				break;
			case 2:
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "C'est fait!");
	...
	void AIUpdate()
	{
		if (_unit->GetAIInterface()->GetNextTarget() != NULL && _unit->GetCurrentSpell() == NULL)
		{
			...
			if (t > spells[1].casttime)
			{
				...
				{
					case 0:
						_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Detruisez les mes sujet!");
						_unit->PlaySoundToSet(11323);
						break;
					case 1:
						_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Par les marées!");
			...
			if (_unit->GetHealthPct() < 25)
			{
				if (t > spells[3].casttime)
				{
					...
					{
						case 0:
							_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Bientôt, ce sera terminé.");
							_unit->PlaySoundToSet(11325);
							break;
						case 1:
							_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Il est impossible de m'echapper!");
...
public:
	...
	void OnCombatStart(Unit* mTarget)
	{
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je vais te dechirer, de part en part!");
			_unit->PlaySoundToSet(11532);
			break;

		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Victoire au seigneur Illidan!");
			_unit->PlaySoundToSet(11533);
			break;

		case 2:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Je te crache dessus, racaille de la surface!");
			_unit->PlaySoundToSet(11534);
			break;

		case 3:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Mort aux etrangers!");
	...
	void OnDied(Unit * mKiller)
	{
		_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Seigneur Illidan, Je suis... désolé.");
	...
	void OnTargetDied(Unit* mTarget)
	{
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Votre vie touche à sa fin!");
			_unit->PlaySoundToSet(11541);
			break;

		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Vous avez échouer!");
	...
	void PhaseOneAndThree()
	{
		...
		else
		{
			if(_unit->GetHealthPct() <= 70)
			{
				...
				_unit->GetAIInterface()->setMoveType(MOVEMENTTYPE_WANTEDWP);
				_unit->GetAIInterface()->setWaypointToMove(1);
				_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "L'heure est venue! N'epargnez personne!");
	...
	void PhaseTwo()
	{
		...
		if(_unit->GetHealthPct() <= 50)
		{
			...
			_unit->SetUInt64Value(UNIT_FIELD_FLAGS, 0);
			_unit->RemoveAura(VASHJ_SHIELD);
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Il faudrait peut-être vous mettre à l'abris.");
	...
	void Shoot(Unit *target)
	{
		...
		{
		case 0:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Droit au coeur!");
			_unit->PlaySoundToSet(11536);
			break;

		case 1:
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "File vers ta cible!");
...
public:
	...
	void OnReachWP(uint32 iWaypointId, bool bForwards)
	{
		if (Phase == 1)
		{
			_unit->GetAIInterface()->setMoveType(MOVEMENTTYPE_WANTEDWP);
			_unit->GetAIInterface()->setWaypointToMove(6);
			_unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "C'etait la seul issue...pour moi!");
			_unit->PlaySoundToSet(11243);
		}
