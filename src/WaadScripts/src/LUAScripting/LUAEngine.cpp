...
RegType<Unit> UnitMethods[] = {
	...
	{ "MarkQuestKillObjectiveAsComplete", &luaUnit_MarkQuestKillObjectiveAsComplete },
	...
	{ "SetFlags", &luaUnit_SetFlags },
	{ "GetModel", &luaUnit_GetModel },
	{ "SetEquipModel", &luaUnit_SetEquipModel },	
...
void LuaEngine::LoadScripts()
{
	...
	do 
	{
		...
		if(!fn) fn = fd.cFileName;

        char * ext = strrchr(fd.cFileName, '.');
		if(ext != NULL)
		{
		  if((!stricmp(ext, ".lua")) && (strlen(ext)>3))
			luaFiles.insert(string(fn));
		  else if((!stricmp(ext, ".luc")) && (strlen(ext)>3))
	...
	while(filecount--)
	{
		...
		if(ext != NULL) 
		{
		 if((!strcmp(ext, ".lua")) && (strlen(ext)>3))
		 {
...
int luaItem_GossipMenuAddItem(lua_State * L, Item * ptr)
{
    ...
    int Coded = luaL_checkint(L, 4);

	Menu->AddItem(icon, menu_text, IntId, Coded=true);
	return 1;
}
...
int luaUnit_SpawnGameObject(lua_State * L, Unit * ptr)
{
	...
	if(entry_id)
	{
		GameObject* pC = ptr->GetMapMgr()->GetInterface()->SpawnGameObject(entry_id,x,y,z,o,false,0,0,1);
...
{
	CHECK_TYPEID(TYPEID_UNIT);
	float faction = luaL_checknumber(L,1);
	if(faction==0.0)
		return 0;
...
int luaUnit_SetEquipModel(lua_State * L, Unit * ptr)
{
	if(!ptr||!ptr->IsUnit())
		return 0;
	int equipmodel1 = luaL_checkint(L,1);
	int equipmodel2 = luaL_checkint(L,2);
	int equipmodel3 = luaL_checkint(L,3);
	ptr->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID,equipmodel1);
	ptr->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID_1,equipmodel2);
	ptr->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID_2,equipmodel3);
	return 0;
}
...
{
	CHECK_TYPEID(TYPEID_UNIT);
	int npcflags = luaL_checkint(L,1);
	if(!npcflags)
		return 0;

	ptr->SetUInt32Value(UNIT_NPC_FLAGS,npcflags);
	return 0;
}

int luaUnit_SetFlags(lua_State * L, Unit * ptr)
{
	CHECK_TYPEID(TYPEID_UNIT);
...
int luaUnit_MarkQuestKillObjectiveAsComplete(lua_State * L, Unit * ptr)
{
	CHECK_TYPEID(TYPEID_PLAYER);
	int questid = luaL_checkint(L,1);
	int creatureid = luaL_checkint(L,2);
	int objective = luaL_checkint(L,3);
	...
	qle->GetQuest()->required_mob[objective]=creatureid;
	qle->SendUpdateAddKill(objective,creatureid);
	if(qle->CanBeFinished()) qle->SendQuestComplete();
...
int luaUnit_MarkQuestObjectiveAsComplete(lua_State * L, Unit * ptr)
{
	...
	QuestLogEntry * qle = pl->GetQuestLogForEntry(questid);
	qle->SetMobCount(objective, qle->GetQuest()->required_mobcount[objective]);
	qle->SendUpdateAddKill(objective,0);
	if(qle->CanBeFinished()) qle->SendQuestComplete();
    qle->UpdatePlayerFields();
...
int luaUnit_KnockBack(lua_State * L, Unit * ptr)
{
	...
	float affect1 = luaL_checknumber(L,3);
	float affect2 = luaL_checknumber(L,4);
...
int luaUnit_ResurrectPlayer(lua_State * L, Unit * ptr)
{
	CHECK_TYPEID(TYPEID_PLAYER);
	if(ptr)
		static_cast< Player * > ( ptr ) ->ResurrectPlayer(NULL);
	return 0;
}
