...
bool ConfigFile::SetSource(const char *file, bool ignorecase)
{
	...
	if(file != 0)
	{
		...
		if(!f)
		{
			sLog.outError("ConfigFile", "Could not open %s.", file);
			return false;
		}
...
parse:
			...
			if(in_block)
			{
				...
				if(in_multiline_quote)
				{
					...
					{
						sLog.outError("ConfigFile", "Quote without variable.");
						return false;
					}
			...
			else
			{
				...
				if(offset != string::npos)
				{
					...
					{
						sLog.outError("ConfigFile", "Block without name.");
						return false;
					}
			...
			{
				sLog.outError("ConfigFile", "Exception in config parsing.");
				return false;
			}
		...
		if(in_block)
		{
			sLog.outError("ConfigFile", "Unterminated block.");
			return false;
		}

		if(in_multiline_comment)
		{
			sLog.outError("ConfigFile", "Unterminated comment.");
			return false;
		}

		if(in_multiline_quote)
		{
			sLog.outError("ConfigFile", "Unterminated quote.");
			return false;
		}
