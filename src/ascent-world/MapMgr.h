...
/////////////////////////////////////////////
	...
	GameObjectMap m_GOStorage;
	uint32 m_GOArraySize;
	uint32 m_GOHighGuid;
	GameObject * CreateGameObject(uint32 entry);

	ASCENT_INLINE uint32 GetGoHighGuid() { return(m_GOHighGuid); }
	...
	{
		GameObjectMap::iterator itr = m_GOStorage.find(guid);
		return (itr != m_GOStorage.end()) ? m_GOStorage[guid] : NULL;
	}
...
public:
	...
	VehicleSqlIdMap _sqlids_vehicles; // HearthStone
	CreatureSqlIdMap _sqlids_creatures;
	GameObjectSqlIdMap _sqlids_gameobjects;

	Vehicle  *GetSqlIdVehicle(uint32 sqlid); // HearthStone
	...
	deque<uint32> _reusable_guids_vehicle;  // HearthStone
	...
	WorldStateManager* m_DBstateManager;
...
public:
	...
	// get!
	ASCENT_INLINE WorldStateManager& GetDBStateManager() { return *m_DBstateManager; } // Waad
