...
bool HandleInfoCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	...
	pConsole->Write("======================================================================\r\n");
	pConsole->Write("Server Revision: AscentWaad %s r%u/%s-%s-%s\r\n(url: ascentcommunity.com/arbonne.games-rpg.net)\r\n", BUILD_TAG,BUILD_REVISION, CONFIG, PLATFORM_TEXT, ARCH);
...
bool HandleGMsCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	...
	pConsole->Write("Les GMs suivants sont connectés sur le serveur: \r\n");
	pConsole->Write("======================================================\r\n");
	pConsole->Write("| %21s | %15s | % 03s  |\r\n" , "Nom", "Permissions", "Latence");
	pConsole->Write("======================================================\r\n");
	...
	for (itr = objmgr._players.begin(); itr != objmgr._players.end(); itr++)
	{
		if(itr->second->GetSession()->HasPermissions())
		{
			pConsole->Write("| %21s | %15s | %03u ms |\r\n" , itr->second->GetName(), sGMSecurityMgr.Serialize(itr->second->GetSession()->GetPermissions()).c_str(), itr->second->GetSession()->GetLatency());
		}
	}
	objmgr._playerslock.ReleaseReadLock();
...
bool HandleOnlinePlayersCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	...
	pConsole->Write("Les joueurs suivants sont connectés sur le serveur: \r\n");
	pConsole->Write("======================================================\r\n");
	pConsole->Write("| %21s | %15s | % 03s  |\r\n" , "Nom", "Niveau", "Latence");
	pConsole->Write("======================================================\r\n");
...
bool HandleBanAccountCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	...
	pConsole->Write("Le compte '%s' a été banni %s%s. Les changements seront effectifs avec le prochain cycle de rechargement.\r\n", argv[1], 
		timeperiod ? "jusqu'a " : "pour toujours", timeperiod ? ConvertTimeStampToDataTime(timeperiod+(uint32)UNIXTIME).c_str() : "");
...
bool HandleUnbanAccountCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	...
	pConsole->Write("Le compte '%s' a été débanni.\r\n", argv[1]);
	return true;
}
