...
enum Target_Filter // Note Randdrick : énumération des différentes trajectoires possibles
{
	TARGET_FILTER_NONE,
...
//	void HandleProcSpellOnSpell(Unit* Victim,uint32 damage,bool critical);//nasty, some spells proc other spells
	...
	//----- Waad - Gestion des nouvelles trajectoire des sorts lancés par les NPC - Fortement inspiré des MoonScripts - Par Randdrick
	float GetDetectRange(Unit *Target); // distance entre la source et la cible.
	bool IsUnitValidTarget(Object* pObject, Target_Filter pFilter); // Validité de la trajectoire ?
	Unit* GetBestTargetPlayer(Target_Filter pFilter=TARGET_FILTER_NONE); // Meilleur Trajectoire trouvée vers le joueur aggro.
	Unit* GetBestTargetUnit(Target_Filter pFilter=TARGET_FILTER_NONE); // Meilleur Trajectoire trouvée pour l'unité (Player, pets, NPCs, totem, etc.)	
	Unit* ChoiceOfBestTarget(UnitArray& pTargetArray, Target_Filter pFilter); // Choix de la meilleur trajectoire sur l'unité visée( S'applique aussi bien au joueur qu'au NPC).
	Unit* ChoiceOfNearestTarget(UnitArray& pTargetArray); // Choix de la trajectoire la plus proche.
	Unit* ChoiceOfSecondMostHatedTarget(UnitArray& pTargetArray); // Choix de la deuxième cible la plus détestée.
