...
void LogonCommHandler::ReloadForcedPermissions() //ajout par B.B.
{
	...
	sLog.outDebug("LogonCommClient: Mise a jour des permissions player.");

	QueryResult * result = CharacterDatabase.Query("SELECT * FROM account_forced_permissions");

	forced_permissions.clear(); //on purge les permissions forcées
...
void LogonCommHandler::Connect(LogonServer * server)
{
 ...
 Log.Success("Miscellaneous","Conversion Latin1 vers UTF8 : %s.",(sWorld.m_Latin2UTF8) ? "Oui":"Non");

 // WaadFun
 if(sWorld.m_WarriorPetAllowed) Log.Success("[WaadFun]","Fun: Les familers pour Guerriers sont autorises.");
 if(sWorld.m_JdRPetListenning)  Log.Success("[WaadFun]","JdR: Les familers sont a l'ecoute en fonction de leur loyaute.");

 // Affichage Zone de Debug (Branruz)
 if(sWorld.m_DebugTransporters) Log.Warning("[DebugArea]", "Suivi console: Transporter %u",sWorld.m_DebugTransporters);
 if(sWorld.m_DebugPlayers)      Log.Warning("[DebugArea]", "Suivi console: Suivi des joueurs");

 // Relay Irc
 if(sWorld.m_IrcUseChatIrc) Log.Success("[WaadIrcRelay]","Le relay Irc est utilisé.");
}
