...
enum ChatMsg
{
	...
	CHAT_MSG_LOOT									= 27,
	//CHAT_MSG_SYSTEM									= 28, 
	CHAT_MSG_MONEY									= 28, // HearthStone
	CHAT_MSG_OPENING								= 29, // HearthStone
	CHAT_MSG_TRADESKILLS							= 30, // HearthStone
	CHAT_MSG_PET_INFO								= 31, // HearthStone
	CHAT_MSG_COMBAT_MISC_INFO						= 32, // HearthStone
	CHAT_MSG_XP_GAIN								= 33, // HearthStone
	CHAT_MSG_HONOR_GAIN								= 34, // HearthStone
	CHAT_MSG_COMBAT_FACTION_CHANGE					= 35, // HearthStone
	CHAT_MSG_BG_SYSTEM_NEUTRAL						= 36, // HearthStone
	CHAT_MSG_BG_SYSTEM_ALLIANCE						= 37, // HearthStone
	CHAT_MSG_BG_SYSTEM_HORDE						= 38, // HearthStone
	CHAT_MSG_BG_EVENT_NEUTRAL						= 36, // HearthStone
	CHAT_MSG_BG_EVENT_ALLIANCE						= 37, // HearthStone
	CHAT_MSG_BG_EVENT_HORDE							= 38, // HearthStone
	...
	//CHAT_MSG_RAID_BOSS_WHISPER					= 42, // HearthStone
	...
	//CHAT_MSG_ACHIEVEMENT							= 47, // HearthStone
	//CHAT_MSG_GUILD_ACHIEVEMENT					= 48, // HearthStone
	CHAT_MSG_ACHIEVEMENT							= 48, // Test
...
class SERVER_DECL CommandTableStorage : public Singleton<CommandTableStorage>
{
	...
	ChatCommand * _worldannounceCommandTable;	// cinemania
	ChatCommand * _serverCommandTable;			// cinemania
	ChatCommand * _lookupCommandTable;			// cinemania
	ChatCommand * _achCommandTable;		// Mizuka
...
protected:
	...
	int statStringAbbr(const char* s1, const char* s2);
	void SendMultilineMessage(WorldSession *m_session, const char *str);

#ifdef SMARTCACHE
	typedef WDFK::smart_cache<std::string, ChatCommand*>::const_iterator smart_iterator;
	WDFK::smart_cache<std::string, ChatCommand*> m_Cache;
#endif
	bool ExecuteCommandInTable(ChatCommand *table, const char* text, WorldSession *m_session, bool cache = false);
	...
	bool HandleModifyHappinessCommand(const char* args, WorldSession *m_session);
	void HandleModifyHapinesseCommandHelp(WorldSession *plrSession);
	...
	bool HandlePvPCommand( const char * args , WorldSession * m_session ); // (Branruz)
	bool HandleAddPlayerAura( const char *args , WorldSession *m_session ); // (Branruz)
	...
	// WorldAnnounce commands
	bool HandleWorldAnnounceList(const char* args, WorldSession *m_session);
	bool HandleWorldAnnounceStart(const char* args, WorldSession *m_session);
	bool HandleWorldAnnounceStop(const char* args, WorldSession *m_session);
	bool HandleWorldAnnounceModify(const char* args, WorldSession *m_session);
	bool HandleWorldAnnounceCreate(const char* args, WorldSession *m_session);
	bool HandleWorldAnnounceRemove(const char* args, WorldSession *m_session);
	...
	bool HandleRespawnCommand(const char* args, WorldSession *m_session);
	...
	bool HandleCreatureSetPhasing(const char * args, WorldSession * m_session);
	...
	bool HandleGODisplay(const char *args, WorldSession *m_session);
	...
	bool HandleListAIAgentBaseCommand(const char* args, WorldSession *m_session);
	bool HandleTargetTypeAIAgentCommand(const char* args, WorldSession *m_session);
    ...
    bool HandleNpcSpeakCommand(const char *args, WorldSession *m_session);
	...
	bool HandleAchievementComplete( const char *args , WorldSession *m_session );
	bool HandleAchievementGetName( const char *args , WorldSession *m_session );
	...
	bool HandleChatIrc( const char * args, WorldSession * m_session );

	bool HandleDebugSynchroClient(const char *args, WorldSession *m_session);
	// HearthStone
