...
struct CreatureHeight
{
  uint32 Id;
  // char *Name; // Pas utilisé
  float Taille;
};
...
struct CreatureProto
{
	...
	/* AI Stuff */ 
	int32 vehicle_entry; // HearthStone (Vehicule)
...
public:
	...
	void LooseRage(int32 decayValue);
	void LooseRunic(int32 decayValue);
	...
	//void LoadAIAgents(CreatureTemplate * t); // OBSOLETE (Branruz)
	//void LoadAIAgents();                     // OBSOLETE (Branruz)
	void DeleteFromDB();

	void OnJustDied();
	void OnRemoveCorpse();
	void OnRespawn(MapMgr * m);
	//If you can't guarantee the context, leave this as false and it will do it itself.
	void SafeDelete();
	//void Despawn();
