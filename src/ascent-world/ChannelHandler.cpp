...
void WorldSession::HandleChannelJoin(WorldPacket& recvPacket)
{
	...
	if((dbc_id==CHANNEL_RECHERCHE_GROUPE) && !sWorld.m_lfgForNonLfg) // Recherche de Groupe
	{
	...
	if( sWorld.GmClientChannel.size() && !stricmp(sWorld.GmClientChannel.c_str(), channelname.c_str()) && !HasPermissions())
		return;
...
{
	// CHECK_PACKET_SIZE(recvPacket, 1); Pourquoi ca ?
	/* En commentaire, vu le CHECK_PACKET_SIZE, a voir plus tard (Brz)
	string channelname;
	Channel * chn;
	recvPacket >> channelname;
