...
void ArenaTeam::Destroy()
{
	...
	snprintf(buffer,1024, "L'équipe d'arène, '%s', est dissoute.", m_name.c_str());
...
bool ArenaTeam::AddMember(PlayerInfo * info)
{
	...
	if(plr)
	{
		...
		plr->GetSession()->SystemMessage("Vous êtes maintenant un membre de l'équipe d'arène, '%s'.", m_name.c_str());
	}
	return true;
}
...
void ArenaTeam::Roster(WorldPacket & data)
{
	...
	for(uint32 i = 0; i < m_memberCount; i++) // test  // ++i) (branruz)
	{
		PlayerInfo* info = m_members[i].Info;
		// TODO : burlex figure out why this became null => ++i est different de i++
		if( info != NULL )
		{
...
void ArenaTeam::SetLeader(PlayerInfo * info)
{
	...
	char buffer[1024];
	WorldPacket * data;
	snprintf(buffer, 1024,"%s est désormais le capitaine de l'équipe d'arène, '%s'.", info->name, m_name.c_str());
	data = sChatHandler.FillSystemMessageData(buffer);
	m_leader=info->guid;
    SendPacket(data);
	delete data;
...
void WorldSession::HandleArenaTeamAddMemberOpcode(WorldPacket & recv_data)
{
	...
	if(plr == NULL)
	{
		SystemMessage("Le joueur `%s` est inexistant ou n'est pas en ligne.", player_name.c_str());
		return;
	}

	if(pTeam->m_leader != _player->GetLowGUID())
	{
		SystemMessage("Vous n'êtes pas le capitaine de cette équipe d'arène.");
		return;
	}

	if(plr->getLevel() < 70)
	{
		SystemMessage("Le joueur doit être au niveau 70 pour rejoindre une équipe d'arène.");
		return;
	}

	if(plr->m_arenaTeams[pTeam->m_type] != NULL)
	{
		SystemMessage("Ce joueur est déjà dans une équipe d'arène de ce type.");
		return;
	}

	if(plr->m_arenateaminviteguid != 0)
	{
		SystemMessage("Ce joueur est déjà invité dans une équipe d'arène");
		return;
	}

	if(plr->GetTeam() != _player->GetTeam() && !HasGMPermissions())
	{
		SystemMessage("Ce joueur est un membre d'une faction différente.");
		return;
	}
...
void WorldSession::HandleArenaTeamRemoveMemberOpcode(WorldPacket & recv_data)
{
	...
	{
		SystemMessage("Vous n'êtes pas dans une équipe de ce type.");
		return;
	}
	...
	{
		SystemMessage("Ce joueur est introuvable.");
		return;
	}

	if(!team->HasMember(inf->guid))
	{
		SystemMessage("Ce joueur n'est pas un membre de votre équipe d'arène.");
		return;
	}
	...
	if(team->RemoveMember(inf))
	{
		char buffer[1024];
		WorldPacket * data;
		snprintf(buffer,1024,"%s a été enlevé de l'équipe d'arène '%s'.", inf->name, team->m_name.c_str());
		data = sChatHandler.FillSystemMessageData(buffer);
		team->SendPacket(data);
		delete data;
		SystemMessage("Enlevé %s de l'équipe d'arène '%s'.", inf->name, team->m_name.c_str());
	}
}
...
void WorldSession::HandleArenaTeamInviteAcceptOpcode(WorldPacket & recv_data)
{
	...
	{
		SystemMessage("Vous n'avez pas été invité dans une autre équipe d'arène.");
		return;
	}
	...
	if(team == 0)
	{
		SystemMessage("Cette équipe d'arène n'existe pas.");
		return;
	}

	if(team->m_memberCount >= team->m_slots)
	{
		SystemMessage("Cette équipe est désormais complète.");
		return;
	}

	if(_player->m_arenaTeams[team->m_type] != NULL)		/* shouldn't happen */
	{
		SystemMessage("Vous avez déjà été dans une équipe d'arène de cette taille.");
		return;
	}
	...
	if(team->AddMember(_player->m_playerInfo))
	{
		char buffer[1024];
		WorldPacket * data;
		snprintf(buffer,1024,"%s rejoind l'équipe d'arène, '%s'.", _player->GetName(), team->m_name.c_str());
		data = sChatHandler.FillSystemMessageData(buffer);
		team->SendPacket(data);
		delete data;
	}
	...
	{
		SendNotification("Erreur interne.");
	}
}
...
void WorldSession::HandleArenaTeamInviteDenyOpcode(WorldPacket & recv_data)
{
	...
	if(_player->m_arenateaminviteguid == 0)
	{
		SystemMessage("Vous n'avez pas été invité.");
		return;
	}
	...
	if(plr != NULL)
		plr->GetSession()->SystemMessage("%s refuse votre invitation en équipe d'arène pour %s.", _player->GetName(), team->m_name.c_str());
}
...
void WorldSession::HandleArenaTeamLeaveOpcode(WorldPacket & recv_data)
{
	...
	{
		SystemMessage("Vous n'êtes pas dans une équipe d'arène de ce type.");
		return;
	}
	...
	if(team->m_leader == _player->GetLowGUID())
	{
		SystemMessage("Vous ne pouvez pas encore quitter l'équipe, nommer quelqu'un d'autre comme capitain d'abord.");
		return;
	}
	...
	if(team->RemoveMember(_player->m_playerInfo))
	{
		char buffer[1024];
		WorldPacket * data;
		snprintf(buffer,1024,"%s quitte l'équipe d'arène, '%s'.", _player->GetName(), team->m_name.c_str());
		data = sChatHandler.FillSystemMessageData(buffer);
		team->SendPacket(data);
		delete data;
		SystemMessage("Vous avez quitté l'équipe d'arène '%s'.", team->m_name.c_str());
	}
}
...
void WorldSession::HandleArenaTeamDisbandOpcode(WorldPacket & recv_data)
{
	...
	if(team->m_leader != _player->GetLowGUID())
	{
		SystemMessage("Vous n'êtes pas le capitaine de cette équipe.");
		return;
	}
...
void WorldSession::HandleArenaTeamPromoteOpcode(WorldPacket & recv_data) 
{
	...
	if( (inf = objmgr.GetPlayerInfoByName(name.c_str())) == NULL )
	{
		SystemMessage("Ce joueur ne peut pas être trouvé.");
		return;
	}
