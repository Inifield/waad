...
{
	Log.Notice("AuctionMgr", "Chargement contenu des Hotels de ventes...");
	...
	if(res)
	{
		...
		do
		{
			...
			if( !((++c) % period) )
				Log.Notice("AuctionHouse", "%u/%u Fait, complet à %u%%.", c, res->GetRowCount(), float2int32( (float(c) / float(res->GetRowCount()))*100.0f ));
