...
Channel::Channel(const char * name, uint32 team, uint32 type_id)
{
	...
	if( pDBC != NULL )
	{
		...
		if( (pDBC->flags & 0x00000008) && !(pDBC->flags & 0x00000020) ) // ---- ---- 0011 1011
			m_flags |= 0x08;		// trade

		if( pDBC->flags & 0x00000010 || pDBC->flags & 0x00000020 )
			m_flags |= 0x20;		// city flag

		if( pDBC->flags & 0x00040000 )                                  // ---- ---- 0011 1001
	...
	if( (type_id == CHANNEL_GENERAL) || (type_id == CHANNEL_COMMERCE) || (type_id == CHANNEL_RECHERCHE_GROUPE) )
	{
		m_minimumLevel = 1; // 10; pourquoi lv 10 min, on s'en cogne finalement de ca (Brz)
		m_general = true;
		m_announce = false;
	}
}
...
void Channel::AttemptJoin(Player * plr, const char * password)
{
	...
	if( !m_general && sGMSecurityMgr.UseUniversalLang(plr->GetSession()->GetPermissions()) )
...
void Channel::Moderate(Player * plr)
{
	...
	if(!(itr->second & CHANNEL_FLAG_OWNER || itr->second & CHANNEL_FLAG_MODERATOR) && !sGMSecurityMgr.UseUniversalLang(plr->GetSession()->GetPermissions()))
	{
