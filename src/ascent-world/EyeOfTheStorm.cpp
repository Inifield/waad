...
static const char * EOTSControlPointNames[EOTS_TOWER_COUNT] = 
	{
		"la tour des Elfs de Sang",
		"les ruines des Saccageurs Gangrenes",
		"la tour des Mages",
		"les ruines Draenei"
...
void EyeOfTheStorm::HookOnAreaTrigger(Player * plr, uint32 id)
{
	...
	if(bonusid > -1){
		...
		{
			spellid = EOTSm_buffs[x]->GetInfo()->trap.spellId; // sound3
	...
	*/
	...
	const char * msgs[2] = { "L'Alliance a capture le drapeau.", "La Horde a capture le drapeau." };

	SendChatMessage( CHAT_MSG_BG_EVENT_ALLIANCE + team, 0, msgs[team] );
	//SendChatMessage( CHAT_MSG_BG_EVENT_ALLIANCE + team, 0,"%s a pris %s",(team) ? "La Horde" : "L'Alliance",
	//	                                    &EOTSControlPointNames[tid][0] );
...
void EyeOfTheStorm::HookFlagDrop(Player * plr, GameObject * obj)
{
	...
	PlaySoundToAll( 8212 );
	SendChatMessage( CHAT_MSG_BG_EVENT_ALLIANCE + plr->GetTeam(), plr->GetGUID(), "$N a pris le drapeau!" );
...
bool EyeOfTheStorm::HookSlowLockOpen(GameObject * pGo, Player * pPlayer, Spell * pSpell)
{
	...
	SetWorldState( 2757, 0 );
	SendChatMessage( CHAT_MSG_BG_EVENT_ALLIANCE + pPlayer->GetTeam(), pPlayer->GetGUID(), "$N a pris le drapeau!" );
...
void EyeOfTheStorm::EventResetFlag()
{
	...
	SetWorldState( 2757, 1 );
	//PlaySoundToAll( 8192 );
	PlaySoundToAll( SOUND_ALLIANCE_RETURNED ); // SOUND_HORDE_RETURNED c'est le meme!
	SendChatMessage( CHAT_MSG_BG_EVENT_NEUTRAL, 0, "Le drapeau a repris son emplacement." );
	m_flagHolder = 0;
}
...
void EyeOfTheStorm::OnCreate()
{
	...
	/* eww worldstates */
	wState.CreateWorldState(2565, 142);
	wState.CreateWorldState(2720, 0);
	wState.CreateWorldState(EOTS_WORLDSTATE_DISPLAYVALUE, 0);
	wState.CreateWorldState(EOTS_WORLDSTATE_DISPLAYON, 0);
	wState.CreateWorldState(2260, 0);
	wState.CreateWorldState(2264, 0);
	wState.CreateWorldState(2263, 0);
	wState.CreateWorldState(2262, 0);
	wState.CreateWorldState(2261, 0);
	wState.CreateWorldState(2259, 0);
	wState.CreateWorldState(2742, 0);
	wState.CreateWorldState(2741, 0);
	wState.CreateWorldState(2740, 0);
	wState.CreateWorldState(2739, 0);
	wState.CreateWorldState(2738, 0);
	wState.CreateWorldState(2737, 0);
	wState.CreateWorldState(2736, 0);
	wState.CreateWorldState(2735, 0);
	wState.CreateWorldState(2733, 0); // WORLDSTATE_EOTS_DRAENEI_RUINS_HORDE_DISPLAY
	wState.CreateWorldState(2732, 0); // WORLDSTATE_EOTS_DRAENEI_RUINS_ALLIANCE_DISPLAY
	wState.CreateWorldState(2731, 1); // WORLDSTATE_EOTS_DRAENEI_RUINS_NETURAL_DISPLAY
	wState.CreateWorldState(2730, 0); // WORLDSTATE_EOTS_MAGE_TOWER_ALLIANCE_DISPLAY
	wState.CreateWorldState(2729, 0); // WORLDSTATE_EOTS_MAGE_TOWER_HORDE_DISPLAY
	wState.CreateWorldState(2728, 1); // WORLDSTATE_EOTS_MAGE_TOWER_NETURAL_DISPLAY
	wState.CreateWorldState(2727, 0); // WORLDSTATE_EOTS_FELREAVER_RUINS_HORDE_DISPLAY
	wState.CreateWorldState(2726, 0); // WORLDSTATE_EOTS_FELREAVER_RUINS_ALLIANCE_DISPLAY
	wState.CreateWorldState(2725, 1); // WORLDSTATE_EOTS_FELREAVER_RUINS_NETURAL_DISPLAY
	wState.CreateWorldState(2724, 0); // WORLDSTATE_EOTS_BLOODELF_TOWER_HORDE_DISPLAY
	wState.CreateWorldState(2723, 0); // WORLDSTATE_EOTS_BLOODELF_TOWER_ALLIANCE_DISPLAY
	wState.CreateWorldState(2722, 1); // WORLDSTATE_EOTS_BLOODELF_TOWER_NEUTRAL_DISPLAY
	wState.CreateWorldState(2757, 1); // WORLDSTATE_EOTS_FLAG_NEUTRAL_DISPLAY
	wState.CreateWorldState(EOTS_WORLDSTATE_HORDE_BASES, 0); 
	wState.CreateWorldState(EOTS_WORLDSTATE_ALLIANCE_BASES, 0); 
	wState.CreateWorldState(2770, 1); // WORLDSTATE_EOTS_HORDE_VICTORY_POINTS_DISPLAY
	wState.CreateWorldState(2769, 1); // WORLDSTATE_EOTS_ALLIANCE_VICTORY_POINTS_DISPLAY
	wState.CreateWorldState(EOTS_WORLDSTATE_HORDE_VICTORYPOINTS, 0); 
	wState.CreateWorldState(EOTS_WORLDSTATE_ALLIANCE_VICTORYPOINTS, 0); 
	wState.CreateWorldState(3191, 2); // 
	wState.CreateWorldState(3218, 0);
	wState.CreateWorldState(3217, 0);
	wState.CreateWorldState(3085, 379);
	...
	for( i = 0; i < 2; ++i )
	{
		...
		m_bubbles[i]->SetFloatValue(OBJECT_FIELD_SCALE_X,0.12f);
		m_bubbles[i]->SetByte(GAMEOBJECT_BYTES_1, 0, 1);
		m_bubbles[i]->SetUInt32Value(GAMEOBJECT_FLAGS,GO_FLAG_NO_DESPAWN);
...
void EyeOfTheStorm::UpdateCPs()
{
	...
	for(i = 0; i < EOTS_TOWER_COUNT; ++i)
	{
		...
		{
			plr = *itr;
			if( plr->isAlive() && !(plr->IsStealth()) && !(plr->m_isGmInvisible) && !(plr->SchoolImmunityList[0]) 
				&& (plr->GetDistance2dSq( go ) <= EOTS_CAPTURE_DISTANCE) )
			{
				...
				{
					disp->insert( plr );
					SetWorldState(EOTS_WORLDSTATE_DISPLAYON, 1);
				}
			}
		}
		...
		if(playercounts[0] != playercounts[1])
		{
			...
			m_CPStatus[i] += delta;
			if( m_CPStatus[i] > 100 ) 	 m_CPStatus[i] = 100;
			else if( m_CPStatus[i] < 0 ) m_CPStatus[i] = 0;
			...
			if( m_CPStatus[i] == 0 )
			{
				if( m_CPBanner[i] && (m_CPBanner[i]->GetEntry() != EOTS_BANNER_HORDE) )
				{
					...
					SendChatMessage( CHAT_MSG_BG_EVENT_HORDE, 0,"La Horde a pris %s",
		                                                                  &EOTSControlPointNames[i][0] );
			...
			else if( m_CPStatus[i] == 100 )
			{
				if( m_CPBanner[i] && (m_CPBanner[i]->GetEntry() != EOTS_BANNER_ALLIANCE) )
				{
					...
					SendChatMessage( CHAT_MSG_BG_EVENT_ALLIANCE, 0,"L'Alliance a pris %s",
		                                                                  &EOTSControlPointNames[i][0] );
			...
			{
				if(  m_CPBanner[i] && 
					(((m_CPBanner[i]->GetEntry() == EOTS_BANNER_ALLIANCE) && (m_CPStatus[i] <= 50)) ||
		...
		for( ; eitr != eitrend; )
		{
			...
			{
				disp->erase( eitr2 );
				SetWorldState(EOTS_WORLDSTATE_DISPLAYON, 0);			// hide the cp bar
			}
			else
				SetWorldState(EOTS_WORLDSTATE_DISPLAYVALUE, m_CPStatus[i]);
		}
	}
...
bool EyeOfTheStorm::GivePoints(uint32 team, uint32 points)
{
	...
	if( m_points[team] >= 2000 )
	{
		...
		for(uint32 i = 0; i < 2; ++i)
		{
			for(set<Player*>::iterator itr = m_players[i].begin(); itr != m_players[i].end(); ++itr)
			{
				...
				if(i == m_winningteam)
				{
					...
					{
						(*itr)->GetSession()->SendNotification("Votre inventaire est plein!");
						delete item;
					}
					else
					{
		...
		} // End for(uint32 i = 0; i < 2; ++i)
		SendChatMessage( CHAT_MSG_BG_EVENT_ALLIANCE + team, 0,"%s Gagne!",(team) ? "La Horde" : "L'Alliance");
		m_mainLock.Release();
		SetWorldState( EOTS_WORLDSTATE_ALLIANCE_VICTORYPOINTS + team, m_points[team] );
		UpdatePvPData();
		return true;
	} // End if( m_points[team] >= 2000 )
...
void EyeOfTheStorm::SpawnBuff(uint32 x)
{
	...
	if(EOTSm_buffs[x] == NULL)
	{
		...
		EOTSm_buffs[x]->SetByte(GAMEOBJECT_BYTES_1, 0, 1); // GAMEOBJECT_ID
...
LocationVector EyeOfTheStorm::GetStartingCoords(uint32 Team)
{
	return LocationVector( EOTSStartLocations[Team][0], 
		...
		EOTSStartLocations[Team][2],EOTSStartLocations[Team][3] );
}
