...
//	*data << uint32( 0x00000300 );
	...
	*data << uint32( nodecounter ); // waypoint count
	pos = data->wpos();
	*data << nx << ny << nz;
...
void TaxiMgr::_LoadTaxiNodes()
{
	...
	for(i = 0; i < dbcTaxiNode.GetNumRows(); i++)
	{
		...
		if (node)
		{
			...
			{
				Log.Error("TaxiNode","Node out of range (%u) <- Report this to devs.",node->id);
				break;
			}
		}
	}
...
// DirectTaxi: Un seul chemin DOIT etre defini dans le dbc (le premier trouvé est pris)
void TaxiMgr::GetTaxiNodeFromTo(uint32 TaxiNodeId,uint32 *fromNode, uint32 *toNode)
{
	...
	if(gotcha)
	{
		*fromNode = itr->second->from; 
		*toNode   = itr->second->to;
		return;
	}
	else // Pas de chemin defini dans le From->To, seulement la destination recu (??)
	{
		for (itr = m_taxiPaths.begin(); itr != m_taxiPaths.end(); itr++)
		{
			if(itr->second->to == TaxiNodeId)
			{
				*fromNode = itr->second->from; 
				*toNode   = itr->second->to;
				return;
...
bool TaxiMgr::GetGlobalTaxiNodeMask( uint32 curloc, uint32 *Mask )
{
	...
	for (itr = m_taxiPaths.begin(); itr != m_taxiPaths.end(); itr++)
	{
		...
		{*/
			...
			if(field >= 12) continue;  //ArcEmu : The DBC can contain negative TO values??? That'll be 255 here (because we store everything unsigned), skip them!
