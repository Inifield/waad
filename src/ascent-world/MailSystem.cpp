	...
	if( !items.empty( ) )
	{
		for( itr = items.begin( ); itr != items.end( ); ++itr )
		{
			...
			for( j = 0; j < 7; ++j )
			{
				...
				data << pItem->GetUInt32Value( ITEM_FIELD_ENCHANTMENT_1_3 + 2 + ( j * 3 ) );
			}
...
void WorldSession::HandleSendMail(WorldPacket & recv_data )
{
	...
	if(player->name == _player->GetName() && !HasPermissions())
	{
...
void WorldSession::HandleMailCreateTextItem(WorldPacket & recv_data )
{
	...
	Item * pItem = objmgr.CreateItem(8383, _player); // Lettre simple : http://fr.wowhead.com/item=8383
	if (pItem== NULL) 
		return;
