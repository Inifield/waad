...
{
	// Table Creature_Spawns/Creature_StaticSpawns 22 colonnes

	int ColonneCount = 22; // Table Creature_Spawns/Creature_StaticSpawns 22 colonnes

	if( res->GetFieldCount() != ColonneCount ) 
	{
		if( first_table_warning )
		{
			first_table_warning = false;
			Log.LargeErrorMessage(LARGERRORMESSAGE_WARNING, "Votre table creature_spawns n'a pas le bon nombre de colonne (22).",
				"AscentWaad ne peut plus se charger sans crash du serveur.",
				"Vous devez corriger votre table.");
		}
...
bool CheckResultLengthGameObject(QueryResult * res)
{
	if( res->GetFieldCount() != 16 )
	{
		if( first_table_warningg )
		{
			...
			Log.LargeErrorMessage(LARGERRORMESSAGE_WARNING, "One of your gameobject_spawns table has the wrong column count. (16)",
		...
		if(result)
		{
			if(CheckResultLengthCreatures( result) )
			{
				do{
					...
					if(!spawns[cellx][celly]) spawns[cellx][celly] = new CellSpawns;
	...
	if(result)
	{
		if( CheckResultLengthCreatures(result) )
		{
			do{
				...
				cspawn->emote_state = fields[14].GetUInt32();
				//cspawn->respawnNpcLink = fields[15].GetUInt32();
				cspawn->channel_spell           = fields[16].GetUInt32();;
				cspawn->channel_target_creature = fields[17].GetUInt32();;
				cspawn->channel_target_go       = fields[18].GetUInt32();;
				cspawn->stand_state             = fields[19].GetUInt16();
				cspawn->phaseMask               = fields[20].GetInt32();;
		...
		if(result)
		{
			if( CheckResultLengthGameObject(result) )
				...
				do{
					...
					gspawn->faction = fields[13].GetUInt32();
					gspawn->scale   = fields[14].GetFloat();
					gspawn->phase   = fields[15].GetUInt32();
