...
void InstanceMgr::Shutdown()
{
	...
	for(i = 0; i < NUM_MAPS; ++i)
	{
		...
		if(m_singleMaps[i])
		{
			MapMgr* ptr = new MapMgr(NULL, NULL, NULL);
			ptr = m_singleMaps[i];
			ptr->KillThread();
			delete ptr;
			ptr=NULL;
...
uint32 InstanceMgr::PreTeleport(uint32 mapid, Player* plr, uint32 instanceid)
{
	...
	if( !plr->triggerpass_cheat )
	{
		...
		if( plr->getLevel()< sWorld.m_levelCap ) // 80)
		{
			...
			bool reqkey = (inf->heroic_key_1||inf->heroic_key_2)? true : false;
			bool haskey = (plr->GetItemInterface()->GetItemCount(inf->heroic_key_1, false) || plr->GetItemInterface()->GetItemCount(inf->heroic_key_2, false))? true : false;
			if(reqkey && !haskey)
				return INSTANCE_ABORT_HEROIC_MODE_NOT_AVAILABLE;
		}
	}
		...
		if(instanceid != 0)
		{
			...
			if(itr != instancemap->end()) 
			{
				...
				if( owns >= OWNER_CHECK_OK )
				{
					...
					sLog.outDebug("InstanceMgr","Check failed %s, return code %u",plr->GetName(), owns);
			}
			m_mapLock.Release();
			return INSTANCE_ABORT_NOT_FOUND;
		}
	...
	sLog.outDebug("InstanceMgr", "Prepared new instance %u for player %u and group %u on map %u. (%u)",in->m_instanceId, in->m_creatorGuid, in->m_creatorGroup, in->m_mapId, in->m_instanceId);
		...
		if(instancemap != NULL)
		{
			...
			for(itr = instancemap->begin(); itr != instancemap->end();)
			{
				...
				if(owns >= OWNER_CHECK_OK )
				{
					...
					sLog.outDebug("InstanceMgr","Check failed %s",plr->GetName());
			}
		}
...
void InstanceMgr::_LoadInstances()
{
	...
	if(result)
	{
		...
		sLog.outDebug("InstanceMgr", "No saved instances found.");
}
