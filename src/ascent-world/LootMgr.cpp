		...
		if(LootTable->end()==LootTable->find(entry_id))
		{
			...
			for(vector< tempy >::iterator itr2 = (*itr).second.begin(); itr2 != (*itr).second.end(); ++itr2)
			{
				...
				if(!proto)
				{
					...
					Log.Warning("LootMgr", "%u refer non-existant item %u. (%s)",entry_id,itemid,szTableName);
				}
				else
				{
...
*/
	...
	int8 error = _player->GetItemInterface()->CanReceiveItem(it, 1);
	if(error)
	{
