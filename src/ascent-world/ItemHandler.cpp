...
void WorldSession::HandleSwapItemOpcode(WorldPacket& recv_data)
{
	...
	if(!SrcItem)
	{
		sLog.outDetail("ITEM: GetInventoryItem failed SrcInvSlot: %d, SrcSlot: %d", SrcInvSlot, SrcSlot);
		return;
	}
	...
	// Debug
	//Log.Warning("HandleSwapItemOpcode","SrcItem: %u",(SrcItem)?SrcItem->GetEntry():NULL);
	//Log.Warning("HandleSwapItemOpcode","DestItem: %u",(DstItem)?DstItem->GetEntry():NULL);
	//-----
	...
	{   //check if it will go to equipment slot
		if(SrcInvSlot==INVENTORY_SLOT_NOT_SET)//not bag
		{
			if(DstItem->IsContainer())
			{
				...
				{
					if(SrcSlot < INVENTORY_SLOT_BAG_START || SrcSlot >= INVENTORY_SLOT_BAG_END || 
					   SrcSlot < BANK_SLOT_BAG_START      || SrcSlot >= BANK_SLOT_BAG_END)
					{
			...
			{
				error=GetPlayer()->GetItemInterface()->CanEquipItemInSlot(SrcInvSlot, SrcSlot, DstItem->GetProto());
				if(error)
				{
		...
		else
		{
			...
			error=GetPlayer()->GetItemInterface()->CanEquipItemInSlot(SrcInvSlot, SrcInvSlot, DstItem->GetProto());
			if(error)
			{
				_player->GetItemInterface()->BuildInventoryChangeError(SrcItem, DstItem, error);
				return;
	...
	{   //check if it will go to equipment slot
		if(DstInvSlot==INVENTORY_SLOT_NOT_SET)//not bag
		{
			...
			{
				error=GetPlayer()->GetItemInterface()->CanEquipItemInSlot(DstInvSlot, DstSlot, SrcItem->GetProto());
				if(error)
				{
					_player->GetItemInterface()->BuildInventoryChangeError(SrcItem, DstItem, error);
					return;
		...
		else
		{
			...
			error=GetPlayer()->GetItemInterface()->CanEquipItemInSlot(DstInvSlot, DstInvSlot, SrcItem->GetProto());
			if(error)
			{
				_player->GetItemInterface()->BuildInventoryChangeError(SrcItem, DstItem, error);
				return;
	...
	else
	{
		...
		if(SrcItem)
		{
			...
			if(!result)
			{
				Log.Error("HandleSwapItem","Error while adding item to dstslot");
		...
		if(DstItem)
		{
			...
			if(!result)
			{
				Log.Error("HandleSwapItem","Error while adding item to srcslot");
				//put items back if we can!
...
void WorldSession::HandleSwapInvItemOpcode( WorldPacket & recv_data )
{
	...
	error = _player->GetItemInterface()->CanEquipItemInSlot(INVENTORY_SLOT_NOT_SET, dstslot, srcitem->GetProto(), skip_combat);
	if(error)
	{
		if( dstslot < INVENTORY_KEYRING_END )
		{
...
*/
	if(dstitem)
	{
		error=_player->GetItemInterface()->CanEquipItemInSlot(INVENTORY_SLOT_NOT_SET, srcslot, dstitem->GetProto(), skip_combat);
		if(error)
		{
			if(srcslot < INVENTORY_KEYRING_END)
			{
	...
	// swap items
	//Log.Warning("CMSG_SWAP_INV_ITEM","%s range ses sacs",_player->GetName());
	_player->GetItemInterface()->SwapItemSlots(srcslot, dstslot);
}
// Reponse a CMSG_DESTROYITEM
void WorldSession::HandleDestroyItemOpcode( WorldPacket & recv_data )
{
	...
	if(it)
	{
		...
		/* Note randdrick : n'existe plus depuis la version 3.3.3
		...
		if(_player->GetCurrentSpell() && _player->GetCurrentSpell()->m_caster==pItem)
		{
			_player->GetCurrentSpell()->m_caster=NULL;
			_player->GetCurrentSpell()->cancel(SPELL_FAILED_INTERRUPTED); // "Interrompu"
		}

		pItem->DeleteFromDB();
		pItem->Delete();
		pItem = NULL; // (Randdrick)
	}
	else
	{
	 Log.Warning("CMSG_DESTROYITEM","%s : Item non trouvé, SrcInvSlot: %u, SrcSlot %u",
		 this->GetPlayer()->GetName(),SrcInvSlot,SrcSlot);
	}
}
			...
			if( mainhandweapon != NULL && mainhandweapon->GetProto()->InventoryType == INVTYPE_2HWEAPON )
			{
				...
				if( !_player->GetItemInterface()->SafeAddItem(mainhandweapon, result.ContainerSlot, result.Slot) )
					if( !_player->GetItemInterface()->AddItemToFreeSlot(mainhandweapon) )		// shouldn't happen either.
						mainhandweapon->Delete();
			}
		}
	}
	else
	{
		error = _player->GetItemInterface()->CanEquipItemInSlot(INVENTORY_SLOT_NOT_SET, Slot, eitem->GetProto());
		if(error)
		{
			_player->GetItemInterface()->BuildInventoryChangeError(eitem,NULL, error);
			return;
...
void WorldSession::HandleBuyBackOpcode( WorldPacket & recv_data )
{
	...
	if (it)
	{
		...
		error = _player->GetItemInterface()->CanReceiveItem(it->GetProto(), amount);
		if (error)
		{
...
void WorldSession::HandleBuyItemInSlotOpcode( WorldPacket & recv_data ) // drag & drop
{
	...
	uint32 itemid;
	int32 slot; // Test, il faudrait faire un sniff offi (int16 ?)
	uint8 amount = 0;
	uint8 error;
	...
	//recv_data >> slot; // 309
	recv_data >> amount;
	recv_data >> slot;   // 322 - 332 // Dernier recu, passe en int32 (a surveiller)
    if(amount == 0)
      return;
...
//		sLog.outDebug( "SUPADBG can't carry #1 (%u>%u)" , ci.amount*amount , it->MaxCount );
	...
	if(slot != INVENTORY_SLOT_NOT_SET)
	{
		...
		{
			Item *tmpitem = _player->GetItemInterface()->GetItemByGUID(bagguid);
			if(c == NULL || !tmpitem->IsContainer())
				return;
			c = (Container*)(tmpitem);
	...
	else
	{
		if((bagguid>>32))
		{
			...
			// Le player ne possede pas l'item (??)
			if(tmpitem == NULL)
			{
				//Log.Error("[HandleBuyItemInSlotOpcode]","Bag Guid inconnu: %u - %08X",bagguid,bagguid); 
                _player->GetItemInterface()->BuildInventoryChangeError(0, 0, INV_ERR_ITEM_NOT_FOUND);
				return;//non empty

			}

			c = tmpitem->IsContainer()? (Container*)(tmpitem) : NULL;
	...
	error = _player->GetItemInterface()->CanReceiveItem(it, amount);
	if(error)
	{
		_player->GetItemInterface()->BuildInventoryChangeError(NULL, NULL, error);
		return;
	...
	error = _player->GetItemInterface()->CanAffordItem(it,amount,unit);
	if(error)
	{
...
//	int8 bagslot = 0;
	...
	recv_data >> srcguid >> itemid;
	//recv_data >> amount >> slot; // 309
	recv_data >> slot >> amount;   // 322 - 332

	sLog.outDetail( "CMSG_BUY_ITEM: itemid %u ,amount %u ,slot %u",itemid,amount,slot );
	...
	error = _player->GetItemInterface()->CanReceiveItem(it, amount*item.amount);
	if(error)
	{
		_player->GetItemInterface()->BuildInventoryChangeError(NULL, NULL, error);
		return;
		...
		if(itr->itemid && (itr->max_amount == 0 || (itr->max_amount>0 && itr->available_amount >0)))
		{
			...
			if(curItem)
			{
				...
				if(curItem->AllowableRace && !(_player->getRaceMask() & curItem->AllowableRace))
					continue;
                if( (curItem->BuyPrice == 0) && (itr->extended_cost == NULL )) // (Randdrick)
					continue;
				...
				//else if( curItem->extended_cost != NULL ) // en commentaire (Randdrick)
...
//	Item *item= NULL;
	...
	if(srcitem)
	{
		...
		{
			error=_player->GetItemInterface()->CanEquipItemInSlot(DstInv,  DstInv, srcitem->GetProto());
			if(error)
			{
...
void WorldSession::HandleReadItemOpcode(WorldPacket &recvPacket)
{
	...
	sLog.outDetail("Received CMSG_READ_ITEM %d", slot);
...
void WorldSession::HandleRepairItemOpcode(WorldPacket &recvPacket)
{
	...
	sLog.outDetail("Received CMSG_REPAIR_ITEM %d", itemguid);
}
...
void WorldSession::HandleBuyBankSlotOpcode(WorldPacket& recvPacket) 
{
    ...
    if(!bsp)
	{
		Log.Error("HandleBuyBankSlotOpcode","BankSlotPrice index not found (%u), verify your DBCs!!",slots+1);
		//return; // -> de 8 à 12 price = 999999999 en 309 (Branruz)
	}

	price = (bsp != NULL ) ? bsp->Price : 999999999;
...
void WorldSession::HandleAutoBankItemOpcode(WorldPacket &recvPacket)
{
	if(!_player->IsInWorld()) return;
	CHECK_PACKET_SIZE(recvPacket, 2);
	sLog.outDetail("WORLD: CMSG_AUTO_BANK_ITEM");
...
void WorldSession::HandleAutoStoreBankItemOpcode(WorldPacket &recvPacket)
{
	if(!_player->IsInWorld()) return;
	CHECK_PACKET_SIZE(recvPacket, 2);
	sLog.outDetail("WORLD: CMSG_AUTOSTORE_BANK_ITEM");
...
{
	sLog.outDetail("WORLD: CMSG_CANCEL_TEMP_ENCHANTMENT");
...
void WorldSession::HandleInsertGemOpcode(WorldPacket &recvPacket)
{
	...
	sLog.outDetail("WORLD: CMSG_SOCKET_GEMS");

	recvPacket >> itemguid ;

    if(!_player->IsInWorld()) return;
    
	GetPlayer()->ObjLock(); // Randdrick

	Item * TargetItem =_player->GetItemInterface()->GetItemByGUID(itemguid);
	if(!TargetItem)
	{
		GetPlayer()->ObjUnlock(); // Randdrick
		return;
	}
...
void WorldSession::HandleWrapItemOpcode( WorldPacket& recv_data )
{
	...
	sLog.outDetail("WORLD: CMSG_WRAP_ITEM");
	...
	if(src == dst || !(src->GetProto()->Class == ITEM_CLASS_CONSUMABLE && src->GetProto()->SubClass == ITEM_CLASS_GENERIC))
	{
	...
	switch( source_entry )
	{
	case 5042:         // Papier d'emballage à ruban rouge
		itemid = 5043; // Cadeau au ruban rouge
		break;

	case 5048:         // Papier d'emballage à ruban bleu
		itemid = 5044; // Cadeau au ruban bleu
		break;

	case 17303:         // Papier d'emballage à ruban bleu
		itemid = 17302; // Paquet cadeau à ruban bleu
		break;

	case 17304:         // Papier d'emballage à ruban vert
		itemid = 17305; // Paquet cadeau à ruban vert
		break;

	case 17307:         // Papier d'emballage à ruban violet
		itemid = 17308; // Paquet cadeau à ruban violet
		break;

	case 21830:         // Papier d'emballage vide
		itemid = 21831; // Cadeau enveloppé 
		break;
