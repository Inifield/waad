...
AuctionHouse::AuctionHouse(uint32 ID)
{
	...
	if(!dbc)
	{
		Log.Error("AuctionHouse::AuctionHouse","Erreur, Id non trouvé (%u)",ID);
		return;
	}
...
void AuctionHouse::AddAuction(Auction * auct)
{
	...
	Log.Debug("Maison des ventes", "%u: Ajouter l'enchère %u, temps d'expiration %u.", dbc->id, auct->Id, auct->ExpiryTime);
}
...
{
	Log.Debug("Maison des ventes", "%u: Supprimer l'enchère %u en raison de %u.", dbc->id, auct->Id, auct->DeletedReason);
