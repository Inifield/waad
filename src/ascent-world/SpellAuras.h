...
{
	AURASTATE_FLAG_DODGE_BLOCK			= 0x00000001, //1
    AURASTATE_FLAG_HEALTH20             = 0x00000002, //2
    AURASTATE_FLAG_BERSERK              = 0x00000004, //3
    AURASTATE_FLAG_FROZEN               = 0x00000008, //4
    AURASTATE_FLAG_JUDGEMENT            = 0x00000010, //5
    AURASTATE_FLAG_STUNNED				= 0x00000020, //6
    AURASTATE_FLAG_PARRY                = 0x00000040, //7
	AURASTATE_FLAG_UNK8					= 0x00000080, //8
	AURASTATE_FLAG_UNK9					= 0x00000100, //9
    AURASTATE_FLAG_LASTKILLWITHHONOR    = 0x00000200, //10
    AURASTATE_FLAG_CRITICAL             = 0x00000400, //11
	AURASTATE_FLAG_UNK12				= 0x00000800, //12
    AURASTATE_FLAG_HEALTH35             = 0x00001000, //13
    AURASTATE_FLAG_IMMOLATE             = 0x00002000, //14
    AURASTATE_FLAG_REJUVENATE           = 0x00004000, //15 //where do i use this ?
    AURASTATE_FLAG_POISON               = 0x00008000, //16
	AURASTATE_FLAG_ENRAGE               = 0x00010000, //17
	AURASTATE_FLAG_UNK18				= 0x00020000, //18
	AURASTATE_FLAG_UNK19				= 0x00040000, //19
	AURASTATE_FLAG_UNK20				= 0x00080000, //20
	AURASTATE_FLAG_UNK21				= 0x00100000, //21
	AURASTATE_FLAG_UNK22				= 0x00200000, //22
	AURASTATE_FLAG_HEALTHABOVE75		= 0x00400000, //23
};
...
struct ProcTriggerSpell
{
	...
	// Fix Warning compile linux Franck77 30 dec 08
	ProcTriggerSpell() : origId(0), spellId(0), caster(0), procChance(0), procFlags(0), procCharges(0), LastTrigger(0), index(0xFF), m_targetClass(false), deleted(false) { }
};
#else
struct ProcTriggerSpell
{
...
public:
    Aura(SpellEntry *proto, int32 duration, uint32 _type, Object* caster, Unit *target);
	~Aura();

	uint32 m_applycount[3];
	...
	void ApplyMechanicStates();
	void ApplyDispelStates();

	void HandleStackAdd();
	void HandleStackRemove();
	void UpdateDurations(uint32 _type);
	...
	uint32 m_stackcount;
	uint32 m_appliedstacks; //this is the real applied count, for inernal stuff
	bool m_wasremoved;
    ...
    void Remove();
    void Expire();
    void AddMod(uint32 t, int32 a,uint32 miscValue,uint32 index);

	ASCENT_INLINE uint8 GetFlags() const { return m_auraflags; }
	ASCENT_INLINE void SetFlags(uint8 flags) { m_auraflags = flags; }
	...
	{ 
		switch(_type)
		{
		 case INVOC_DURATION    : return(m_duration_invoc);
		 case INSTANT_DURATION	: return(m_duration_instant);
		 case COOLDOWN_DURATION	: return(m_duration_cooldown);

		 default:
			 Log.Error("","SetDuration Type inconnu %u <- Report this to devs",_type);
			 break;
		 
		}
		return(2000); 
	}
    void SetDuration(int32 duration,uint32 _type) 
	{ 
		switch(_type)
		{
	...
	ASCENT_INLINE void SetTimeLeft(int32 time, uint32 _type) //in ms
	{
		timeleft = getMSTime() + GetDuration(_type) - time;

		//modify the remove event
		if (GetDuration(_type) > 0)
		{
			if (!sEventMgr.HasEvent(this, EVENT_AURA_REMOVE))
				sEventMgr.AddEvent(this, &Aura::Remove, EVENT_AURA_REMOVE, GetTimeLeft(_type) + 500, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
			else
				sEventMgr.ModifyEventTimeLeft(this, EVENT_AURA_REMOVE, GetTimeLeft(_type) + 500);
		}
		UpdateDurations(_type);
	}

	int32 GetTimeLeft(uint32 _type)
	{
		if (GetDuration(_type) == -1) return -1;

		int32 n = getMSTime() - timeleft;

		if (n > GetDuration(_type)) return 0;

		return (GetDuration(_type) - n);
	}
	...
	void SpellAuraModRegen(bool apply=true);
...
//	void SpellAuraIncreaseHealingByInt(bool apply);
	...
	void SpellAuraModifyPhase(bool apply);
	...
	void EventPeriodicScript(uint32 amount);
	...
	void EventPeriodicTriggerSpellWithValue(SpellEntry* spellInfo, int32 v);
    ...
    /************************************************************************/
    /* Nourriture et Boisson (devrait etre géré niveau Unit) (Branruz)      */
    /************************************************************************/
    void EventFoodRegen(SpellEntry *SpellRegen,int32 BasePoint );
    void EventDrunkRegen(SpellEntry *SpellRegen,int32 TypePowerRegen,int32 BasePoint );
...
private:
	...
	Player * p_target;
	uint32 timeleft;
	int32 m_duration_invoc;    // in msecs
	int32 m_duration_instant;  // in msecs
	int32 m_duration_cooldown; // in msecs
//	bool m_positive;
