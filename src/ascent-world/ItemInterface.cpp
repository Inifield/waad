...
//-------------------------------------------------------------------//
Item *ItemInterface::SafeAddItem(uint32 ItemId, int32 ContainerSlot, int32 slot)
{
	Item *pItem;
...
//Description: Creates and adds a item that can be manipulated after		  
//-------------------------------------------------------------------//
AddItemResult ItemInterface::SafeAddItem(Item *pItem, int32 ContainerSlot, int32 slot)
{
...
//-------------------------------------------------------------------//
AddItemResult ItemInterface::m_AddItem(Item *item, int32 ContainerSlot, int32 slot)
{
	...
	if(item->GetProto())
	{
		...
		if(ContainerSlot == INVENTORY_SLOT_NOT_SET)
		{
			...
			if(!GetInventoryItem(slot)) //slot is free, add item.
			{
				...
				if (item->GetProto()->BagFamily & ITEM_TYPE_PVP) // Currency
...
//-------------------------------------------------------------------//
bool ItemInterface::IsBagSlot(int32 slot)
{
...
//-------------------------------------------------------------------//
Item *ItemInterface::SafeRemoveAndRetreiveItemFromSlot(int32 ContainerSlot, int32 slot, bool destroy)
{
	...
	if(ContainerSlot == INVENTORY_SLOT_NOT_SET)
	{
		...
		if(pItem->GetOwner() == m_pOwner)
		{
			...
			if ( slot < EQUIPMENT_SLOT_END )
			{
				...
				// Entry + Enchantment (seulement 2 en 332.11403)
				int VisibleBase = PLAYER_VISIBLE_ITEM_1_ENTRYID + (slot * PLAYER_VISIBLE_ITEM_LENGTH); 
...
{
	//int8 i = 0; // N'importe nawak, WTFCDM!! (Branruz)
    uint32 i = 0;

	//Log.Warning("SafeRemoveAndRetreiveItemByGuid","Scanning EQUIPMENT_SLOT");
	...
	//Log.Warning("SafeRemoveAndRetreiveItemByGuid","Scanning INVENTORY_SLOT");
	...
	//Log.Warning("SafeRemoveAndRetreiveItemByGuid","Scanning INVENTORY_KEYRING");
	...
	//Log.Warning("SafeRemoveAndRetreiveItemByGuid","Scanning INVENTORY_CURRENCY");
	...
	//Log.Warning("SafeRemoveAndRetreiveItemByGuid","Scanning INVENTORY_SLOT_BAG");
	...
	//Log.Warning("SafeRemoveAndRetreiveItemByGuid","Scanning BANK_SLOT_ITEM");
	...
	//Log.Warning("SafeRemoveAndRetreiveItemByGuid","Scanning BANK_SLOT_BAG");
...
//-------------------------------------------------------------------//
bool ItemInterface::SafeFullRemoveItemFromSlot(int32 ContainerSlot, int32 slot)
{
	...
	if(ContainerSlot == INVENTORY_SLOT_NOT_SET)
	{
		...
		if( pItem->GetOwner() == m_pOwner )
		{
			...
			{
				m_pOwner->ApplyItemMods(pItem, slot, false );
				// Seulement Entry+Enchantment (332.11403)
				int VisibleBase = PLAYER_VISIBLE_ITEM_1_ENTRYID + (slot * PLAYER_VISIBLE_ITEM_LENGTH);
				for (int i = VisibleBase; i < (VisibleBase + PLAYER_VISIBLE_ITEM_LENGTH); ++i)
				{
					m_pOwner->SetUInt32Value(i, 0);
...
//-------------------------------------------------------------------//
Item *ItemInterface::GetInventoryItem(int32 slot)
{
...
//-------------------------------------------------------------------//
Item *ItemInterface::GetInventoryItem(int32 ContainerSlot, int32 slot)
{
	if(ContainerSlot <= INVENTORY_SLOT_NOT_SET)
	{
		if(slot < 0 || slot > MAX_INVENTORY_SLOT)
...
AddItemResult ItemInterface::AddItemToFreeSlot(Item *item)
{
	...
	{
		switch(item->GetProto()->BagFamily)
		{
		  // case ITEM_TYPE_GLYPHES : break;

		  case ITEM_TYPE_KEYRING  :
          case ITEM_TYPE_KEYRING2 :
		  ...
		  default :
		    ...
		    break;
		} // End of switch(item->GetProto()->BagFamily
	} // End if( item->GetProto()->BagFamily )
		...
		if(proto->BagFamily)
		{
			switch(proto->BagFamily)
			{
			  ...
			  default :
			    ...
			    break;
			} // End of switch(proto->BagFamily)
		} // End of if(proto->BagFamily)
	}
...
//-------------------------------------------------------------------//
int8 ItemInterface::GetInternalBankSlotFromPlayer(int32 islot)
{
	switch(islot)
	{
...
//-------------------------------------------------------------------//
int8 ItemInterface::CanEquipItemInSlot(int32 DstInvSlot, int32 slot, ItemPrototype* proto, bool ignore_combat /* = false */, bool skip_2h_check /* = false */)
{
	...
	// Equipement du player
	if((slot < INVENTORY_SLOT_BAG_END && DstInvSlot == INVENTORY_SLOT_NOT_SET) || 
	   (slot >= BANK_SLOT_BAG_START && slot < BANK_SLOT_BAG_END && DstInvSlot == INVENTORY_SLOT_NOT_SET))
	{
		...
		{
			//Log.Warning("CanEquipItemInSlot","INV_ERR_CANT_CARRY_MORE_OF_THIS");
			return INV_ERR_CANT_CARRY_MORE_OF_THIS;
		}
		
		if( proto->AllowableRace && !(proto->AllowableRace & m_pOwner->getRaceMask()))
		{
			//Log.Warning("CanEquipItemInSlot","INV_ERR_YOU_CAN_NEVER_USE_THAT_ITEM");
		...
		{
			//Log.Warning("CanEquipItemInSlot","INV_ERR_YOU_CAN_NEVER_USE_THAT_ITEM2");
		...
		if(proto->RequiredFaction)
		{
		    ...
		    {
			    //Log.Warning("CanEquipItemInSlot","INV_ERR_ITEM_REPUTATION_NOT_ENOUGH");
		...
		{
		    //Log.Warning("CanEquipItemInSlot","INV_ERR_YOU_MUST_REACH_LEVEL_N");
		...
		if(proto->Class == ITEM_CLASS_ARMOR)
		{
            ...
            {
		        //Log.Warning("CanEquipItemInSlot","INV_ERR_NO_REQUIRED_ARMOR_PROFICIENCY");
				//Log.Warning("                  ","ArmorProficiency:%08X, SubClass:%08X (%u)",m_pOwner->GetArmorProficiency(),proto->SubClass,proto->SubClass);
		...
		else if(proto->Class == ITEM_CLASS_WEAPON)
		{
			...
			{
				//Log.Warning("CanEquipItemInSlot","INV_ERR_NO_REQUIRED_WEAPON_PROFICIENCY");
				return INV_ERR_NO_REQUIRED_PROFICIENCY;
			}
		}
		...
		if(proto->RequiredSkill)
		{
			...
			{
				//Log.Warning("CanEquipItemInSlot","INV_ERR_SKILL_ISNT_HIGH_ENOUGH");
		...
		if(proto->RequiredSkillSubRank)
		{
			...
			{
				//Log.Warning("CanEquipItemInSlot","INV_ERR_NO_REQUIRED_RequiredSkillSubRank_PROFICIENCY");
				return INV_ERR_NO_REQUIRED_PROFICIENCY;
			}
		}
		...
		{
			//Log.Warning("CanEquipItemInSlot","INV_ERR_YOU_ARE_DEAD");
			...
			if(type == INVTYPE_WEAPON || type == INVTYPE_WEAPONOFFHAND)
			{
				...
				if(mainweapon) //item exists
				{
					if(mainweapon->GetProto())
					{
						...
						// A cet endroit , ca sert a rien, à eclaircir, relatif au DK (Branruz)
						/*
						if(m_pOwner->HasSpell( __Titan_s_Grip_Rank_1 )) //Titan's Grip Spell 46917
								return 0;
							else
								return INV_ERR_ITEM_DOESNT_GO_TO_SLOT;
						*/	
			...
			else if(type == INVTYPE_SHIELD || type == INVTYPE_HOLDABLE)
			{
				...
				if(mainweapon) //item exists
				{
					if(mainweapon->GetProto())
					{
						if(mainweapon->GetProto()->InventoryType != INVTYPE_2HWEAPON)
						{
							//if(m_pOwner->HasSpell( __Titan_s_Grip_Rank_1 )) //Titan's Grip Spell
								return 0;
						}
						else 
						{
							if(!skip_2h_check)
							{
								//sLog.outDetail("ITEM: Ne peut pas equiper un bouclier avec une arme a 2 mains (Id:%u - Inv:%u)",mainweapon->GetProto()->ItemId,mainweapon->GetProto()->InventoryType);
								return INV_ERR_CANT_EQUIP_WITH_TWOHANDED;
							}
							else
								return 0;
	...
	case BANK_SLOT_BAG_7:
		{
			...
			if(!GetInventoryItem(INVENTORY_SLOT_NOT_SET,slot))
			{
				...
				slots =(uint8) (bytes >> 16); // Ok 67 à 73
	...
	case INVENTORY_KEYRING_32:
		{
			if((proto->BagFamily == ITEM_TYPE_KEYRING) || (proto->BagFamily == ITEM_TYPE_KEYRING2) )
			{
					return 0;
			}
			else
	...
	//case INVENTORY_CURRENCY_0: // A voir (Branruz)
    case INVENTORY_CURRENCY_1: // 119
	...
	case INVENTORY_CURRENCY_32: // 150
		{
		 if( proto->BagFamily & ITEM_TYPE_PVP )
			return 0;
		 else
			return INV_ERR_ITEM_DOESNT_GO_INTO_BAG;
		}
	...
	default:
		Log.Warning("CanEquipItemInSlot","%s: Slot %u non gere!",m_pOwner->GetName(),slot);
		return 0;
	}
}
...
//-------------------------------------------------------------------//
void ItemInterface::SwapItemSlots(int32 srcslot, int32 dstslot)
{
	//Log.Warning("SwapItemSlots","%s range ses sacs",GetOwner()->GetName());
	...
	//Log.Warning("SwapItemSlots","%s inverse emplacement %u avec %u" ,GetOwner()->GetName(), srcslot , dstslot );
	...
	//---------- Application des stats de l'item -------------------------
	if( srcslot < INVENTORY_SLOT_BAG_END ) 
	{
		if( m_pItems[(int)srcslot] != NULL )
		{
			//Log.Warning("SwapItemSlots","%s prend les stats du src slot %u" ,GetOwner()->GetName(), srcslot);
			GetOwner()->ApplyItemMods( m_pItems[(int)srcslot], srcslot, false );
		}
	}
	...
	if( dstslot < INVENTORY_SLOT_BAG_END ) 
	{
		...
		{
			//Log.Warning("SwapItemSlots","%s prend les stats du dst slot %u" ,GetOwner()->GetName(), dstslot );
			GetOwner()->ApplyItemMods( m_pItems[(int)dstslot], dstslot, false );
		}
	}
    //---------------------------------------------------------------------
   ...
   m_pItems[(int)srcslot] = DstItem;
	...
	if( srcslot < INVENTORY_SLOT_BAG_END )	// source item is equiped
	{
		if( m_pItems[(int)srcslot] ) // dstitem goes into here.
		{
			...
			{
				GetOwner()->GetCurrentSpell()->cancel(SPELL_FAILED_INTERRUPTED);
			}
		} 
		else 
		{
			...
			{
				int VisibleBase = PLAYER_VISIBLE_ITEM_1_ENTRYID + (srcslot * PLAYER_VISIBLE_ITEM_LENGTH);
				for(int i=0;i < PLAYER_VISIBLE_ITEM_LENGTH;i++)
				{
				 m_pOwner->SetUInt32Value( VisibleBase+i, 0 );
				}
	...
	// Maj des stats player avec l'item
	if( srcslot < INVENTORY_SLOT_BAG_END ) 
 	{
		if( m_pItems[(int)srcslot] != NULL )		
	...
	if( result )
	{
		do
		{
			...
			if( proto != NULL )
			{
				...
				if(item != NULL && proto->BagFamily & ITEM_TYPE_PVP)
				{
					if(containerslot != ((uint32)-1) || (containerslot == ((uint8)-1) && slot < INVENTORY_CURRENCY_1))
			...
			if( m_pItems[i]->GetProto()->BagFamily == item->GetProto()->BagFamily )
			{
				if( m_pItems[i]->IsContainer() )
				{
					...
					if( result )
					{
						...
						/* Pourquoi interrompre le spell si on le change de slot
...
SlotResult ItemInterface::FindFreeInventorySlot(ItemPrototype *proto)
{
	...
	if( proto != NULL )
	{
	   ...
	   {
		switch(proto->BagFamily)
		{ // Ici on tri le numero du slot qui recevra l'item

		  // case ITEM_TYPE_GLYPHES : break;

		 case ITEM_TYPE_KEYRING  : // des clefs ?
		 case ITEM_TYPE_KEYRING2 :
			    for(uint32 i = INVENTORY_KEYRING_START; i < INVENTORY_KEYRING_END; i++ )
				{
					if( m_pItems[i] == NULL )
...
bool ItemInterface::HasGemEquipped( uint32 GemID , int32 IgnoreSlot /* = -1 */ )
{
	Item * itm;
	EnchantEntry * ench;
	for( int32 x = EQUIPMENT_SLOT_START; x <= EQUIPMENT_SLOT_END; ++x )
	{
		if( x == IgnoreSlot )
			continue;
		if( m_pItems[x] != NULL )
		{
			...
			if( !itm->HasEnchantments() )
				continue;
			for( uint32 z = EQUIPMENT_SLOT_SHOULDERS ; z <= EQUIPMENT_SLOT_WAIST ; z ++ )
			{
