...
void WorldSession::HandleAttackSwingOpcode( WorldPacket & recv_data )
{
	...
	if(!GetPlayer()->CanAct()) return; // if (!IsFeared() && !IsStunned() && !IsPacified ()&& !IsConfused() && isAlive()) return false;
...
//	printf("Got ATTACK SWING: %08X %08X\n", GUID_HIPART(guid), GUID_LOPART(guid));
	...
	{
		HandleAttackStopOpcode(recv_data); // Test (Branruz)
		//_player->EventAttackStop(); // _player->m_attacking = false;      // Test (Branruz), regler par timer (Branruz)
		return;
	}
	...
	if(pEnemy->IsPlayer() && isHostile(_player, pEnemy))
	{
		...
		if( (static_cast< Player* >( pEnemy )->GetTeam() != _player->GetTeam()) && (!_player->IsPvPFlagged()) )
		{
			_player->SetPvPFlag();
		}
	}
