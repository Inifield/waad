...
void Group::Disband()
{
	...
	{
		m_isqueued=false;
		WorldPacket * data = sChatHandler.FillSystemMessageData("Une modification a ete apportee a votre groupe. Suppression de la file d attente Arena.");
		SendPacketToAll(data);
		delete data;
...
void Group::UpdateOutOfRangePlayer(Player* pPlayer, uint32 Flags, bool Distribute, WorldPacket * Packet)
{
	...
	if(!data)  data = new WorldPacket(SMSG_PARTY_MEMBER_STATS, 500);
	...
	*data << Flags;
	...
	{
		if(pPlayer->IsPvPFlagged()) 			member_flags |= 0x02;
		if(pPlayer->getDeathState() == CORPSE)  member_flags |= 0x08;
		else if(pPlayer->isDead())              member_flags |= 0x10;

		*data << uint8(member_flags) << uint8(0);
	}

	if(Flags & GROUP_UPDATE_FLAG_HEALTH)     *data << pPlayer->GetUInt32Value(UNIT_FIELD_HEALTH);
	if(Flags & GROUP_UPDATE_FLAG_MAXHEALTH)  *data << pPlayer->GetUInt32Value(UNIT_FIELD_MAXHEALTH);
	if(Flags & GROUP_UPDATE_FLAG_POWER_TYPE) *data << uint8(pPlayer->GetPowerType());
	if(Flags & GROUP_UPDATE_FLAG_POWER)      *data << uint16(pPlayer->GetUInt32Value(UNIT_FIELD_POWER1 + pPlayer->GetPowerType()));
	if(Flags & GROUP_UPDATE_FLAG_MAXPOWER)   *data << uint16(pPlayer->GetUInt32Value(UNIT_FIELD_MAXPOWER1 + pPlayer->GetPowerType()));
	if(Flags & GROUP_UPDATE_FLAG_LEVEL)      *data << uint16(pPlayer->getLevel());
	if(Flags & GROUP_UPDATE_FLAG_ZONEID)     *data << uint16(pPlayer->GetAreaID());
