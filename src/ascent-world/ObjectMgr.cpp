...
const char * NormalTalkMessage = "Que pourrais je bien vous apprendre, $N?";
...
void ObjectMgr::SaveGMTicket(uint64 guid, QueryBuffer * buf)
{
	...
	ss2 << "INSERT INTO gm_tickets (guid, name, level, map, posX, posY, posZ, message, timestamp) VALUES(";
	ss2 << ticket->guid << ", '";
...
uint32 ObjectMgr::GenerateLowGuid(uint32 guidhigh)
{
	...
	else if(guidhigh==HIGHGUID_TYPE_CONTAINER)
	{
		m_guidGenMutex.Acquire();
	...
	Log.Error("ObjectMgr","[GenerateLowGuid] GuidHigh non traité (%X)",guidhigh);
	return (NULL);
}
...
void ObjectMgr::ProcessGameobjectQuests()
{
	...
	ClearProgressBar();*/
	QueryResult * result3  = WorldDatabase.Query("SELECT * FROM gameobject_quest_item_binding");
...
void ObjectMgr::AddGuild(Guild *pGuild)
{
	...
	if(pGuild == NULL) 
	{
		Log.Error("ObjectMgr"," [AddGuild] pGuild est NULL");
		return;
	}
...
void ObjectMgr::AddGMTicket(GM_Ticket *ticket,bool startup)
{
	...
	if(ticket == NULL)
	{
		Log.Error("ObjectMgr","[AddGMTicket] Ticket GM = NULL");
		return;
	}
...
{
	SpellEntry* sp;
	QueryResult * result = WorldDatabase.Query("SELECT * FROM spell_fixes");
	if(result)
	{
...
void ObjectMgr::LoadSpellEffectsOverride()
{
	...
	if(result)
	{
		do
		{
			...
			uint32 seo_interruptFlags = f[1].GetUInt32();
			uint32 seo_procFlags = f[2].GetUInt32();
			uint32 seo_procChance = f[3].GetUInt32();
			uint32 seo_procCharges = f[4].GetUInt32();
			uint32 seo_EffectId = f[5].GetUInt32();
			uint32 seo_Disable = f[6].GetUInt32();
			uint32 seo_Effect = f[7].GetUInt32();
			uint32 seo_BasePoints = f[8].GetUInt32();
			uint32 seo_ApplyAuraName = f[9].GetUInt32();
			uint32 seo_ApplyAuraName_1 = f[10].GetUInt32();
			uint32 seo_ApplyAuraName_2 = f[11].GetUInt32();
			uint32 seo_SpellGroupRelation = f[12].GetUInt32();
			uint32 seo_MiscValue = f[13].GetUInt32();
			uint32 seo_TriggerSpell = f[14].GetUInt32();
			uint32 seo_TriggerSpell_1 = f[15].GetUInt32();
			uint32 seo_TriggerSpell_2 = f[16].GetUInt32();
			uint32 seo_ImplicitTargetA = f[17].GetUInt32();
			uint32 seo_ImplicitTargetB = f[18].GetUInt32();
			...
			if( seo_SpellId )
			{
				...
				if( sp != NULL )
				{
					if( seo_procFlags != 0xFFFFFFFF)  sp->procFlags    = seo_procFlags;
					if( seo_interruptFlags )      sp->interruptFlags           = seo_interruptFlags;
					if( seo_procChance )      sp->procChance           = seo_procChance;
					if( seo_procCharges)      sp->procCharges = seo_procCharges;
					if( seo_Disable )         sp->Effect[seo_EffectId] = 0;
					if( seo_Effect )          sp->Effect[seo_EffectId]              = seo_Effect;
					if( seo_BasePoints )      sp->EffectBasePoints[seo_EffectId]    = seo_BasePoints;
					if( seo_ApplyAuraName )   sp->EffectApplyAuraName[seo_EffectId] = seo_ApplyAuraName;
					if( seo_ApplyAuraName_1 ) sp->EffectApplyAuraName[1]            = seo_ApplyAuraName_1;
					if( seo_ApplyAuraName_2 ) sp->EffectApplyAuraName[2]            = seo_ApplyAuraName_2;
					if( seo_MiscValue )       sp->EffectMiscValue[seo_EffectId]     = seo_MiscValue;
					if( seo_TriggerSpell )    sp->EffectTriggerSpell[seo_EffectId]  = seo_TriggerSpell;
					if( seo_TriggerSpell_1 )  sp->EffectTriggerSpell[1]             = seo_TriggerSpell_1;
					if( seo_TriggerSpell_2 )  sp->EffectTriggerSpell[2]             = seo_TriggerSpell_2;
					if( seo_ImplicitTargetA ) sp->EffectImplicitTargetA[seo_EffectId] = seo_ImplicitTargetA;
					if( seo_ImplicitTargetB ) sp->EffectImplicitTargetB[seo_EffectId] = seo_ImplicitTargetB;
				}
			}
...
{
	Corpse * pCorpse = new Corpse(NULL, NULL);
	pCorpse = objmgr.GetCorpse((uint32)Guid);
...
{
	// First=LowGuid, Second = Corpse*
	// C'est bien mieux ecrit comme ca.
...
void GossipMenu::AddItem(uint32 IndexPOI)
{
	GossipMenuAddPOI GossipPOIItem;
	GossipPOIItem.m_IndexPOI = IndexPOI;

	POIMenu.push_back(GossipPOIItem);
}
...
void GossipMenu::SendPointOfInterest( uint32 poi_id )
{
	PointOfInterest const* poi = objmgr.GetPointOfInterest(poi_id);
	if(!poi)
	{
		sLog.outError("La requete envoyee n/'existe pas. Le POI (Id: %u) est ignore.",poi_id);
		return;
	}
...
void ObjectMgr::CreateGossipMenuForPlayer(GossipMenu** Location, uint64 Guid, uint32 TextID, Player* Plr)
{
	...
	if(Menu == NULL)
	{
		Log.Error("ObjectMgr","[CreateGossipMenuForPlayer] Menu == NULL (%s)",Plr->GetName());
		return;
	}
		...
		else
		{
			do
			{
					...
					if( ts.pCastSpell )
					{
						for(int k=0;k<3;k++)
						{
							if(ts.pCastSpell->Effect[k]==SPELL_EFFECT_LEARN_SPELL)
							{
								...
								{
									Log.Warning("Trainers", "Trainer %u : spell.dbc => CastSpell = %u n'exsite plus", entry, CastSpellID);
									abrt=true;
								}
								break;
							}
				...
				if( ts.pCastSpell == NULL && ts.pLearnSpell == NULL )
				{
					...
					Log.Warning("ObjectMgr", "Trainer %u n'existe plus (%u/%u).", entry, CastSpellID, LearnSpellID);
...
void ObjectMgr::GenerateLevelUpInfo()
{
	...
	uint32 BaseLevel,MaxLevel;

	for(uint32 Class = WARRIOR; Class <= DRUID; ++Class)
	{
		// These are empty.
		if(Class == UNK_CLASS2) continue;
		...
		for(uint32 Race = RACE_HUMAN; Race <= RACE_DRAENEI; ++Race )
		{
			...
			if(PCI == 0) continue;   // Class not valid for this race.

			// Generate each level's information
			MaxLevel = sWorld.m_levelCap+1;
			...
			*lvl = lastlvl;
			...
			// Notes Randdrick :
			// Le BaseLevel doit être modifié pour correspondre au niveau 1 dans les autres classes
			// L'ajustement des stats est obligatoire. Le décalage semble être provoqué par le
			// fait des bonus lié à équipement de départ du DK.
			// Le MaxLevel doit être réajusté à la valeur du m_levelCap. Comme tout autre
			// classe, Un DK peut atteindre le niveau 80. Comme il débute au niveau 55, il ne peut
			// évoluer que sur 25 niveaux ( 80 - 55 )
			if(Class == DEATHKNIGHT)
			{
				BaseLevel = 55;
				lastlvl.Stat[0] = PCI->strength-2;
				lastlvl.Stat[1] = PCI->ability-2;
				lastlvl.Stat[2] = PCI->stamina-2;
				/*lastlvl.Stat[3] = PCI->intellect-2;*/
				lastlvl.Stat[4] = PCI->spirit-1;
			...
			uint32 val;
			for(uint32 Level = BaseLevel; Level <= MaxLevel; ++Level)
			{
				lvl = new LevelInfo;
				...
				switch(Class)
				{
                 case DEATHKNIGHT:
					if(Level > 55)
					{
						if(Level <= 60) TotalHealthGain = (Level-54)+92;
					    else 		    TotalHealthGain = (Level-54)+206;
					}
					break;
				 ...
				 case HUNTER:
					...
					if(Level<11)        TotalManaGain = 16;
					else if(Level<27)   TotalManaGain = Level+18;
					else                TotalManaGain = Level+21;
				 ...
				 default: // Randdrick
					 TotalHealthGain = 0;
				     TotalManaGain = 0; 
					 Log.Warning("GenerateLevelUpInfo","Classe Inconnu: %u",Class);					
					 break;
				}
				...
				else nextLvlXP = 500000; // ne peut pas arriver (Branruz)
...
LevelInfo* ObjectMgr::GetLevelInfo(uint32 Race, uint32 Class, uint32 Level)
{
	...
	for(; itr != mLevelInfo.end(); ++itr )
	{
		if( itr->first.first == Race && itr->first.second == Class)
		{
			...
			if( Level > sWorld.m_levelCap ) // too far.
			...
			{
				Log.Error("[ObjectMgr]","GetLevelInfo introuvable Level:%u, Class:%u, Race:%u",Level,Class,Race);
			    return(NULL);
			}
...
void ObjectMgr::LoadPetSpellCooldowns()
{
	...
	sprintf(_bufftemp_,"%s/CreatureSpellData.dbc",DBCPath);
	if( !dbc.open( _bufftemp_ )) 
	{
		Log.Error( "DBC:", "Cannot find file %s",_bufftemp_ );
		return;
	}
		...
		for(uint32 j = 0; j < 3; ++j)
		{
			...
			if(SpellId)
			{
				...
				else
				{
					...
					//ASSERT(Cooldown == SP2);
					if(Cooldown != SP2)
						Log.Error("ObjectMgr","[LoadPetSpellCooldowns] CoolDown inconnu, SpellId %u",SpellId);
...
//	int i = 0;
	...
	if(!result)
	{
		Log.Notice("ObjectMgr", "No spell overrides loaded.");
		return;
	}
...
void Charter::AddSignature(uint32 PlayerGuid)
{
	...
	if(i >= Slots)
	{
	 Log.Error("ObjectMgr","[Charter::AddSignature] Nombre de slots depassé (Player Guid %u)",PlayerGuid);
	}
}
...
// (franck) petlevelup
	...
	for	(uint32	i =	0; i < dbcCreatureFamily.GetNumRows(); ++i)
	{
		...
		// Si pettalenttype -1 pas un pet de chasseur :)
		...
		for(uint32 j = 0; j < dbcSkillLineSpell.GetNumRows(); ++j)
		{
			...
			// Si raceMask #4 ou classMask #5 <= mais pourquoi ?
			if(!sk || sk->raceMask != 0 || sk->classMask != 0)
				continue;

			sp = dbcSpell.LookupEntry(sk->spell);

			// si pas spell de familier de chasseur 
			...
			switch(sk->skilline)
			{
				case SKILL_PET_IMP:
				{
					PLU.creatureFamily = FAMILY_FAKE_IMP;
					PLU.spellLevel = sp->spellLevel;
					PLU.spellId = sk->spell;
					PetLevelUpList.push_back(PLU);
					//mPetLevelupSpellMap[FAMILY_FAKE_IMP][sp->spellLevel] = sk->spell;
				}break;
				case SKILL_PET_VOIDWALKER:
				{
					PLU.creatureFamily = FAMILY_FAKE_VOIDWALKER;
					PLU.spellLevel = sp->spellLevel;
					PLU.spellId = sk->spell;
					PetLevelUpList.push_back(PLU);
				...
				{
					PLU.creatureFamily = FAMILY_FAKE_SUCCUBUS;
					PLU.spellLevel = sp->spellLevel;
					PLU.spellId = sk->spell;
					PetLevelUpList.push_back(PLU);
				...
				{
					PLU.creatureFamily = FAMILY_FAKE_FELHUNTER;
					PLU.spellLevel = sp->spellLevel;
					PLU.spellId = sk->spell;
					PetLevelUpList.push_back(PLU);
				...
				{
					PLU.creatureFamily = FAMILY_FAKE_FELGUARD;
					PLU.spellLevel = sp->spellLevel;
					PLU.spellId = sk->spell;
					PetLevelUpList.push_back(PLU);
				...
				{
					PLU.creatureFamily = creatureFamily->ID;
					PLU.spellLevel = sp->spellLevel;
					PLU.spellId = sk->spell;
					PetLevelUpList.push_back(PLU);
...
uint32 ObjectMgr::WaadPetLevelUp (uint32 Family, uint32 Level)
{

	vector<PetLevelUp>::iterator itr;
	
	for (itr = PetLevelUpList.begin (); itr != PetLevelUpList.end (); ++itr  )
	{
		PetLevelUp p;
		p = *itr;

		if ( (p.creatureFamily == Family) && (p.spellLevel == Level) )
		{
			return p.spellId;
		}
...
void ObjectMgr::LoadPointsOfInterest() // Point d'intérêts et Quêtes - Adapté de Mangos pour Waad par Randdrick
{
    mPointsOfInterest.clear();                              // need for reload case

	uint32 count = 0;

    QueryResult *result = WorldDatabase.Query("SELECT * FROM points_of_interest");

    if(!result)
    {
        Log.Warning("ObjectMgr","Chargement de 0 definitions de POI. La table `points_of_interest` est vide.");
        return;
    }
    ...
    do
    {
		...
		/*if (!IsValidMapCoord)
        {
            Log.Error("ObjectMgr", "La table `points_of_interest` (Entry: %u) ont les coordonnees (X: %f Y: %f) invalides. Elles sont ignorees.",point_id,POI.x,POI.y);
            continue;
        }*/
    ...
    Log.Notice("ObjectMgr", "Loaded %u Points of Interest definitions", count);
}

void ObjectMgr::LoadQuestPOI() // Point d'intérêts et Quêtes - Adapté de Mangos pour Waad par Randdrick
{
    ...
    if(!result)
    {
        Log.Warning("ObjectMgr","Chargement de 0 definitions de POI de quetes. La table `quest_poi` est vide.");
        return;
    }
	...
	Log.Notice("ObjectMgr", "Chargement de %u definitions de POI de quetes", result->GetRowCount());
    delete result;
