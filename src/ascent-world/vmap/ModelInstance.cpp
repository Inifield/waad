...
namespace VMAP
{
	...
	bool ModelInstance::intersectRay(const G3D::Ray& pRay, float& pMaxDist, bool pStopAtFirstHit) const
	{
		...
		{
#ifdef VMAP_DEBUG
			sLog.outDebug("ModelInstance", "<object not loaded>");
#endif
			return false;
		}
		...
		{
#ifdef VMAP_DEBUG
			sLog.outDebug("ModelInstance", "Ray does not hit '%s'", name.c_str());
#endif
			return false;
		}
	...
	bool ModelSpawn::readFromFile(FILE *rf, ModelSpawn &spawn)
	{
		...
		{
			if (ferror(rf))
                sLog.outError("Error reading ModelSpawn!");
			return false;
		}
		...
		{
            sLog.outError("Error reading ModelSpawn, file name too long!");
			return false;
		}
		check = (uint32)fread(nameBuff, sizeof(char), nameLen, rf);
		if (check != nameLen)
		{
            sLog.outError("Error reading name string of ModelSpawn!");
			return false;
		}
