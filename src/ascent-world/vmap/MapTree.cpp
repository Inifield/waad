...
namespace VMAP
{
	...
	class AreaInfoCallback
	{
		public:
			...
			{
#ifdef VMAP_DEBUG
				sLog.outDebug("MapTree", "trying to intersect '%s'", prims[entry].name.c_str());
#endif
	...
	*/
	...
	{
		sLog.outDebug("MapTree", "Initializing StaticMapTree '%s'", fname.c_str());
		bool success = true;
		...
		else
		{
			...
			ModelSpawn spawn;
#ifdef VMAP_DEBUG
			sLog.outDebug("MapTree", "Map isTiled: %u", static_cast<uint32>(iIsTiled));
#endif
		...
		if (tf)
		{
			...
			for (uint32 i=0; i<numSpawns && result; ++i)
			{
				...
				if (result)
				{
					...
					WorldModel *model = vm->acquireModelInstance(iBasePath, spawn.name);
					if (!model)
                        sLog.outError("StaticMapTree::LoadMapTile() could not acquire WorldModel pointer for '%s'!", spawn.name.c_str());
					...
					if (!iLoadedSpawns.count(referencedVal))
					{
						...
						{
							sLog.outDebug("MapTree", "invalid tree element! (%u/%u)", referencedVal, iNTreeValues);
							continue;
						}
#endif
					...
					else
					{
						...
						if (iTreeValues[referencedVal].ID != spawn.ID)
							sLog.outDebug("MapTree", "Error: trying to load wrong spawn in node!");
						else if (iTreeValues[referencedVal].name != spawn.name)
							sLog.outDebug("MapTree", "Error: name mismatch on GUID=%u", spawn.ID);
#endif
					}
				}
			}
