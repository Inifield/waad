...
_logoutTime(0), permissions(NULL), _loggingOut(false), instanceId(0)
{
	_player = NULL;
...
WorldSession::~WorldSession()
{
	...
	if(permissions)
	{
		Log.Notice("GMSecurityMgr", "Suppression des permissions %s.", sGMSecurityMgr.Serialize(permissions).c_str());
		delete permissions;
	}
...
int WorldSession::Update(uint32 InstanceID)
{
	...
	if(!_socket)
	{
		...
		if(_player && _player->m_beingPushed) // check timeout
		{
			...
			{
				sLog.outDebug("WorldSession","Removing InQueue player due to socket timeout.");
				LogoutPlayer(true);
				bDeleted = true;
				return 1;
			}
	...
	while ((packet = _recvQueue.Pop()))
	{
		...
		if(!packet)
		{
			Log.Error("WorldSession::Update","Fatal: Packet Error (NULL)");
			errorpacket = true;
		}
		else if(packet->GetOpcode() >= NUM_MSG_TYPES)
		{
			sLog.outError("[Session] Received out of range packet with opcode 0x%04X", packet->GetOpcode());
			errorpacket = true;
		}
		else
		{
			...
			if(Handler->status == STATUS_LOGGEDIN && !_player && Handler->handler != 0)
			{
				if(!GetPlayer())
				{
				 sLog.outError("[Session] %s [0x%04X]\n[Session] Reception non prise en compte, Joueur non present!",
					LookupName(packet->GetOpcode(), g_worldOpcodeNames), packet->GetOpcode());
				 errorpacket = true;
				}
				else
				{
					sLog.outError("[Session] %s [0x%04X]\n[Session] Reception non prise en compte (%s)",GetPlayer()->GetName(),
					                 LookupName(packet->GetOpcode(), g_worldOpcodeNames), packet->GetOpcode());
					errorpacket = true;
				}
			}
			else
			{
				...
				{
					sLog.outError("[Session] Received unhandled packet with opcode %s (0x%04X)",
						LookupName(packet->GetOpcode(), g_worldOpcodeNames), packet->GetOpcode());
					errorpacket = true;
				}
				else
...
void WorldSession::LogoutPlayer(bool Save)
{
	...
	if( _player != NULL )
	{
		...
		//remove guardians!
		for (std::set<Creature*>::iterator itr=_player->m_summonedCreatures.begin(); itr!=_player->m_summonedCreatures.end(); ++itr)
			(*itr)->SafeDelete();
		_player->m_summonedCreatures.clear();
		...
		for (std::multimap<uint32, Aura*>::iterator itr=_player->m_aurascast.begin(); itr!=_player->m_aurascast.end(); ++itr)
		{
			if (itr->second->m_spellProto->UniqueTargetBased & 2) //1 target at a time
				sEventMgr.AddEvent(itr->second, &Aura::Remove, EVENT_AURA_REMOVE, 1000, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT | EVENT_FLAG_DELETES_OBJECT);
		}
		...
		if( _player->m_currentSpell != NULL )
			_player->m_currentSpell->cancel(SPELL_FAILED_INTERRUPTED); // "Interrompu"
...
{
	permissions = sGMSecurityMgr.Create(securitystring);
	sLog.outDebug("Loaded permissions for %u : [%s]", this->GetAccountId(), sGMSecurityMgr.Serialize(permissions).c_str());
}
...
void WorldSession::SetSecurity(std::string securitystring)
{
	...
	// update db
	CharacterDatabase.Execute("UPDATE accounts SET gm=\"%s\" WHERE acct=%u", CharacterDatabase.EscapeString(sGMSecurityMgr.Serialize(permissions, true)).c_str(), _accountId);
}

bool WorldSession::CanUseCommand(char cmdstr)
{
	return permissions && permissions->CanUse(cmdstr);
}
...
void WorldSession::InitPacketHandlerTable()
{
	...
	// Player Interaction
	WorldPacketHandlers[CMSG_TIME_SYNC_RESP].handler						= &WorldSession::HandleTimeSynchroOpcode;  // Ajout (Branruz)
	...
	WorldPacketHandlers[CMSG_SEARCH_LFG_LEAVE].handler						= &WorldSession::HandleDisableAutoAddMembers;
    ...
    // Achievement (HearthStone)
	WorldPacketHandlers[CMSG_QUERY_INSPECT_ACHIEVEMENTS].handler 			= &WorldSession::HandleAchievementInspect;

    WorldPacketHandlers[CMSG_REMOVE_GLYPH].handler   						= &WorldSession::HandleGlyphRemoveOpcode;
	WorldPacketHandlers[CMSG_SET_GLYPH].handler      						= &WorldSession::HandleGlyphSetOpcode;
	WorldPacketHandlers[CMSG_SET_GLYPH_SLOT].handler 						= &WorldSession::HandleGlyphSetSlotOpcode;
...
void WorldSession::SendChatPacket(WorldPacket * data, uint32 langpos, int32 lang, WorldSession * originator)
{
	...
	{
		if(sGMSecurityMgr.UseUniversalLang(permissions) || (originator && sGMSecurityMgr.UseUniversalLang(originator->permissions)))
...
void WorldSession::Handle38C(WorldPacket & recv_data)
{
	uint32 v;
	recv_data >> v;
	WorldPacket data(SMSG_REALM_SPLIT, 17); // 0x038B
	data << v << uint32(0);
	data << "01/01/01";
	SendPacket(&data);
}
...
void WorldSession::HandleAchievementInspect(WorldPacket &recv_data)
{
	...
	if( pUnit && pUnit->IsPlayer() && ((Player *)pUnit)->GetAchievementInterface()->HasAchievements() )
	{
	 SendPacket( ((Player *)pUnit)->GetAchievementInterface()->BuildAchievementData(true) );
	}
}
...
// CMSG_REMOVE_GLYPH
void WorldSession::HandleGlyphRemoveOpcode(WorldPacket & recv_data)
{
	Log.Warning("CMSG_SET_GLYPH","Size recu: %u, report this to WaadDev",recv_data.size());
/*
	uint32 slot;
	recv_data >> slot;

	if (slot >= 6 || GetPlayer() == NULL || GetPlayer()->GetUInt32Value(PLAYER_FIELD_GLYPHS_1 + slot) == 0)
		return;

	GlyphPropertyEntry* glyph = dbcGlyphProperties.LookupEntry(GetPlayer()->GetUInt32Value(PLAYER_FIELD_GLYPHS_1 + slot));

	if (glyph == NULL)
		return;

	GetPlayer()->RemoveAura(glyph->SpellId);
	GetPlayer()->SetUInt32Value(PLAYER_FIELD_GLYPHS_1 + slot, 0);
	*/
}

// CMSG_SET_GLYPH
void WorldSession::HandleGlyphSetOpcode(WorldPacket & recv_data)
{
	Log.Warning("CMSG_SET_GLYPH","Size recu: %u, report this to WaadDev",recv_data.size());
...
// CMSG_SET_GLYPH_SLOT
void WorldSession::HandleGlyphSetSlotOpcode(WorldPacket & recv_data)
{
	Log.Warning("CMSG_SET_GLYPH_SLOT","Size recu: %u, report this to WaadDev",recv_data.size());

}

// CMSG_GAMEOBJ_REPORT_USE
// 898B00E3 E80210F1 GameObject Guid
