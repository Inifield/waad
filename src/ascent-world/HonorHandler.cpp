...
void HonorHandler::AddHonorPointsToPlayer(Player *pPlayer, uint32 uAmount)
{
	...
	pPlayer->HandleProc(PROC_ON_XP_OR_HONOR_KILL, pPlayer, NULL);
	...
	if( Points > 0 )
	{
		...
		else
		{
			...
			for(set<Player*>::iterator itr = contributors.begin(); itr != contributors.end(); itr++)
			{
				...
				int32 contributorpts = (int32)((float)Points / (float)contributors.size());
...
{
 uint32  PlayerKillCount = ((((uint32)pPlayer->m_killsYesterday)&0x0000FFFF) << 16) | ((uint16)pPlayer->m_killsToday); 
 pPlayer->SetUInt32Value(PLAYER_FIELD_KILLS,PlayerKillCount);  // Type: TWO_SHORT, 2*uint16 (Branruz)
 pPlayer->SetUInt32Value(PLAYER_FIELD_TODAY_CONTRIBUTION, pPlayer->m_honorToday);       
 pPlayer->SetUInt32Value(PLAYER_FIELD_YESTERDAY_CONTRIBUTION, pPlayer->m_honorYesterday); // Etrange ce *10 ??, m_killsYesterday | ( (pPlayer->m_honorYesterday * 10) << 16));
 ...
 pPlayer->SetUInt32Value(PLAYER_FIELD_ARENA_CURRENCY, pPlayer->m_arenaPoints);
	...
	/* DEBUG: Test pour alignement (Branruz)
  uint32  PlayerKillCount = 0x00020001; 
 pPlayer->SetUInt32Value(PLAYER_FIELD_KILLS,PlayerKillCount);  
 pPlayer->SetUInt32Value(PLAYER_FIELD_TODAY_CONTRIBUTION, 0x00030004);      
 pPlayer->SetUInt32Value(PLAYER_FIELD_YESTERDAY_CONTRIBUTION, 0x000500006); 
 pPlayer->SetUInt32Value(PLAYER_FIELD_LIFETIME_HONORBALE_KILLS, 0x00070008); 
 pPlayer->SetUInt32Value(PLAYER_FIELD_HONOR_CURRENCY, 0x0009000A); 
 pPlayer->SetUInt32Value(PLAYER_FIELD_ARENA_CURRENCY, 0x000B000C);
 */
}
...
bool ChatHandler::HandleAddKillCommand(const char* args, WorldSession* m_session)
{
	...
	GreenSystemMessage(plr->GetSession(), "%u d honneur a ete ajoute a votre personnage.", KillAmount);
...
bool ChatHandler::HandleAddHonorCommand(const char* args, WorldSession* m_session)
{
	...
	GreenSystemMessage(plr->GetSession(), "%u points d honneur a ete ajoute a votre personnage.", HonorAmount);
