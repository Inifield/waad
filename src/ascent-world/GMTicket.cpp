...
void WorldSession::HandleGMTicketCreateOpcode(WorldPacket & recv_data)
{
	...
	//objmgr.remGMTicket(GetPlayer()->GetGUID()); Pourquoi re delete le ticket, ca causait un bug pour le modifier ?? ( Crash )
...
void WorldSession::HandleGMTicketUpdateOpcode(WorldPacket & recv_data)
{
	...
	objmgr.SaveGMTicket(ticket->guid, NULL); // SaveGMTicket beaucoup plus pratique ( Crash )
...
void WorldSession::HandleGMTicketGetTicketOpcode(WorldPacket & recv_data)
{
	...
	// Send current Ticket
	data << uint32(6); // unk
	data << uint32(1); // unk, 335.12340 permet d'avoir tout le texte ( Crash )
