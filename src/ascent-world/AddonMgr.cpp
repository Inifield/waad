...
bool AddonMgr::IsAddonBanned(std::string name, uint64 crc)
{
	...
	if(i != KnownAddons.end())
	{
		if(i->second->banned)
		{
			sLog.outDebug("L'addon %s est banni.", name.c_str());
			return true;
		}
	}
	else
	{
		...
		sLog.outDebug("Nouvel addon découvert %s envoyé par le client.", name.c_str());
...
void AddonMgr::LoadFromDB()
{
	...
	if(!result)
	{
		sLog.outString("Echec de requête: SELECT * FROM clientaddons");
		return;
	}
...
{
	sLog.outString("AddonMgr: Sauvegarde de tout les nouveaux addons découvert dans cette session dans la base de données.");
	for(std::map<std::string, AddonEntry*>::iterator itr = KnownAddons.begin();itr!=KnownAddons.end();++itr)
	{
		if(itr->second->isNew)
		{
			sLog.outDetail("Sauvegarde nouvel addon %s", itr->second->name.c_str());
			std::stringstream ss;
