...
void ClusterInterface::InitHandlers()
{
	...
	PHandlers[ISMSG_DELETE_PLAYER] = &ClusterInterface::HandleDeletePlayer;
...
string ClusterInterface::GenerateVersionString()
{
	...
	ss << ARCH;
	//char str[200];
	//snprintf(str, 200, "AscentWaad r%u/%s-%s-%s", BUILD_REVISION, CONFIG, PLATFORM_TEXT, ARCH);
	return ss.str();
}

void ClusterInterface::ForwardWoWPacket(uint16 opcode, uint32 size, const void * data, uint32 sessionid)
{
	Log.Debug("ForwardWoWPacket", "Transfert %s vers le serveur", LookupName(opcode, g_worldOpcodeNames));
	bool rv;
...
void ClusterInterface::ConnectToRealmServer()
{
	...
	Log.Notice("ClusterInterface", "Connection a %s port %u", hostname.c_str(), port);
    WSClient * s = ConnectTCPSocket<WSClient>(hostname.c_str(), port);
	if(!s)
	{
		Log.Error("ClusterInterface", "Ne peut pas se connecter a %s:%u", hostname.c_str(), port);
		return;
	}

	Log.Success("ClusterInterface", "Connection a %s:%u", hostname.c_str(), port);
...
void ClusterInterface::HandleAuthRequest(WorldPacket & pck)
{
	uint32 x;
	pck >> x;
	Log.Debug("ClusterInterface", "Arrivée d'une demande auth de %s (RS build %u)", _clientSocket->GetRemoteIP().c_str(), x);
	...
	Log.Notice("ClusterInterface", "La latence entre les serveurs de royaume est %u ms", m_latency);
}
...
void ClusterInterface::HandleAuthResult(WorldPacket & pck)
{
	uint32 res;
	pck >> res;
	Log.Debug("ClusterInterface", "Résultat Auth: %u", res);
	if(!res)
	{
		Log.Error("ClusterInterface", "Erreur d'authentication");
...
void ClusterInterface::Update()
{
	...
	while((pck = _pckQueue.Pop()))
	{
		...
		if(opcode < IMSG_NUM_TYPES && ClusterInterface::PHandlers[opcode] != 0)
			...
			Log.Error("ClusterInterface", "Paquet non traité %u\n", opcode);
	}
}
...
void ClusterInterface::HandleWoWPacket(WorldPacket & pck)
{
	...
	if(!_sessions[sid])
	{
		Log.Error("HandleWoWPacket", "Session invalide: %u", sid);
		return;
	}

	Log.Debug("HandleWoWPacket", "Transfert %s vers le client", LookupName(opcode, g_worldOpcodeNames));
...
void ClusterInterface::HandlePlayerChangedServers(WorldPacket & pck)
{
	...
	if(!_sessions[dsid])
	{
		Log.Error("HandlePlayerChangedServers", "Session invalide: %u", sessionid);
		return;
	}
...
void ClusterInterface::HandleTeleportResult(WorldPacket & pck)
{
	...
	else
	{
		...
		sEventMgr.AddEvent(s->GetPlayer(), &Player::HandlePlayerTeleportServers, EVENT_UNK, 1, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	}
}
...
void ClusterInterface::HandleTransporterMapChange(WorldPacket & pck)
{
	...
	Log.Debug("Transport", "Handling clustered map change");
	uint32 tentry, mapid;
	float x, y, z;
...
void ClusterInterface::HandleChannelAction(WorldPacket & pck)
{
	...
	default:
		{
			Log.Debug("ClusterInterface", "HandleChannelAction opcode, unhandled action %u", action);
			return;
		}
	}
}
