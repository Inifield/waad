...
void Creature::OnRemoveCorpse()
{
	...
	if (IsInWorld() && (int32)m_mapMgr->GetInstanceID() == m_instanceId)
	{
		...
		sLog.outDetail("Removing corpse %s (id: %u) (%0.3f,%0.3f,%0.3f)...",GetCreatureName()->Name,GetCreatureName()->Id,
...
void Creature::OnRespawn(MapMgr * m)
{
	...
	sLog.outDetail("Respawning corpse %s (id: %u) (%0.3f,%0.3f,%0.3f)...",GetCreatureName()->Name,GetCreatureName()->Id,
		                                                                                GetSpawnX(),GetSpawnY(),GetSpawnZ());

	// Reset procCounter des AISpells (si y'en a ;) (Branruz))
	if(this->GetAIInterface()->npc_spells.size() != 0)
	{
	 std::vector<AI_Spell*>::iterator itr = this->GetAIInterface()->npc_spells.begin(); 
	 for(;itr!=this->GetAIInterface()->npc_spells.end();itr++)
	 {
		 (*itr)->procCounter = 0;
	 }
	...
	{
		uint32 newhealth = (uint32)((float)m_uint32Values[UNIT_FIELD_HEALTH] / 100.0);
...
void Creature::SaveToDB()
{
	...
	ss << (IsVehicle() ? static_cast<Vehicle *>(this)->GetVehicleEntry() : 0) << ")";
...
void Creature::setDeathState(DeathState s) 
{
	if(s == JUST_DIED) 
	{
		...
		if(!IsPet() && lootmgr.IsSkinnable(GetEntry()))
...
void Creature::RemoveFromWorld(bool addrespawnevent, bool free_guid)
{
	...
	// A quoi sert ce test, on verifie plus bas si la creature est tjs sur la map
	// si c'est un familier, on clear la liste et le summoned, 
	// Sur ce test, le summoned n'est pas reset.
    /*
	if(IsPet()) // Is a pet: IsPet() actually returns false on a pet? o_X
	{
		if(IsInWorld()) Unit::RemoveFromWorld(true);
	...
	if(IsInWorld())
	{
		...
		if (GetUInt64Value(UNIT_FIELD_SUMMONEDBY) != 0)
		{
			...
			if (u != NULL) u->m_summonedCreatures.erase(this);
		}
...
void Creature::EnslaveExpire()
{
	...
	if(caster)
	{
		...
		data << uint64(0); // Reset de la barre d'action
...
void Creature::CalcResistance(uint32 type)
{
	...
	if (IsPet() && isAlive() && IsInWorld() && static_cast<Pet*>(this)->GetOwner() != NULL)
	{
...
void Creature::CalcStat(uint32 type)
{
	/*if(type >= 5) 
	{
		Log.Error("[Creature::CalcStat]","FATAL: Type >= 5 (%u) <--- Report this to devs.",type); 
		return;
	}
	
	//  m_auracast->second != NULL (test pas tout les Aura* de la map)
	if ( m_aurascast.size() < 1 )
	{
		Log.Error("[Creature::CalcStat]","FATAL: m_auracast.size() 0 ou NULL (Type %d) <--- Report this to devs.",
			                      type); 
		return;
	}*/
...
/*	
	...
	if (IsPet() && isAlive() && IsInWorld() && static_cast<Pet*>(this)->GetOwner() != NULL)
	{
		...
		else Log.Error(" [Creature::CalcStat]","(%s - %d) Pet NULL <- Report this to devs.",
                                       this->GetCreatureName()->Name,this->GetCreatureName()->Id);
	}
*/
}
...
void Creature::RegenerateHealth()
{
	...
	//Log.Warning("Creature::RegenerateHealth","(%s) HP: %u/%u",this->creature_info->Name,cur,mh);

	//though creatures have their stats we use some wierd formula for amt
	float amt = 0.0f;
	float lvl = float(getLevel());

	if(PctRegenModifier == 0.0f)   amt = lvl*2.0f;
	else if(PctRegenModifier > 0)  amt = (lvl*2.0f)*(1.0f+PctRegenModifier);
	else                           amt = (lvl*2.0f)*(-1.0f+PctRegenModifier);
...
void Creature::RegenerateMana()
{
	...
	// Le client gere le RegenerateMana, suffit d'etre synchro (Branruz)
	/*WorldPacket data(SMSG_POWER_UPDATE, 14);
	FastGUIDPack(data, GetGUID());
	data << uint8(POWER_TYPE_MANA);
	data << power;
...
void Creature::RegenerateFocus()
{
	...
	if(cur>=mm)return;
	float amt = 2.5f * PctPowerRegenModifier[POWER_TYPE_FOCUS];
	if(amt<=1.0) cur++; // Secu du rate < 1
	else
	cur+=(uint32)amt;
	SetUInt32Value(UNIT_FIELD_POWER3,(cur>=mm)?mm:cur);

	// Verifier de quel coté est géré le Focus
	/*WorldPacket data(SMSG_POWER_UPDATE, 14);
	FastGUIDPack(data, GetGUID());
...
void Creature::RegenerateEnergy()
{
    ...
    amt*=sWorld.getRate(RATE_POWER4); // Section Power Regen du fichier world.conf
	if(amt<=1.0) cur++; // Secu du rate < 1
	else
	cur += float2int32(amt);
...
void Creature::LooseRage(int32 decayValue)
{
 ...
 void Creature::LooseRunic(int32 decayValue)
{
...
bool Creature::Load(CreatureSpawn *spawn, uint32 mode, MapInfo *info)
{
	...
	Taille = 1; 
#else 
	CreatureHeight *cH = CreatureHeightStorage.LookupEntry(spawn->entry);
	if(!cH)
	{
		Log.Error("Creature","Load error, Creature %u, verify your creature_height table!",spawn->entry);
		Taille = 1;
	}
	else Taille = cH->Taille;
#endif
    //----------

	spawnid = spawn->id;
	m_Phase = spawn->phase; // Note Randdrick - 3.22 - : gestion du phasing pour les Npcs.
	...
	//Note Randdrick - Initialisation du Mana
	SetUInt32Value(UNIT_FIELD_POWER1, proto->Mana);
	SetUInt32Value(UNIT_FIELD_MAXPOWER1, proto->Mana);
	
	//Note Randdrick - Initialisation de la Rage
	SetUInt32Value(UNIT_FIELD_POWER2, 0);
	SetUInt32Value(UNIT_FIELD_MAXPOWER2, proto->Mana);
	
	//Note Randdrick - Initialisation du Focus
	SetUInt32Value(UNIT_FIELD_POWER3, proto->Mana);
	SetUInt32Value(UNIT_FIELD_MAXPOWER3, proto->Mana);
	
	//Note Randdrick - Initialisation de l'énergie
	SetUInt32Value(UNIT_FIELD_POWER4, proto->Mana);
	SetUInt32Value(UNIT_FIELD_MAXPOWER4, proto->Mana);
	
	//Note Randdrick - Initialisation de la puissance runique
	...
	//Note Randdrick : permet d'affecter un displayId pour les NPCs créés par un script.
	if(!(UNIT_FIELD_DISPLAYID,spawn->displayid))
	{
	...
	sLog.outDebug("Creatures","NPC %u (model %u) got scale %f, found in DBC %f", proto->Id, spawn->displayid, GetFloatValue(OBJECT_FIELD_SCALE_X), GetScale( dbcCreatureDisplayInfo.LookupEntry( spawn->displayid ))); 
	...
	SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID, proto->ItemEntry1);
	SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID+1, proto->ItemEntry2);
	SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID+2, proto->ItemEntry3);
...
//SETUP NPC FLAGS
	...
	if ( HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_TAXIVENDOR) || HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_DIRECTTAXI))
...
void Creature::Load(CreatureProto * proto_, float x, float y, float z,float o) 
{
	...
	// Classe et Race a faire (Branruz)
	//setClass();
	//setRace();

	SetFloatValue( OBJECT_FIELD_SCALE_X,( proto->Scale ? proto->Scale : GetScale( dbcCreatureDisplayInfo.LookupEntry( model ))));
	sLog.outDebug("Creatures","NPC %u (model %u) got scale %f, found in DBC %f", proto->Id, model, GetFloatValue(OBJECT_FIELD_SCALE_X), GetScale( dbcCreatureDisplayInfo.LookupEntry( model ))); 
...
void Creature::Despawn(int32 delay, uint32 respawntime)
{
	if(delay > 0) 
	{
		sEventMgr.AddEvent(this, &Creature::Despawn, 0, respawntime, EVENT_CREATURE_RESPAWN, delay, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		return;
	}
...
void Creature::SetGuardWaypoints()
{
	...
	for(int i = 1; i <= 4; i++)
	{
		...
		if( wp->z == NO_WMO_HEIGHT )
			wp->z = m_mapMgr->GetLandHeight(wp->x, wp->y, m_spawnLocation.z);
...
const char* Creature::GetObjectName()
{
	if (GetCreatureName() == NULL)
		return "Inconnu";
	return GetCreatureName()->Name;
}
