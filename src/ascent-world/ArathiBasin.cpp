 ...
 * Copyright (C) 2005-2008 Ascent Team <http://www.ascentcommunit.com/>
 *
...
void ArathiBasin::SpawnBuff(uint32 x)
{
	...
	if(m_buffs[x] == NULL)
	{
		...
		m_buffs[x]->SetByte(GAMEOBJECT_BYTES_1, 0, 1); //SetUInt32Value(GAMEOBJECT_STATE, 1);
		m_buffs[x]->SetByte(GAMEOBJECT_BYTES_1, 1, 6); //SetUInt32Value(GAMEOBJECT_TYPE_ID, 6);
		m_buffs[x]->SetByte(GAMEOBJECT_BYTES_1, 3, 100); //SetUInt32Value(GAMEOBJECT_ANIMPROGRESS, 100);
...
void ArathiBasin::SpawnControlPoint(uint32 Id, uint32 Type)
{
	...
	if(m_controlPoints[Id] == NULL)
	{
		...
		m_controlPoints[Id]->SetByte(GAMEOBJECT_BYTES_1, 0, 1); // SetUInt32Value(GAMEOBJECT_STATE, 1);
		m_controlPoints[Id]->SetByte(GAMEOBJECT_BYTES_1, 1, gi->Type); //SetUInt32Value(GAMEOBJECT_TYPE_ID, gi->Type);
		m_controlPoints[Id]->SetByte(GAMEOBJECT_BYTES_1, 3, 100); //SetUInt32Value(GAMEOBJECT_ANIMPROGRESS, 100);
	...
	if(m_controlPointAuras[Id] == NULL)
	{
		...
		m_controlPointAuras[Id]->SetByte(GAMEOBJECT_BYTES_1, 0, 1); //SetUInt32Value(GAMEOBJECT_STATE, 1);
		m_controlPointAuras[Id]->SetByte(GAMEOBJECT_BYTES_1, 1, 6); //SetUInt32Value(GAMEOBJECT_TYPE_ID, 6);
		m_controlPointAuras[Id]->SetByte(GAMEOBJECT_BYTES_1, 3, 100); //SetUInt32Value(GAMEOBJECT_ANIMPROGRESS, 100);
...
void ArathiBasin::OnCreate()
{
	...
	//gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_1,-0.0602880f);
	...
	//gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_1,0.0151270f);
	...
	// urrrgh worldstates
	wstate.CreateWorldState(0x8D8, 0x00);
	wstate.CreateWorldState(0x8D7, 0x00);
	wstate.CreateWorldState(0x8D6, 0x00);
	wstate.CreateWorldState(0x8D5, 0x00);
	wstate.CreateWorldState(0x8D4, 0x00);
	wstate.CreateWorldState(0x8D3, 0x00);

	// AB world state's
	// unknowns, need more research
	wstate.CreateWorldState(0x7A3, 1800); // unknown
	wstate.CreateWorldState(0x745, 0x02); // unknown

	// Icon stuff for on the map
	wstate.CreateWorldState(AB_SHOW_BACKSMITH_ICON,  0x01);
	wstate.CreateWorldState(AB_SHOW_FARM_ICON, 		 0x01);
	wstate.CreateWorldState(AB_SHOW_LUMBERMILL_ICON, 0x01);
	wstate.CreateWorldState(AB_SHOW_GOLDMINE_ICON, 	 0x01);
	wstate.CreateWorldState(AB_SHOW_STABLE_ICON, 	 0x01);

	// LumberMill
	wstate.CreateWorldState(AB_CAPTURING_LUMBERMILL_HORDE, 	  0x00);
	wstate.CreateWorldState(AB_CAPTURING_LUMBERMILL_ALLIANCE, 0x00);
	wstate.CreateWorldState(AB_CAPTURED_LUMBERMILL_HORDE, 	  0x00);
	wstate.CreateWorldState(AB_CAPTURED_LUMBERMILL_ALLIANCE,  0x00);

	// GoldMine
	wstate.CreateWorldState(AB_CAPTURING_GOLDMINE_HORDE, 	0x00);
	wstate.CreateWorldState(AB_CAPTURING_GOLDMINE_ALLIANCE, 0x00);
	wstate.CreateWorldState(AB_CAPTURED_GOLDMINE_HORDE, 	0x00);
	wstate.CreateWorldState(AB_CAPTURED_GOLDMINE_ALLIANCE, 	0x00);

	// BlackSmith
	wstate.CreateWorldState(AB_CAPTURING_BLACKSMITH_HORDE, 	  0x00);
	wstate.CreateWorldState(AB_CAPTURING_BLACKSMITH_ALLIANCE, 0x00);
	wstate.CreateWorldState(AB_CAPTURED_BLACKSMITH_HORDE, 	  0x00);
	wstate.CreateWorldState(AB_CAPTURED_BLACKSMITH_ALLIANCE,  0x00);

	wstate.CreateWorldState(AB_MAX_SCORE, 			 RESOURCES_WINVAL);
	wstate.CreateWorldState(AB_ALLIANCE_CAPTUREBASE, 0x00);
	wstate.CreateWorldState(AB_HORDE_CAPTUREBASE, 	 0x00);
	wstate.CreateWorldState(AB_HORDE_RESOURCES, 	 0x00);
	wstate.CreateWorldState(AB_ALLIANCE_RESOURCES, 	 0x00);

	// Farm
	wstate.CreateWorldState(AB_CAPTURING_FARM_ALLIANCE, 0x00);
	wstate.CreateWorldState(AB_CAPTURING_FARM_HORDE, 	0x00);
	wstate.CreateWorldState(AB_CAPTURED_FARM_HORDE, 	0x00);
	wstate.CreateWorldState(AB_CAPTURED_FARM_ALLIANCE, 	0x00);

	// Stables
	wstate.CreateWorldState(AB_CAPTURING_STABLES_HORDE,    0x00);
	wstate.CreateWorldState(AB_CAPTURING_STABLES_ALLIANCE, 0x00);
	wstate.CreateWorldState(AB_CAPTURED_STABLES_HORDE, 	   0x00);
	wstate.CreateWorldState(AB_CAPTURED_STABLES_ALLIANCE,  0x00);
...
void ArathiBasin::OnStart()
{
	...
	for(list<GameObject*>::iterator itr = m_gates.begin(); itr != m_gates.end(); ++itr)
	{
		...
		(*itr)->SetByte(GAMEOBJECT_BYTES_1, 0, 0); //SetUInt32Value(GAMEOBJECT_STATE, 0);
	}
...
ArathiBasin::ArathiBasin(MapMgr * mgr, uint32 id, uint32 lgroup, uint32 t) : CBattleground(mgr,id,lgroup,t)
{
	...
	// Ajout Branruz: etat initial du BG, Flag neutre.....
	DefFlag[AB_CONTROL_POINT_STABLE][0]     = false; DefFlag[AB_CONTROL_POINT_STABLE][1]     = true;
	DefFlag[AB_CONTROL_POINT_FARM][0]       = false; DefFlag[AB_CONTROL_POINT_FARM][1]       = true;
	DefFlag[AB_CONTROL_POINT_BLACKSMITH][0] = false; DefFlag[AB_CONTROL_POINT_BLACKSMITH][1] = true;
	DefFlag[AB_CONTROL_POINT_MINE][0]       = false; DefFlag[AB_CONTROL_POINT_MINE][1]       = true;
	DefFlag[AB_CONTROL_POINT_LUMBERMILL][0] = false; DefFlag[AB_CONTROL_POINT_LUMBERMILL][1] = true;
}
...
void ArathiBasin::EventUpdateResources(uint32 Team)
{
	...
	m_mapMgr->GetDBStateManager().UpdateWorldState(resource_fields[Team], current_resources);
	

	if(current_resources >= RESOURCES_WARNING_THRESHOLD && !m_nearingVictory[Team])
	{
		m_nearingVictory[Team] = true;
		SendChatMessage(Team ? CHAT_MSG_BG_EVENT_HORDE : CHAT_MSG_BG_EVENT_ALLIANCE, (uint64)0, "%s a atteint %u points de resources et est proche de la victoire!", Team ? "La Horde" : "L'Alliance", current_resources);
	...
	if(current_resources == RESOURCES_WINVAL)
	{
		...
		sEventMgr.AddEvent(((CBattleground*)this), &CBattleground::Close, EVENT_BATTLEGROUND_CLOSE, 60000, 1,0);
        
        SendChatMessage(Team ? CHAT_MSG_BG_EVENT_HORDE : CHAT_MSG_BG_EVENT_ALLIANCE   , (uint64)0, "%s gagne!", Team ? "La Horde" : "L'Alliance");
	
		/* add the marks of honor to all players */
		m_mainLock.Acquire();

		SpellEntry * winner_spell = dbcSpell.LookupEntry(__Arathi_Basin_Mark_of_Honor__Winner_); // 24953
		SpellEntry * loser_spell = dbcSpell.LookupEntry(__Arathi_Basin_Mark_of_Honor__Loser_);   // 24952
		for(uint32 i = 0; i < 2; ++i)
		{
			for(set<Player*>::iterator itr = m_players[i].begin(); itr != m_players[i].end(); ++itr)
			{
...
void ArathiBasin::OnAddPlayer(Player * plr)
{
	...
	//plr->m_bgScore.MiscScore[0] = 0;
	//plr->m_bgScore.MiscScore[1] = 0;
	UpdatePvPData();
}
...
{
	// nothing?
	printf("Arathi: HookFlagDrop Trig!!\n");
}

void ArathiBasin::HookFlagStand(Player * plr, GameObject * obj)
{
	// nothing?
	printf("Arathi: HookFlagStand Trig!!\n");
}
...
void ArathiBasin::HookOnAreaTrigger(Player * plr, uint32 id)
{
	uint32 spellid=0;
	int32 buffslot = -1;
	//char _buff_debug_[80]; // Debug (Branruz)
    ...
    // Je les rajoute au cas ou
    case 4020 : // Arathi Basin, Trollbane Hall
		  Log.Warning("HookOnAreaTrigger","Areatrigger id %u, Hall de Trollemort non utilisé!", id);
		  return;

	case 4021 : // Arathi Basin, Defiler's Den
         Log.Warning("HookOnAreaTrigger","Areatrigger id %u, Antre des profanateurs non utilisé!", id);
		  return;

	default:
		Log.Error("ArathiBasin", "Areatrigger non traité id %u rencontré", id);
		return;
		break;
	}
	...
	{
		// apply the spell
		spellid = m_buffs[x]->GetInfo()->trap.spellId; // sound3
		m_buffs[x]->RemoveFromWorld(false);
...
void ArathiBasin::CaptureControlPoint(uint32 Id, uint32 Team)
{
	...
	PlaySoundToAll(Team ? SOUND_HORDE_CAPTURE : SOUND_ALLIANCE_CAPTURE); 
     ...
     SendChatMessage(Team ? CHAT_MSG_BG_EVENT_HORDE : CHAT_MSG_BG_EVENT_ALLIANCE, 0, "%s a pris le contrôle de %s!", Team ? "La Horde" : "L'Alliance", ControlPointNames[Id]);
   	 DefFlag[Id][0] = false;
     DefFlag[Id][1] = false;
...
void ArathiBasin::AssaultControlPoint(Player * pPlayer, uint32 Id)
{
	...
	if(!m_started)
	{
		SendChatMessage(CHAT_MSG_BG_EVENT_NEUTRAL, pPlayer->GetGUID(), "%s a été virer du jeux pour tricherie!", pPlayer->GetName());
    ...
    if(DefFlag[Id][0] && !DefFlag[Id][1] ) // Si le drap est neutre pas de defense mais prise
	{
     ...
     SendChatMessage(Team ? CHAT_MSG_BG_EVENT_HORDE : CHAT_MSG_BG_EVENT_ALLIANCE, pPlayer->GetGUID(), "$N a défendu %s", ControlPointNames[Id]);
	 ...
	 pPlayer->m_bgScore.MiscScore[1]++;
	 UpdatePvPData();
	}
	else
	{
     DefFlag[Id][0] = true; // En prise de Flag	(Assault) 
	 SendChatMessage(Team ? CHAT_MSG_BG_EVENT_HORDE : CHAT_MSG_BG_EVENT_ALLIANCE, pPlayer->GetGUID(), "$N a pris %s! Si personne n'intervient, %s en prendra le contrôle dans 1 minute!", ControlPointNames[Id],
		Team ? "la Horde" : "l'Alliance");
