...
void QuestMgr::BuildOfferReward(WorldPacket *data, Quest* qst, Object* qst_giver, uint32 menutype, uint32 language,Player *plr)
{
	...
	*data << qst->count_reward_item;
	...
	//Gestion des récompenses en Xps, argents, et argent à la place d'Xps par Randdrick
	if (plr->getLevel() >= sWorld.m_levelCap)
	{
		*data << uint32(qst->reward_xp_as_money);
		*data << uint32(0); // Pas d'Xps de gagnés si le niveau du joueur est supérieur au niveau maximum
	}
	else
	{
		uint32 xp = 0;
...
void QuestMgr::BuildQuestDetails(WorldPacket *data, Quest* qst, Object* qst_giver, uint32 menutype, uint32 language,Player *plr)
{
	...
	*data << uint8(0); //new 3.3 - if you set it to 1 then it'll show the quest panel without the decline button, however the accept button won't work either, just closes the dialog. Probably the client expects the server to auto-accept the quest without client side intervention.*/
	...
	if(qst->count_reward_choiceitem > 5) // 6 maximum, de 0 à 5 
	{
	...
	*data << qst->count_reward_choiceitem; 
	...
	if(qst->count_reward_item > 3) // 4 maximum, de 0 à 3 
	{
      Log.Error("SMSG_QUESTGIVER_QUEST_DETAILS",
...
{
	//Log.Notice("SendQuestUpdateAddKill","Player %s, Mob/Pnj Guid tué %u",plr->GetName(),guid); // debug (Branruz)
...
// ATTENTION: A verif, les reput sont maintenant passées dans les trames reseaux
//            vers le client (Les reputs seraient maintenant géré par le client ??)
...
*/	
    ...
    // Inspiré de Mangos et adapté pour Waad par Randdrick
    for(int z = 0; z < 5; z++)
    {
        if (!qst->reward_repfaction[z])
            continue;

        if (qst->reward_repvalueIndex[z] && qst->reward_repvalueIndex[z] > 10) // Permet de gérer la réputation en tant que valeur si besoin
        {
		  uint32 amt = 0;
		  uint32 fact = qst->reward_repfaction[z];
...
{	
 ...
 if(plr->getLevel() >= sWorld.m_levelCap) return((uint32)0); // Evite l'xp si levelmax
	...
	// gestion du nouveau système d'XP en provenance de Mangos, adaptée pour WAAD
	else if (plr)
    {
        uint32 realXP = 0;
...
// Généralisation d'apres une idée d'HearthStone (Branruz)
