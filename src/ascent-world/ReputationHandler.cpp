...
{
	if( Standing_ >= 42000 )  	  return STANDING_EXALTED;  
	else if( Standing_ >= 21000 ) return STANDING_REVERED;  
	else if( Standing_ >= 9000 )  return STANDING_HONORED;  
	else if( Standing_ >= 3000 )  return STANDING_FRIENDLY;  
	else if( Standing_ >= 0 )     return STANDING_NEUTRAL;  
	else if( Standing_ > -3000 )  return STANDING_UNFRIENDLY;  
	else if( Standing_ > -6000 )  return STANDING_HOSTILE;  
...
void Player::SetStanding(uint32 Faction, int32 Value)
{
	...
	{
	  Log.Notice("[Player::SetStanding]","Maj Reputation");
	}
	...
	else
	{
		...
		if(IsInWorld())
		{
			if(sWorld.m_DebugPlayers)
	        {
				Log.Notice("[Player::SetStanding]","SMSG_SET_FACTION_STANDING: PreListId: %u , Reputation: %u",dbc->RepListId,itr->second->CalcStanding());
	        }
...
void Player::ModStanding(uint32 Faction, int32 Value)
{
	...
	if(dbc == 0) 
	{
		Log.Error("[Player::ModStanding]","La faction %u n'existe pas!, Version DBCs ou Infos Base errones!",Faction);
		return;
	}

	if(sWorld.m_DebugPlayers)
	{
      Log.Notice("[Player::ModStanding]","Maj Reputation");
	}
	...
	else
	{
		...
		if(IsInWorld() && Visible(itr->second->flag))
		{
			if(sWorld.m_DebugPlayers)
	        {
				Log.Notice("[Player::ModStanding]","SMSG_SET_FACTION_STANDING: PreListId: %u , Reputation: %u",dbc->RepListId,itr->second->CalcStanding());
	        }
