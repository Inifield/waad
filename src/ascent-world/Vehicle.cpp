...
void Vehicle::InitSeats(uint32 vehicleEntry, Player* pRider)
{
	...
	//Initialisation de l'énergie pour le vehicule par Randdrick
	for (int i=0; i<3; ++i)
	{
		m_powerveh = ve->m_powerType[i];
		...
		switch ( m_powerveh )
        {
            ...
            case POWER_WRATH:
                ...
                me->SetPower(POWER_TYPE_ENERGY, 100);
                break;
            case POWER_PYRITE:
                me->SetPowerType(POWER_TYPE_ENERGY);
                me->SetPower(POWER_TYPE_ENERGY, 50);
                break;
            default:
                ...
                for (list<AI_Spell*>::iterator itr = GetAIInterface()->m_spells.begin(); itr != GetAIInterface()->m_spells.end(); ++itr)
                {
                    ...
                    if ((*itr)->spell->powerType == POWER_TYPE_MANA)
                        break;

                    if ((*itr)->spell->powerType == POWER_TYPE_ENERGY)
                    {
                        me->SetPowerType(POWER_TYPE_ENERGY);
                        me->SetPower(POWER_TYPE_ENERGY, 100);
                        break;
